#!/usr/bin/env node
// Gera supabase/aplicar_macro_lote100_teste_rollback.sql
// a partir de scripts/macro-lote100-explicacoes.mjs.

import fs from 'fs';
import { explicacoes } from './macro-lote100-explicacoes.mjs';

const OUT_PATH = 'supabase/aplicar_macro_lote100_teste_rollback.sql';
const ADMIN_USUARIO_ID = 'e5523807-6cc8-4867-8a56-77c17552e56e';

if (explicacoes.length !== 100) {
  throw new Error(`Esperado 100 explicacoes, encontrado ${explicacoes.length}`);
}

const ids = explicacoes.map(e => e.id);
const uniqueIds = new Set(ids);
if (uniqueIds.size !== 100) {
  throw new Error(`Ha IDs duplicados no array de explicacoes! Total unicos: ${uniqueIds.size}`);
}
...
const idsLiteralArray = `ARRAY[${ids.join(',')}]::bigint[]`;
const idsLiteralList = ids.join(',');
...
console.log(`Harness gerado com sucesso: ${OUT_PATH}`);
