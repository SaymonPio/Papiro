#!/usr/bin/env node
// LOTE 2 — FASE 3B — SUB-LOTE 1: validação estrutural das 38 explicações,
// nos mesmos dois eixos usados no Lote 1 (ver scripts/validar-sublote1..4-
// explicacoes.mjs), adaptada ao formato de dado do Lote 2: aqui não existe
// ainda `id`/`correta`/`ordem` do banco -- a fonte de verdade é
// fase3b_sublote1_conteudo.json (chave `tecId`, `gabarito` como letra
// A-E ou "Certo"/"Errado", `alternativas` como array na ordem A,B,C,D,E).
//
// Camada 1: cobertura estrutural -- GABARITO bate com o campo `gabarito` do
// registro-fonte; toda alternativa realmente existente tem uma seção
// "POR QUE A ALTERNATIVA <letra> ESTÁ (CORRETA|INCORRETA)" (nem faltando,
// nem sobrando letra); BIZU DE PROVA sempre presente; PEGADINHA presente
// nas Certo/Errado.
// Camada 2: reconciliação de IDs -- 38 registros, 38 tecIds únicos, nenhum
// tecId do sub-lote ausente da explicação e vice-versa.
//
// Só leitura -- não escreve nada, não toca Supabase.

import fs from 'fs';
import { explicacoes } from './lote2-fase3b-sublote1-explicacoes.mjs';
...
const sublote = JSON.parse(fs.readFileSync(`${SCRATCH}/fase3b_sublote1_conteudo.json`, 'utf8'));
...
// --- Camada 2: reconciliação de IDs -----------------------------------
const idsExplicacoes = explicacoes.map(e => e.tecId);
const idsUnicos = new Set(idsExplicacoes);
console.log('=== Reconciliação de IDs ===');
console.log('Registros no sub-lote 1 (fonte):', sublote.length, '(esperado 37 -- caderno 225 removido por PROBLEMATICA_GABARITO_AMBIGUO)');
console.log('Explicações escritas:', explicacoes.length, '(esperado 37)');
console.log('tecIds únicos nas explicações:', idsUnicos.size, '(esperado 37)');
...
// --- Camada 1: cobertura estrutural -------------------------------------
for (const { tecId, cadernoNumero, explicacao } of explicacoes) {
  const q = porTecId.get(tecId);
  if (!q) { console.error(`FALHA ${tecId}: nao encontrada em fase3b_sublote1_conteudo.json`); falhas++; continue; }
...
console.log('\n=== VALIDA_COM_RESSALVA no sub-lote 1 ===');
console.log('Cadernos:', JSON.stringify(comRessalva), `(${comRessalva.length} no total)`);
