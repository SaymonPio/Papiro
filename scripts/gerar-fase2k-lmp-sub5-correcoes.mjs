#!/usr/bin/env node
// Fase 2K — sub-lote 5 (Lei Maria da Penha): fecha as 3 pendencias
// confirmadas na auditoria (auditoria/fase2k_lmp_sub5_resultado.json):
//
//   - id 1358: CORRECAO_EXPLICACAO. A explicacao atribuia ao "art. 12-B"
//     a autoridade da policia para afastar o agressor sem autorizacao
//     judicial previa. Confirmado (com correcao de um resultado de fetch
//     inconsistente, descartado apos checagem cruzada) que o dispositivo
//     correto e o art. 12-C (Lei 13.827/2019, validado pelo STF na ADI
//     6.138) -- ja usado com esse mesmo conteudo nas questoes 348, 1352,
//     1353 e 1371. Nao afeta o gabarito (E).
//   - id 1363: CORRECAO_EXPLICACAO. A explicacao afirmava, sem ressalva,
//     que "o STJ ja decidiu" que a prisao preventiva nao pode ser
//     decretada de oficio -- mas a pesquisa feita para a questao 1335
//     (Sub-lote 4) encontrou jurisprudencia do STJ DIVIDIDA sobre esse
//     ponto (Enunciado 51 do FONAVID sustenta o contrario, pela
//     especialidade do art. 20). Nao afeta o gabarito (E), que depende
//     de um fundamento totalmente independente (prazo "de imediato" do
//     art. 10, nao "48 horas").
//   - id 1359: higiene de dado, NAO e correcao juridica. O enunciado usa
//     "$ 8º" e "$ 1º" em vez de "§ 8º" e "§ 1º" -- residuo de OCR/
//     scraping confirmado, as 2 unicas ocorrencias do caractere errado
//     no enunciado inteiro. Corrige so os 2 caracteres, byte a byte.
//
// id 1361 NAO esta neste harness -- por instrucao explicita do usuario,
// nao deve ser alterada so por formatacao/minusculas.
//
// Em todos os 3 casos: NAO se reescreve gabarito, alternativas nem
// nenhum outro trecho alem do exatamente descrito. Gera harness
// (ROLLBACK) e aplicacao real (COMMIT) do MESMO corpo, mesmo padrao dos
// hotfixes anteriores desta fase.
...
const HARNESS_OUT_PATH = path.join(ROOT, 'supabase/fase2k_lmp_sub5_correcoes_teste_rollback.sql');
const APPLY_OUT_PATH = path.join(ROOT, 'supabase/fase2k_lmp_sub5_correcoes.sql');
...
// concurso|materia_id|assunto_id|ativa) e da explicacao (CRLF
// normalizado para LF), capturados AO VIVO via MCP read-only
// imediatamente antes de gerar este script -- precondicao: se o estado
// mudou desde a auditoria, a transacao inteira aborta.
...
begin
  if not exists (select 1 from public.questoes where id = 1358 and ativa = true
      ...
      and md5(regexp_replace(explicacao, E'\\r\\n', E'\\n', 'g')) = ${sqlStr(HASH_EXPLICACAO_ANTES[1358])}) then
  ...
  if not exists (select 1 from public.questoes where id = 1363 and ativa = true
      ...
      and md5(regexp_replace(explicacao, E'\\r\\n', E'\\n', 'g')) = ${sqlStr(HASH_EXPLICACAO_ANTES[1363])}) then
  ...
  if not exists (select 1 from public.questoes where id = 1359 and ativa = true
      ...
      and md5(regexp_replace(explicacao, E'\\r\\n', E'\\n', 'g')) = ${sqlStr(HASH_EXPLICACAO_ANTES[1359])}) then
...
begin
  update public.questoes set explicacao = ${sqlStr(EXPLICACAO_NOVA_1358)} where id = 1358;
...
declare v_linhas int;
begin
  update public.questoes set explicacao = ${sqlStr(EXPLICACAO_NOVA_1363)} where id = 1363;
  get diagnostics v_linhas = row_count;
  if v_linhas <> 1 then
...
declare v_linhas int;
begin
  update public.questoes set enunciado = ${sqlStr(ENUNCIADO_NOVO_1359)} where id = 1359;
  get diagnostics v_linhas = row_count;
  if v_linhas <> 1 then
