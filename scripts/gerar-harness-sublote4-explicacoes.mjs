#!/usr/bin/env node
// Gera o harness transacional (BEGIN...UPDATE...ROLLBACK) do SUB-LOTE 4 de
// explicacoes (Lei Maria da Penha, 31 questoes, ids listados abaixo). So
// gera o arquivo -- nao toca o Supabase. A aplicacao real (COMMIT) so sera
// gerada depois que o usuario validar este harness no SQL Editor, mesmo
// fluxo dos sub-lotes 1, 2 e 3.
//
// Diferenca em relacao aos sub-lotes 1-3: estas 31 questoes JA EXISTIAM no
// banco (nao fazem parte do Lote 1 de importacao) e estavam classificadas
// EXPLICACAO_INCOMPLETA -- tinham texto boilerplate ("Gabarito indicado no
// Caderno TEC...") sem nenhum marcador estrutural. Auditadas na Fase 1
// (aprovada pelo usuario) contra o texto oficial consolidado da Lei
// 11.340/2006 vigente em 2026. id 738, que fazia parte do conjunto
// original de 32 EXPLICACAO_INCOMPLETA, foi excluido (PROBLEMATICA/
// FUNDAMENTO INCERTO) e nao entra neste harness.
//
// Precondicao ADAPTADA (nao removida) para este sub-lote especificamente,
// por decisao explicita do usuario: em vez de exigir "nenhuma inativa" (o
// padrao dos sub-lotes 1-3), este harness exige uma composicao EXATA --
// 28 questoes ativas + exatamente 3 inativas, e essas 3 tem que ser
// especificamente 863, 864 e 865 (gemeas identicas de 134, 129 e 133,
// respectivamente, ja ativas e ja corrigidas neste mesmo sub-lote). Se
// QUALQUER outra questao do lote estiver inativa, ou se a composicao for
// diferente de 28+3, o harness aborta -- a excecao NAO e uma remocao
// generica da protecao, e sim uma regra mais especifica no lugar dela.
//
// O harness prova, por hash e por assert dedicado, que o campo `ativa` de
// TODAS as 31 (incluindo essas 3) permanece exatamente como estava depois
// do UPDATE -- ou seja, 863/864/865 continuam ativa=false, as outras 28
// continuam ativa=true, e em nenhum caso o UPDATE toca esse campo. So o
// texto de explicacao muda. Reaproveitar o mesmo texto nas 3 gemeas
// inativas significa que, se forem reativadas no futuro, ja estarao com
// explicacao completa.
//
...
const OUT_PATH = path.join(ROOT, 'supabase/sublote4_lei_maria_penha_explicacoes_teste_rollback.sql');
...
if (ids.length !== 31) throw new Error(`Esperado 31 questoes, encontrado ${ids.length}`);
if (idsUnicos.size !== 31) throw new Error(`Esperado 31 ids UNICOS, encontrado ${idsUnicos.size} (ha id duplicado no array-fonte)`);
if (idsUnicos.has(738)) throw new Error('id 738 nao pode estar neste sub-lote (PROBLEMATICA/FUNDAMENTO INCERTO)');
