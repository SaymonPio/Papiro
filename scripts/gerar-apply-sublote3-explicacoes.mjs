#!/usr/bin/env node
// Gera a aplicacao real (BEGIN...UPDATE...COMMIT) do SUB-LOTE 3, extraindo
// o corpo (staging + precondicoes + UPDATE) DIRETAMENTE do texto do harness
// ja validado no SQL Editor (supabase/sublote3_lei_maria_penha_explicacoes_
...
// arriscaria uma divergencia acidental de texto entre o que foi validado e
// o que sera aplicado de verdade (mesma tecnica usada no sub-lote 2). So
// gera o arquivo -- nao toca o Supabase.
...
const HARNESS_PATH = path.join(ROOT, 'supabase/sublote3_lei_maria_penha_explicacoes_teste_rollback.sql');
const APPLY_OUT_PATH = path.join(ROOT, 'supabase/sublote3_lei_maria_penha_explicacoes.sql');
...
// Sanidade: confirma que o corpo extraido tem exatamente as 5 questoes
// esperadas.
const idsNoStaging = [...body.matchAll(/^\s*\((\d+),/gm)].map(m => Number(m[1]));
if (idsNoStaging.length !== 5) {
  throw new Error(`Corpo extraido tem ${idsNoStaging.length} linhas de staging, esperado 5`);
}
if (!body.includes('if v_total <> 5 then') || !body.includes('update public.questoes q')) {
...
-- Confirma a escrita: 5 questões (id ${idsListaSql}) passam a ter
