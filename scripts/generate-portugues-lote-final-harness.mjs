#!/usr/bin/env node
// Gera supabase/aplicar_portugues_lote_final_teste_rollback.sql
// a partir de scripts/portugues-lote-final-explicacoes.mjs.

import fs from 'fs';
import { explicacoes } from './portugues-lote-final-explicacoes.mjs';

const IDS_ESPERADOS = [
  883, 884, 885, 886, 887, 888, 889, 890, 891, 892, 893, 894
];
const OUT_PATH = 'supabase/aplicar_portugues_lote_final_teste_rollback.sql';
const ADMIN_USUARIO_ID = 'e5523807-6cc8-4867-8a56-77c17552e56e';

if (explicacoes.length !== 12) throw new Error(`esperado 12 explicacoes, encontrado ${explicacoes.length}`);
...
begin
  ...
  select 'os ids afetados sao exatamente os 12 esperados',
    (select array_agg(id order by id) from _lpf_ids_afetados) = ${idsLiteralArray};
