#!/usr/bin/env node
// Gera supabase/aplicar_rlm_matematica_lote1_teste_rollback.sql
// a partir de scripts/rlm-matematica-lote1-explicacoes.mjs.
...
const OUT_PATH = 'supabase/aplicar_rlm_matematica_lote1_teste_rollback.sql';
const ADMIN_USUARIO_ID = 'e5523807-6cc8-4867-8a56-77c17552e56e';

if (explicacoes.length !== 38) throw new Error(`esperado 38 explicacoes, encontrado ${explicacoes.length}`);
...
begin
  ...
  select 'os ids afetados sao exatamente os 38 esperados',
    (select array_agg(id order by id) from _rlm1_ids_afetados) = ${idsLiteralArray};
