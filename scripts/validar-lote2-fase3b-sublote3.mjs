#!/usr/bin/env node
// LOTE 2 — FASE 3B — SUB-LOTE 3: validação estrutural das 38 explicações.
// Mesmo esquema de validação dos sub-lotes anteriores. Fonte de verdade:
// fase3b_sublote3_conteudo.json.

import fs from 'fs';
import { explicacoes } from './lote2-fase3b-sublote3-explicacoes.mjs';
...
const sublote = JSON.parse(fs.readFileSync(`${SCRATCH}/fase3b_sublote3_conteudo.json`, 'utf8'));
...
console.log('Registros no sub-lote 3 (fonte):', sublote.length, '(esperado 38)');
...
for (const { tecId, cadernoNumero, explicacao } of explicacoes) {
  ...
  if (!q) { console.error(`FALHA ${tecId}: nao encontrada em fase3b_sublote3_conteudo.json`); falhas++; continue; }
...
console.log('\n=== VALIDA_COM_RESSALVA no sub-lote 3 ===');
