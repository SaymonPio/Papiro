...
6. **Granularidade de `artigos_esperados` (regra definitiva).** `artigos_esperados` deve registrar a maior granularidade normativa confirmada e efetivamente cobrada pelas questões — artigo, caput, inciso, parágrafo ou alínea — sem inventar subdivisões. Uma subdivisão (ex.: "art. 29, VI", "art. 37, § 13", "art. 12, I, b") só entra na lista quando uma questão a testa com segurança jurídica suficiente para identificá-la; havendo dúvida, mantém-se o nível superior (o artigo inteiro, ou o artigo genérico ao lado do caput/inciso confirmado). Não se substitui um artigo inteiro por uma subdivisão quando outras questões do mesmo conteúdo cobrarem o caput ou outras partes do artigo — nesse caso a lista contém o artigo genérico e a(s) subdivisão(ões) específica(s) lado a lado, cada uma cobrindo a questão que a testa. **`artigos_esperados` deve refletir os dispositivos normativos efetivamente cobertos pela questão, inclusive assertivas e alternativas relevantes — não apenas o dispositivo que fundamenta o gabarito.** Uma alternativa incorreta/distratora que testa um dispositivo real (mesmo que de forma invertida ou por contraste com o gabarito) também conta como cobertura, porque `artigos_esperados` é a base para o alcance da futura aula, não um resumo do gabarito. Precedente real: no saneamento de Constituição Federal de 1988 e Constituição do Estado do Rio Grande do Sul, revisar todas as assertivas (não só os gabaritos) de questões com múltiplos itens elevou a lista de 6 para 27 referências (CF) e de 18 para 37 referências (CE/RS).
