...
-- Tautologia, contradicao e contingencia (curso_conteudos.id = 8).
--
...
-- exclusoes. 3 questoes classificadas: 2 rows REAL (Fundatec: Q76, Q85)
-- + 1 AUTORAL (Q315). Nenhum saneamento previo foi necessario nesta
-- ordem.

-- 1) A unidade pedagogica do conteudo 8. Esperado: 1 linha, ordem 1,
...
-- 4) QIDs vinculados exatos. Esperado: 76, 85, 315.
...
-- 6) Questoes ativas do conteudo (assunto_id=37) que ficaram SEM nenhuma
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 8
  );
...
select q.id, q.banca, q.ano,
  (select count(*) from public.questao_unidades_pedagogicas qup2 join public.unidades_pedagogicas u2 on u2.id=qup2.unidade_pedagogica_id where qup2.questao_id=q.id and u2.curso_conteudo_id=8) as vinculo_neste_conteudo
from public.questoes q
where q.ativa = true and q.materia_id = 18 and q.assunto_id = 37
order by q.id;

-- 9) Confirma que a Q75 (candidata externa descartada pela
--    microauditoria) permanece intacta: ativa, com seu assunto_id
--    original (38, Tabela-verdade), e SEM vinculo neste conteudo.
--    Esperado: assunto_id=38, vinculos_neste_conteudo=0.
select id, ativa, assunto_id,
  (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where qup.questao_id = q.id and u.curso_conteudo_id = 8) as vinculos_neste_conteudo
from public.questoes q
where id = 75;
...
--     sistema.
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 8) as artigos_esperados_null,
