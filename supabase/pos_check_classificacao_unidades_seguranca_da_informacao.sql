...
-- Seguranca da informacao (curso_conteudos.id = 40).
--
...
-- exclusoes. 13 questoes classificadas: 9 rows REAL (Fundatec) + 4
-- AUTORAL (Q11, Q15, Q32, Q294). Q11 e Q15 chegam com a explicacao ja
-- saneada em operacao separada (commit cab1779).

-- 1) A unidade pedagogica do conteudo 40. Esperado: 1 linha, ordem 1,
...
-- 4) QIDs vinculados exatos. Esperado: 11,15,32,294,639,640,641,642,706,
--    707,708,709,710.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 40
  );
...
-- 8) Confirma banca de todas as 13 candidatas (9 REAL + 4 AUTORAL) e
--    vinculo neste conteudo. Esperado: 13 linhas, todas vinculo=1.
select q.id, q.banca, q.ano,
  (select count(*) from public.questao_unidades_pedagogicas qup2 join public.unidades_pedagogicas u2 on u2.id=qup2.unidade_pedagogica_id where qup2.questao_id=q.id and u2.curso_conteudo_id=40) as vinculo_neste_conteudo
from public.questoes q
where q.ativa = true and q.materia_id = 9 and q.assunto_id = 10
order by q.id;

-- 9) Q11 e Q15: confirmam vinculo E explicacao saneada preservada.
--    Esperado: 1 vinculo cada, explicacao mencionando phishing/2FA e
--    NOTA DE SANEAMENTO.
select id,
  ...
  case when id = 11 then position('PHISHING' in explicacao) > 0
       else position('AUTENTICAÇÃO EM DOIS FATORES' in explicacao) > 0 end as explicacao_coerente
from public.questoes q where id in (11, 15) order by id;

-- 10) Confirma artigos_esperados NULL na unidade e estado geral do
--     sistema.
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 40) as artigos_esperados_null,
