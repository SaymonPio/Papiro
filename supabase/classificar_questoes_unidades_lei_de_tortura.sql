...
-- supabase/mapa_classificacao_lei_de_tortura.sql,
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_declaracao_universal_dos_direitos_humanos.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_lei_de_tortura.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (59, '392fd9fe-a3d2-4062-b912-0a299b414429'::uuid, 1, 'alta'),
  (138, '392fd9fe-a3d2-4062-b912-0a299b414429', 1, 'alta'),
  (293, '392fd9fe-a3d2-4062-b912-0a299b414429', 1, 'alta'),
  (817, '392fd9fe-a3d2-4062-b912-0a299b414429', 1, 'alta'),
  (818, '392fd9fe-a3d2-4062-b912-0a299b414429', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 73 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 102 then
    raise exception 'Precondicao falhou: conteudo 73 materia_id=% assunto_id=% (esperado 11/102)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 73) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 73 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '392fd9fe-a3d2-4062-b912-0a299b414429' and curso_conteudo_id = 73 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_lei_de_tortura.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 73
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 102);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=102', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 73);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 73', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 73) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 73 != 1';
  end if;
