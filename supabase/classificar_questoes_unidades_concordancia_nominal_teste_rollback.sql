...
-- questoes de Concordância nominal (curso_conteudos.id = 19)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/concordancia_nominal.unidades.json + config/concordancia_nominal.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_concordancia_nominal.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_concordancia_nominal.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (223, '9a4936e1-a9a6-452c-9385-d5a5899ae5c5', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_19_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 52,
    format('materia_id=%s assunto_id=%s (esperado 6/52)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '9a4936e1-a9a6-452c-9385-d5a5899ae5c5' and curso_conteudo_id = 19 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 19 — depende de curadoria_unidades_concordancia_nominal.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 19) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 19 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 19)));
  else
  ...
  insert into _relatorio values (
    'total_candidatas_3',
    v_total_candidatas = 3,
    format('total_candidatas=%s (esperado 3)', v_total_candidatas)
  );
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=52', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_1_distintas',
    v_distintas = 1,
    format('mapa cobre %s questoes distintas (esperado 1)', v_distintas)
  );

  select exists (select 1 from _mapa where questao_id in (222,224)) into v_contem_excluida;
  insert into _relatorio values (
    'mapa_nao_contem_excluidas',
    not coalesce(v_contem_excluida, false),
    'as questoes excluidas intencionalmente (222, 224) nao devem constar no mapa'
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 3 ativas', v_fora_do_candidato)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 19', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 19;
  insert into _relatorio values ('total_vinculos_1', v_total_vinculos = 1, format('total_vinculos=%s (esperado 1)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 19;
  insert into _relatorio values ('questoes_classificadas_1', v_questoes_classificadas = 1, format('questoes_classificadas=%s (esperado 1)', v_questoes_classificadas));
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_1',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 1,
    'total de vinculos deve crescer exatamente 1 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_concordancia_nominal) ===';
