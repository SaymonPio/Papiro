-- IMPORTACAO CV-AUT-01 — 6 questoes autorais para a unidade
-- "Concordancia verbal" (curso_conteudo_id=18), curso Brigada
-- Militar RS. Fecha o deficit desta unidade (4 -> 10 uteis).
--
-- Human sign-off explicito (Fase 2D.3 -> 2E.1): Q3, Q4 (com microajuste
-- editorial de cesao — enunciado reescrito para uma unica frase coesa,
-- sem alterar regra gramatical/gabarito/alternativas/explicacao), Q8, Q6,
-- Q7-substituta (Fase 2D.1, substitui a Q7 original REJECTED_PRE_AUDIT,
-- que permanece fora), Q2 (piloto Fase 2B). Reservas Q1/Q5/Q9 NAO importadas.
--
-- Padrao identico aos lotes AUT anteriores (INFO-AUT-01, LEG-AUT-HIERARQUIA-01):
-- staging de questoes/explicacoes/alternativas, insercao por INSERT direto em
...
--
-- NAO altera nenhuma das 4 questoes existentes na unidade (2 REAL + 2 AUTORAL).
-- NAO altera unidades/conteudos/materias/aulas.
--
-- ROLLBACK-TESTADO em importar_cv_aut_01_teste_rollback.sql
-- REVERSAO REAL disponivel em reverter_cv_aut_01.sql
...
begin
  ...
  if v_uteis <> 4 then raise exception 'PRECOND: uteis atuais=% esperado 4', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='834a820d-48a7-440f-a013-be375be8a62d' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 2 then raise exception 'PRECOND: REAL atual=% esperado 2', v_real; end if;
  if v_autoral <> 2 then raise exception 'PRECOND: AUTORAL atual=% esperado 2', v_autoral; end if;
  ...
  if v_gap <> 0 then raise exception 'PRECOND: gap curso_questoes na unidade=% esperado 0', v_gap; end if;
...
Quais afirmativas estão corretas?$DUP5$),
  ...
  raise notice 'PRECONDICOES OK: unidade/conteudo/materia corretos, 4 uteis (2 REAL + 2 AUTORAL), gap curso_questoes=0, 0 duplicidade de enunciado';
end $$;
...
begin
  ...
  where qup.unidade_pedagogica_id='834a820d-48a7-440f-a013-be375be8a62d' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 2 then raise exception 'POSCOND: REAL=% esperado 2', v_real; end if;
  if v_autoral <> 8 then raise exception 'POSCOND: AUTORAL=% esperado 8', v_autoral; end if;
  ...
  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos
  from _mapa_ids m;
  if v_gabaritos <> 'CBAACB' then raise exception 'POSCOND: gabaritos=% esperado CBAACB', v_gabaritos; end if;

  select string_agg((select dificuldade from public.questoes where id=m.questao_id), ',' order by m.ordem)
    into v_dificuldades
  from _mapa_ids m;
  if v_dificuldades <> 'media,media,facil,media,media,media' then raise exception 'POSCOND: dificuldades=% esperado media,media,facil,media,media,media', v_dificuldades; end if;
  ...
  if v_vinc_novos <> 6 then raise exception 'POSCOND: %/6 vinculos novos confirmados', v_vinc_novos; end if;

  select count(*) into v_vinc_novos from _mapa_ids m
    where not exists(select 1 from public.questao_unidades_pedagogicas qup where qup.questao_id=m.questao_id and qup.unidade_pedagogica_id <> '834a820d-48a7-440f-a013-be375be8a62d');
  if v_vinc_novos <> 6 then raise exception 'POSCOND: alguma das 6 tem vinculo em unidade errada'; end if;
  ...
  if v_cq_novos <> 6 then raise exception 'POSCOND: %/6 curso_questoes novas confirmadas', v_cq_novos; end if;

  raise notice 'POSCONDICOES OK: +6 questoes, +30 alternativas, +6 vinculos, +6 curso_questoes; unidade agora com 10 uteis (2 REAL + 8 AUTORAL); gabaritos CBAACB confirmados; dificuldades confirmadas';
end $$;
