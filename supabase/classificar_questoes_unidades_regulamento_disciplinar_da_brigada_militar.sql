...
-- supabase/mapa_classificacao_regulamento_disciplinar_da_brigada_militar.sql,
...
-- mesmo padrão de supabase/classificar_questoes_unidades_direito_administrativo.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_regulamento_disciplinar_da_brigada_militar.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (210, '454f8501-7818-4dc4-b22a-337247678c58'::uuid, 1, 'alta'),
  (211, '454f8501-7818-4dc4-b22a-337247678c58', 1, 'media'),
  (212, '454f8501-7818-4dc4-b22a-337247678c58', 1, 'alta'),
  (368, '454f8501-7818-4dc4-b22a-337247678c58', 1, 'alta'),
  (369, '454f8501-7818-4dc4-b22a-337247678c58', 1, 'alta'),
  (370, '454f8501-7818-4dc4-b22a-337247678c58', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 64
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 64 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 73 then
    raise exception 'Precondicao falhou: conteudo 64 materia_id=% assunto_id=% (esperado 10/73)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 64) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 64 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '454f8501-7818-4dc4-b22a-337247678c58' and curso_conteudo_id = 64 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_regulamento_disciplinar_da_brigada_militar.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 64
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
    and q.materia_id = cm.materia_id
    and (cc.assunto_id is null or q.assunto_id = cc.assunto_id);

  if v_total_candidatas <> 6 then
    raise exception 'Precondicao falhou: total de candidatas ativas = % (esperado 6)', v_total_candidatas;
  end if;

  select count(*) into v_classificacoes_previas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 64
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 73);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=73', v_invalidas;
  end if;

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  if v_distintas <> 6 then
    raise exception 'Mapa invalido: cobre % questoes distintas (esperado 6)', v_distintas;
  end if;
  ...
  if v_fora_do_candidato <> 0 then
    raise exception 'Mapa invalido: % linha(s) fora do conjunto candidato de 6 ativas', v_fora_do_candidato;
  end if;

  select count(*) into v_unidade_fora
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 64);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 64', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 64;
  if v_total_vinculos <> 6 then
    raise exception 'Pos-condicao falhou: total_vinculos=% (esperado 6)', v_total_vinculos;
  end if;
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 64;
  if v_questoes_classificadas <> 6 then
    raise exception 'Pos-condicao falhou: questoes_classificadas=% (esperado 6)', v_questoes_classificadas;
  end if;
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 64) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 64 != 1';
  end if;
  ...
  if (select count(*) from public.questao_unidades_pedagogicas) <> (select total_vinculos from _snapshot_antes) + 6 then
    raise exception 'Pos-condicao falhou: total de vinculos nao cresceu exatamente 6';
  end if;

  raise notice 'Pos-condicoes OK: 6 questoes / 6 vinculos / 0 multiunidade.';
end $$;
