...
-- supabase/mapa_classificacao_seguranca_publica.sql, validado pelo harness
-- classificar_questoes_unidades_seguranca_publica_teste_rollback.sql (tudo_ok =
...
-- mesmo padrão de supabase/classificar_questoes_unidades_estatuto_estadual_da_igualdade_racial.sql.
--
-- NOTA: artigos_esperados desta unidade mistura 15 dispositivos da
-- Constituicao Federal (art. 144) com 1 da Constituicao do Estado do RS
-- (art. 126, caput, testado pelo item III da questao 675) — sem prefixo de
-- diploma no array por compatibilidade com o parser de
-- supabase/functions/gerar-aula/escopo.mjs.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_seguranca_publica.sql precisa já
-- ter sido aplicado.
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (19, '7cc8a187-da9b-457d-beeb-f496ddd32580'::uuid, 1, 'alta'),
  (38, '7cc8a187-da9b-457d-beeb-f496ddd32580', 1, 'alta'),
  (41, '7cc8a187-da9b-457d-beeb-f496ddd32580', 1, 'alta'),
  (675, '7cc8a187-da9b-457d-beeb-f496ddd32580', 1, 'alta'),
  (676, '7cc8a187-da9b-457d-beeb-f496ddd32580', 1, 'alta'),
  (677, '7cc8a187-da9b-457d-beeb-f496ddd32580', 1, 'alta'),
  (678, '7cc8a187-da9b-457d-beeb-f496ddd32580', 1, 'alta'),
  (743, '7cc8a187-da9b-457d-beeb-f496ddd32580', 1, 'alta'),
  (805, '7cc8a187-da9b-457d-beeb-f496ddd32580', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 49 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 17 then
    raise exception 'Precondicao falhou: conteudo 49 materia_id=% assunto_id=% (esperado 10/17)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 49) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 49 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '7cc8a187-da9b-457d-beeb-f496ddd32580' and curso_conteudo_id = 49 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_seguranca_publica.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 49
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
    and q.materia_id = cm.materia_id
    and (cc.assunto_id is null or q.assunto_id = cc.assunto_id);

  if v_total_candidatas <> 9 then
    raise exception 'Precondicao falhou: total de candidatas ativas = % (esperado 9)', v_total_candidatas;
  end if;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 17);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=17', v_invalidas;
  end if;

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  if v_distintas <> 9 then
    raise exception 'Mapa invalido: cobre % questoes distintas (esperado 9)', v_distintas;
  end if;
  ...
  if v_fora_do_candidato <> 0 then
    raise exception 'Mapa invalido: % linha(s) fora do conjunto candidato de 9 ativas', v_fora_do_candidato;
  end if;

  select count(*) into v_unidade_fora
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 49);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 49', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 49;
  if v_total_vinculos <> 9 then
    raise exception 'Pos-condicao falhou: total_vinculos=% (esperado 9)', v_total_vinculos;
  end if;
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 49;
  if v_questoes_classificadas <> 9 then
    raise exception 'Pos-condicao falhou: questoes_classificadas=% (esperado 9)', v_questoes_classificadas;
  end if;
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 49) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 49 != 1';
  end if;
  ...
  if (select count(*) from public.questao_unidades_pedagogicas) <> (select total_vinculos from _snapshot_antes) + 9 then
    raise exception 'Pos-condicao falhou: total de vinculos nao cresceu exatamente 9';
  end if;

  raise notice 'Pos-condicoes OK: 9 questoes / 9 vinculos / 0 multiunidade.';
end $$;
