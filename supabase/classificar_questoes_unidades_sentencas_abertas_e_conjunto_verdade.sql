...
-- config/sentencas_abertas_e_conjunto_verdade.mapa.json, validado pelo
-- harness
...
--
-- Oitavo conteúdo do bloco final de Raciocínio Lógico/Matemática, após
-- Tautologia, contradição e contingência (ordem 90). artigos_esperados
...
-- Q289/Q290). Nenhum saneamento prévio foi necessário nesta ordem —
-- auditoria taxonômica confirmou que as 3 candidatas pertencem
-- genuinamente a este conteúdo, sem sobreposição com Proposições e
-- conectivos, Quantificadores ou Matemática; microauditoria externa
-- não encontrou nenhum candidato externo.
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_sentencas_abertas_e_conjunto_verdade.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- 4ed265ff-578a-4462-bce6-d756b8ad5838 — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (78, '4ed265ff-578a-4462-bce6-d756b8ad5838'::uuid, 1, 'alta'),
  (289, '4ed265ff-578a-4462-bce6-d756b8ad5838'::uuid, 1, 'alta'),
  (290, '4ed265ff-578a-4462-bce6-d756b8ad5838'::uuid, 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 9 nao existe';
  end if;
  if v_materia_id is distinct from 18 or v_assunto_id is distinct from 32 then
    raise exception 'Precondicao falhou: conteudo 9 materia_id=% assunto_id=% (esperado 18/32)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '4ed265ff-578a-4462-bce6-d756b8ad5838' and curso_conteudo_id = 9 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_sentencas_abertas_e_conjunto_verdade.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 9) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 9 — decisao aprovada foi manter unidade unica';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=18/assunto_id=32', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 9', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 9) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 9 != 1';
  end if;
  ...
  where id = '4ed265ff-578a-4462-bce6-d756b8ad5838';
  ...
  raise notice 'Pos-condicoes OK: 3 questoes classificadas (Q78/Q289/Q290) / 3 vinculos / 0 multiunidade / artigos_esperados NULL.';
end $$;
