-- Aplicação REAL do mapa aprovado em config/quantificadores.mapa.json,
...
--
-- Sexto conteúdo do bloco final de Raciocínio Lógico/Matemática, após
-- Condicional e contrapositiva (ordem 88). artigos_esperados desta
-- unidade é NULL (metodologia não jurídica). 0 exclusões — as 2
-- questões ativas LIVE foram classificadas (1 row REAL Fundatec + 1
-- AUTORAL Q288).
--
-- DIVERGÊNCIA FILA×LIVE (cronologia, não erro): a fila histórica
-- (ordem-curadoria.json, ordem 89) documenta questoes_ativas=3, retrato
-- de antes do saneamento taxonômico dedicado da Q287 (commit 4295319,
-- assunto_id 33 -> 35, reclassificada para Negação de proposições por
-- testar apenas a negação do quantificador universal). O estado LIVE
-- nesta curadoria final é 2 candidatas.
--
-- AUDITORIA TAXONÔMICA (executada e aprovada antes deste apply):
-- confirmou que Q83 e Q288 exigem genuinamente compreender/interpretar
-- quantificadores, distinto de (a) negação de sentenças quantificadas
-- (Negação de proposições, destino da Q287), e (b) validade de
-- argumentos com premissas quantificadas (Argumentação lógica —
-- Q90/Q285/Q286, examinadas e confirmadas corretamente classificadas
-- lá).
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_quantificadores.sql ja foi
-- aplicado (reaproveita unidade placeholder ja existente
-- b46a6a8e-d7bf-4c19-81f3-5f0fd3352f8c — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (83, 'b46a6a8e-d7bf-4c19-81f3-5f0fd3352f8c'::uuid, 1, 'alta'),
  (288, 'b46a6a8e-d7bf-4c19-81f3-5f0fd3352f8c'::uuid, 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 7 nao existe';
  end if;
  if v_materia_id is distinct from 18 or v_assunto_id is distinct from 33 then
    raise exception 'Precondicao falhou: conteudo 7 materia_id=% assunto_id=% (esperado 18/33)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'b46a6a8e-d7bf-4c19-81f3-5f0fd3352f8c' and curso_conteudo_id = 7 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_quantificadores.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 7) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 7 — decisao aprovada foi manter unidade unica';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 7
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if v_total_candidatas <> 2 then
    raise exception 'Precondicao falhou: total de candidatas ativas LIVE = % (esperado 2 — apos saneamento taxonomico da Q287 no commit 4295319)', v_total_candidatas;
  end if;
  ...
  -- Q287 (reclassificada para Negacao de proposicoes no commit 4295319)
  -- NAO faz parte desta ordem: precisa continuar com assunto_id=35
  -- (nao 33) e sem nenhum vinculo neste conteudo.
  select assunto_id into v_assunto_287 from public.questoes where id = 287;
  if v_assunto_287 is distinct from 35 then
    raise exception 'Precondicao falhou: Q287 assunto_id=% (esperado 35 — ja deveria estar reclassificada antes desta curadoria)', v_assunto_287;
  end if;

  select count(*) into v_vinculos_287_aqui
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 7 and qup.questao_id = 287;
  if v_vinculos_287_aqui <> 0 then
    raise exception 'Precondicao falhou: Q287 possui vinculo neste conteudo (Quantificadores), nao deveria';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 18 and q.assunto_id = 33);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=18/assunto_id=33', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 7);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 7', v_unidade_fora;
  end if;
end $$;
...
declare
  ...
  v_assunto_287 bigint;
  v_vinculos_287_conteudo7 int;
  v_vinculos_287_conteudo3 int;
...
begin
  ...
  -- Pos-condicao extra: Q287 (ja reclassificada no commit 4295319)
  -- continua fora deste conteudo, com assunto_id=35 intacto e seu 1
  -- vinculo em Negacao de proposicoes (conteudo 3) preservado — este
  -- arquivo nao a toca. A unidade de Negacao permanece com 8 vinculos.
  select assunto_id into v_assunto_287 from public.questoes where id = 287;
  if v_assunto_287 is distinct from 35 then
    raise exception 'Pos-condicao falhou: assunto_id da Q287 foi alterado indevidamente (%), esperado 35', v_assunto_287;
  end if;

  select count(*) into v_vinculos_287_conteudo7
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 7 and qup.questao_id = 287;
  if v_vinculos_287_conteudo7 <> 0 then
    raise exception 'Pos-condicao falhou: Q287 ganhou vinculo neste conteudo (Quantificadores), nao deveria';
  end if;

  select count(*) into v_vinculos_287_conteudo3
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 3 and qup.questao_id = 287;
  if v_vinculos_287_conteudo3 <> 1 then
    raise exception 'Pos-condicao falhou: Q287 deveria manter exatamente 1 vinculo em Negacao de proposicoes (conteudo 3), atual=%', v_vinculos_287_conteudo3;
  end if;

  select count(*) into v_vinculos_negacao
  from public.questao_unidades_pedagogicas
  where unidade_pedagogica_id = 'c6ccefae-14df-4760-8c1d-2822090a2a93';
  if v_vinculos_negacao <> 8 then
    raise exception 'Pos-condicao falhou: unidade de Negacao de proposicoes deveria continuar com 8 vinculos, atual=%', v_vinculos_negacao;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 7) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 7 != 1';
  end if;
  ...
  where id = 'b46a6a8e-d7bf-4c19-81f3-5f0fd3352f8c';
  ...
  raise notice 'Pos-condicoes OK: 2 questoes classificadas (Q83/Q288) / 2 vinculos / 0 multiunidade / artigos_esperados NULL / Q287 intocada (assunto_id=35, 1 vinculo em Negacao de proposicoes, unidade com 8 vinculos totais).';
end $$;
