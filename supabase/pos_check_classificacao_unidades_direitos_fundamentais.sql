-- Pos-check SOMENTE LEITURA da classificacao de questoes de Direitos
-- fundamentais (curso_conteudos.id = 76) — gerado seguindo o mesmo
...
-- SELECT) porque este ambiente local não tinha SUPABASE_URL/
-- SUPABASE_SERVICE_ROLE_KEY configurados para o próprio script rodar
-- essa parte sozinho (ver .env.curadoria / env.curadoria.example). A
-- rodar depois de classificar_questoes_unidades_direitos_fundamentais.sql
...
--
-- NOTA: Constituicao Federal de 1988, arts. 3o, 4o e 5o. Q342 (EXCETO)
-- inclui art. 4o, III em artigos_esperados: o gabarito ("assegurar a
-- autodeterminacao dos povos") nao pertence ao art. 3o, correspondendo
-- na verdade ao art. 4o, III — contraste pontual testado pela questao,
-- sem ampliar a unidade para o estudo geral do art. 4o. Q343: assertiva
-- de acesso a informacao/sigilo da fonte mapeada para art. 5o, XIV (nao
-- art. 5o, X). SOBREPOSICAO TEMATICA (nao duplicata) com
-- curso_conteudo_id 47 (Direitos e Garantias Fundamentais, materia_id
-- 10, ja concluido) — consulta 8 abaixo confirma que o conteudo 47
-- permanece intocado por esta operacao.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 76, com
...
--    ordem 1 (Direitos fundamentais) = 5
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 76
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 76 aponta para questao
...
--    325c5ca6-8165-472f-b02b-eb7b18c6f71d): curso_conteudos=93,
...
-- 8) Conteudo 47 (Direitos e Garantias Fundamentais, ja concluido)
--    permanece intocado por esta operacao — sobreposicao tematica,
--    nao duplicata, nenhuma questao movida. Esperado (baseline ao vivo
--    em 2026-08-21, antes desta operacao): unidades_conteudo_47=2,
--    vinculos_conteudo_47=22.
select
  (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 47) as unidades_conteudo_47,
  (select count(*) from public.questao_unidades_pedagogicas qup
     join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
     where u.curso_conteudo_id = 47) as vinculos_conteudo_47;
