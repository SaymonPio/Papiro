...
-- questoes de Correio eletrônico (curso_conteudos.id = 39)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/correio_eletronico.unidades.json + config/correio_eletronico.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_correio_eletronico.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_correio_eletronico.sql
...
--
-- AJUSTE LOCALIZADO (nao propagado ao gerador generico gerar-rollback.mjs):
-- ao contrario da maioria dos conteudos desta fila, este conteudo NAO
-- parte de zero classificacoes. A questao 635 ja foi saneada
-- taxonomicamente (Internet e navegadores/assunto 62 -> Correio
-- eletronico/assunto 60) e vinculada a esta mesma unidade em operacao
-- separada e ja commitada, ANTES desta curadoria. Por isso, duas
-- checagens do template padrao foram corrigidas a mao para refletir o
-- estado real (marcadas abaixo com "AJUSTE"): a checagem de "sem
-- classificacao previa" (aqui ha exatamente 1, a propria Q635 ja
-- saneada — nao e uma falha) e a checagem de crescimento de vinculos
-- (crescem exatamente 9 nesta operacao — Q95, Q99, Q100, Q626, Q698,
-- Q699, Q700, Q768, Q830 — e nao 10, pois Q635 ja contava no snapshot
-- antes). O loop de aplicacao via RPC abaixo continua cobrindo os 10
-- QIDs do mapa aprovado (inclusive Q635): a RPC e ON CONFLICT DO
-- NOTHING, entao reprocessar Q635 e um no-op seguro, sem duplicar
-- vinculo — o resultado final continua sendo exatamente 1 vinculo por
-- QID, explicitamente validado nas pos-condicoes.
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (95, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta'),
  (99, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta'),
  (100, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta'),
  (626, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta'),
  (635, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta'),
  (698, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta'),
  (699, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta'),
  (700, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta'),
  (768, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta'),
  (830, '1c682e95-da6f-43a2-8b53-6669b41a376b', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_39_materia_assunto',
    v_materia_id is not distinct from 9 and v_assunto_id is not distinct from 60,
    format('materia_id=%s assunto_id=%s (esperado 9/60)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '1c682e95-da6f-43a2-8b53-6669b41a376b' and curso_conteudo_id = 39 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 39 — depende de curadoria_unidades_correio_eletronico.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 39) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 39 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 39)));
  else
  ...
  insert into _relatorio values (
    'total_candidatas_10',
    v_total_candidatas = 10,
    format('total_candidatas=%s (esperado 10)', v_total_candidatas)
  );
  ...
  -- unidade correta — nao 0 como no template generico.
  insert into _relatorio values (
    'classificacao_previa_e_apenas_q635_ja_saneada',
    ...
    format('classificacoes_previas=%s (esperado exatamente 1: Q635, ja saneada e vinculada antes desta curadoria)', v_classificacoes_previas)
  );
end $$;
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=9/assunto_id=60', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_10_distintas',
    v_distintas = 10,
    format('mapa cobre %s questoes distintas (esperado 10)', v_distintas)
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 10 ativas', v_fora_do_candidato)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 39', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 39;
  insert into _relatorio values ('total_vinculos_10', v_total_vinculos = 10, format('total_vinculos=%s (esperado 10)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 39;
  insert into _relatorio values ('questoes_classificadas_10', v_questoes_classificadas = 10, format('questoes_classificadas=%s (esperado 10)', v_questoes_classificadas));
  ...
  -- AJUSTE (ver nota no cabecalho): crescimento esperado e 9 (Q95, Q99,
  -- Q100, Q626, Q698, Q699, Q700, Q768, Q830 — novos vinculos nesta
  -- operacao), nao 10, pois o vinculo de Q635 ja existia no snapshot
  -- antes (saneada em operacao separada).
  insert into _relatorio values ('vinculos_cresceram_exatamente_9',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 9,
    'total de vinculos deve crescer exatamente 9 em relacao ao snapshot antes (9 novos — Q635 ja contava no snapshot)');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_correio_eletronico) ===';
