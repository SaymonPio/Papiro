...
-- config/vozes_verbais.mapa.json,
...
--
-- Vigésimo terceiro conteúdo de Língua Portuguesa da fila (ordem 76).
...
-- questões REAIS neste corpus). GAP_DE_BANCO_VOZES_VERBAIS quantitativo
-- real extremo, confirmado por microauditoria direcionada prévia (Q72/
-- Q280 em Reescrita e Q283/Q284/Q332 em Transitividade Verbal e Termos
-- Integrantes, todos corretamente onde estão). Decisão taxonômica
-- definitiva sobre Q244 (aprovada pelo usuário): permanece aqui, não em
-- Reescrita — comando determina explicitamente a transformação de voz
-- exigida.
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_vozes_verbais.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- 1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9 — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (243, '1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9'::uuid, 1, 'alta'),
  (244, '1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9'::uuid, 1, 'alta'),
  (245, '1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9'::uuid, 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 32 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 51 then
    raise exception 'Precondicao falhou: conteudo 32 materia_id=% assunto_id=% (esperado 6/51)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9' and curso_conteudo_id = 32 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_vozes_verbais.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 32) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 32 — decisao aprovada foi manter unidade unica';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=51', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 32', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 32) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 32 != 1';
  end if;
  ...
  where id = '1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9';
