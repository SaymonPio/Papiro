...
--
-- Cronologia: a ordem 84 (Proposicoes e conectivos) foi concluida no
-- commit ec47531 com o estado existente naquele momento (3 vinculos:
-- Q74, Q87, Q313). A ordem 84 NAO e reaberta nem sua curadoria
-- reaplicada — permanece "concluido" na fila. Q314 e reclassificada
-- posteriormente, nesta operacao dedicada, durante a auditoria da
-- ordem 85, apos confirmacao de erro taxonomico. O estado LIVE da
-- unidade passa a ter 4 vinculos a partir deste commit.
--
-- NAO fecha a ordem 85. NAO toca Q75 nem Q89. Apos esta operacao, as
-- candidatas LIVE restantes do assunto 38 (Tabela-verdade) passam a
-- ser Q75 e Q89 (total 2) — a fila historica documentando 3 candidatas
-- foi produzida antes deste saneamento e nao deve ser tratada como
-- corrupcao, apenas como um retrato anterior a esta correcao.
--
-- Diferenca deste arquivo para o harness de teste
-- (saneamento_taxonomia_q314_teste_rollback.sql, ja executado com
...
begin
  ...
  raise notice 'Pos-condicoes OK: Q314 reclassificada para assunto_id=36 com exatamente 1 vinculo na unidade Proposicoes e conectivos, unidade destino com 4 vinculos totais (Q74/Q87/Q313/Q314), conteudo/gabarito/metadados de Q314 inalterados.';
end $$;
