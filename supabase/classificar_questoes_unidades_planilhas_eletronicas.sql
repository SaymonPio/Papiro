...
-- config/planilhas_eletronicas.mapa.json,
-- validado pelo harness
-- classificar_questoes_unidades_planilhas_eletronicas_teste_rollback.sql
-- (tudo_ok = true confirmado antes de rodar este arquivo).
--
-- Terceiro conteúdo de Informática curado nesta metodologia após Windows
-- (ordem 78) e Editor de textos (ordem 79). artigos_esperados desta
-- unidade é NULL (metodologia não jurídica). 2 exclusões intencionais:
-- Q495 (PROBLEMA_DE_FIDELIDADE_IMAGEM_Q495, confirmado por fonte primária
-- — prova original Fundatec/GM Imbé 2023, Questão 17, mesma prova de Q791
-- na ordem 79) e Q499 (DUPLICATA_DE_FONTE_REFORMULADA, canônica Q105 —
-- mesma questão-fonte da SUSEPE/Polícia Penal RS 01/2022, Questão 26,
-- confirmada por fonte primária, com alternativas idênticas mas enunciado
-- reformulado). Ambas permanecem ativas, intactas, sem vínculo, não
-- corrigidas nesta operação. As 13 questões vinculadas são 12 rows REAL
-- (Fundatec) + 1 AUTORAL (Q33).
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_planilhas_eletronicas.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- d7635b99-48c1-4599-a014-68e8eb9ca3c2 — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (33, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (62, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (93, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (97, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (105, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (109, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (340, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (492, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (493, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (498, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (637, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (638, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta'),
  (834, 'd7635b99-48c1-4599-a014-68e8eb9ca3c2'::uuid, 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 37 nao existe';
  end if;
  if v_materia_id is distinct from 9 or v_assunto_id is distinct from 29 then
    raise exception 'Precondicao falhou: conteudo 37 materia_id=% assunto_id=% (esperado 9/29)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'd7635b99-48c1-4599-a014-68e8eb9ca3c2' and curso_conteudo_id = 37 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_planilhas_eletronicas.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 37) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 37 — decisao aprovada foi manter unidade unica';
  end if;
  ...
  -- Q495 e Q499 (exclusoes intencionais) nao podem ja estar classificadas aqui.
  if exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 37 and qup.questao_id in (495, 499)
  ) then
    raise exception 'Precondicao falhou: a questao 495 (PROBLEMA_DE_FIDELIDADE_IMAGEM_Q495) ou 499 (DUPLICATA_DE_FONTE_REFORMULADA) ja possui vinculo neste conteudo — nao deveria';
  end if;
end $$;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=9/assunto_id=29', v_invalidas;
  end if;
  ...
  if coalesce(v_contem_excluida, false) then
    raise exception 'Mapa invalido: as questoes excluidas intencionalmente (495, 499) nao deveriam constar no mapa';
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 37', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  where questao_id in (495, 499);
  if v_excluidas_vinculos <> 0 then
    raise exception 'Pos-condicao falhou: questoes 495/499 (excluidas intencionalmente) ganharam vinculo (%), deveriam permanecer sem vinculo', v_excluidas_vinculos;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 37) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 37 != 1';
  end if;
  ...
  where id = 'd7635b99-48c1-4599-a014-68e8eb9ca3c2';
  ...
  raise notice 'Pos-condicoes OK: 13 questoes classificadas / 13 vinculos / 0 multiunidade / Q495+Q499 sem vinculo / artigos_esperados NULL.';
end $$;
