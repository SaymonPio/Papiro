...
-- supabase/mapa_classificacao_declaracao_universal_dos_direitos_humanos.sql,
...
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_declaracao_universal_dos_direitos_humanos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (145, 'e7bef052-b882-4a2f-b1e6-88ce12740c26'::uuid, 1, 'alta'),
  (251, 'e7bef052-b882-4a2f-b1e6-88ce12740c26', 1, 'alta'),
  (252, 'e7bef052-b882-4a2f-b1e6-88ce12740c26', 1, 'alta'),
  (349, 'e7bef052-b882-4a2f-b1e6-88ce12740c26', 1, 'alta'),
  (788, 'e7bef052-b882-4a2f-b1e6-88ce12740c26', 1, 'alta'),
  (812, 'e7bef052-b882-4a2f-b1e6-88ce12740c26', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 83
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 83 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 88 then
    raise exception 'Precondicao falhou: conteudo 83 materia_id=% assunto_id=% (esperado 11/88)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 83) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 83 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'e7bef052-b882-4a2f-b1e6-88ce12740c26' and curso_conteudo_id = 83 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_declaracao_universal_dos_direitos_humanos.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 83
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 88);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=88', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 83);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 83', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 83) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 83 != 1';
  end if;
