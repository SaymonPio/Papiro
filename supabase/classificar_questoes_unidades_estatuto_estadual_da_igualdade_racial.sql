...
-- supabase/mapa_classificacao_estatuto_estadual_da_igualdade_racial.sql, validado pelo harness
-- classificar_questoes_unidades_estatuto_estadual_da_igualdade_racial_teste_rollback.sql (tudo_ok =
...
-- mesmo padrão de supabase/classificar_questoes_unidades_abuso_de_autoridade.sql.
--
-- NOTA: Q132/Q852 e Q136/Q851 são duplicatas confirmadas (mesmo fato,
-- mesmas alternativas) — ambas classificadas por decisão explícita do
-- usuário nesta curadoria; nenhuma foi desativada ou alterada. O
-- saneamento de duplicatas fica para um fluxo separado, futuro.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_estatuto_estadual_da_igualdade_racial.sql precisa já
-- ter sido aplicado.
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (132, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e'::uuid, 1, 'alta'),
  (136, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta'),
  (301, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta'),
  (365, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta'),
  (366, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta'),
  (367, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta'),
  (798, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta'),
  (850, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta'),
  (851, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta'),
  (852, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 68
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 68 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 63 then
    raise exception 'Precondicao falhou: conteudo 68 materia_id=% assunto_id=% (esperado 10/63)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 68) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 68 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'f0f6bae6-df70-450a-b4ee-3d9747962c2e' and curso_conteudo_id = 68 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_estatuto_estadual_da_igualdade_racial.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 68
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 63);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=63', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 68);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 68', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 68) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 68 != 1';
  end if;
  ...
  if (select count(*) from public.questao_unidades_pedagogicas) <> (select total_vinculos from _snapshot_antes) + 10 then
    raise exception 'Pos-condicao falhou: total de vinculos nao cresceu exatamente 10';
  end if;

  raise notice 'Pos-condicoes OK: 10 questoes / 10 vinculos / 0 multiunidade.';
end $$;
