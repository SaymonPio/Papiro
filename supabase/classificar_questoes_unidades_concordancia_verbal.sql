...
-- config/concordancia_verbal.mapa.json,
...
--
-- Decimo primeiro conteudo de Lingua Portuguesa da fila. artigos_esperados
-- desta unidade e NULL (metodologia nao juridica). 0 exclusoes
-- intencionais — todas as 4 questoes ativas foram classificadas, apos
-- microauditoria global (sem restringir a assunto_id=14) que confirmou
-- ausencia de candidatas reais adicionais e GAP_DE_BANCO_CONCORDANCIA_VERBAL
-- como gap real quantitativo, nao problema de taxonomia/duplicacao/
-- fidelidade.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_crase.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_concordancia_verbal.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 834a820d-48a7-440f-a013-be375be8a62d — não cria unidade
-- nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (16, '834a820d-48a7-440f-a013-be375be8a62d'::uuid, 1, 'alta'),
  (116, '834a820d-48a7-440f-a013-be375be8a62d', 1, 'alta'),
  (304, '834a820d-48a7-440f-a013-be375be8a62d', 1, 'alta'),
  (318, '834a820d-48a7-440f-a013-be375be8a62d', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 18
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 18 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 14 then
    raise exception 'Precondicao falhou: conteudo 18 materia_id=% assunto_id=% (esperado 6/14)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 18) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 18 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '834a820d-48a7-440f-a013-be375be8a62d' and curso_conteudo_id = 18 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_concordancia_verbal.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 18
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 14);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=14', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 18);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 18', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 18) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 18 != 1';
  end if;
  ...
  where id = '834a820d-48a7-440f-a013-be375be8a62d';
