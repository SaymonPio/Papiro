-- Pos-check SOMENTE LEITURA da classificacao de questoes de Estatuto do Desarmamento
-- (curso_conteudos.id = 56) — gerado seguindo o mesmo template de
...
-- classificar_questoes_unidades_estatuto_do_desarmamento.sql (a versao
...
--
-- NOTA: Q142 e Q853 sao duplicatas confirmadas (mesmo fato, mesmas 5
-- alternativas na mesma ordem) — ambas classificadas por decisao explicita
-- do usuario nesta curadoria; o saneamento de duplicatas fica para um
-- fluxo separado, futuro.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 56, com
...
--    ordem 1 (Estatuto do Desarmamento) = 12
...
--    Esperado: total_vinculos = 12, questoes_distintas = 12.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 56
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 56 aponta para questao
...
--    d88a80ae-187b-4a24-a5cb-3b20ff32e26f): curso_conteudos=93,
