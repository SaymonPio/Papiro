...
-- questoes de Redação oficial (curso_conteudos.id = 30)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/redacao_oficial.unidades.json + config/redacao_oficial.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_redacao_oficial.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_redacao_oficial.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (18, '29bfb433-4013-4164-85de-fd847963199d', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_30_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 16,
    format('materia_id=%s assunto_id=%s (esperado 6/16)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '29bfb433-4013-4164-85de-fd847963199d' and curso_conteudo_id = 30 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 30 — depende de curadoria_unidades_redacao_oficial.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 30) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 30 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 30)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=16', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 30', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '29bfb433-4013-4164-85de-fd847963199d';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_redacao_oficial) ===';
