...
-- Condicional e contrapositiva (curso_conteudos.id = 6).
--
...
-- exclusoes. 2 questoes classificadas: 1 row REAL (Fundatec: Q80) + 1
-- AUTORAL (Q309). Q84 foi previamente reclassificada para Proposicoes
-- e conectivos (commit 6256b12, assunto_id 39 -> 36) — por isso a fila
-- historica (ordem-curadoria.json, ordem 88) documenta 3 candidatas
-- enquanto o LIVE nesta curadoria e 2. Isso e cronologia, nao erro.

-- 1) A unidade pedagogica do conteudo 6. Esperado: 1 linha, ordem 1,
...
-- 4) QIDs vinculados exatos. Esperado: 80, 309.
...
-- 6) Questoes ativas do conteudo (assunto_id=39) que ficaram SEM nenhuma
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 6
  );
...
select q.id, q.banca, q.ano,
  (select count(*) from public.questao_unidades_pedagogicas qup2 join public.unidades_pedagogicas u2 on u2.id=qup2.unidade_pedagogica_id where qup2.questao_id=q.id and u2.curso_conteudo_id=6) as vinculo_neste_conteudo
from public.questoes q
where q.ativa = true and q.materia_id = 18 and q.assunto_id = 39
order by q.id;

-- 9) Confirma que a Q84 (reclassificada no commit 6256b12) permanece
--    corretamente fora deste conteudo. Esperado: assunto_id=36,
--    vinculos_neste_conteudo=0, vinculos_conteudo_1=1.
select id, assunto_id,
  (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where qup.questao_id = q.id and u.curso_conteudo_id = 6) as vinculos_neste_conteudo,
...
--     sistema.
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 6) as artigos_esperados_null,
