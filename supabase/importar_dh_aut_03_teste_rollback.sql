-- TESTE DE ROLLBACK do Lote DH-AUT-03 de Direitos Humanos e Cidadania — 22
-- questoes AUTORAL_PAPIRO + 110 alternativas + 22 vinculos (cc82,83,84,85).
--
-- Mirror exato de supabase/importar_dh_aut_03.sql, substituindo os
...
insert into _relatorio select 'staging_cc82_6', (select count(*) from _lote_questoes where cc_id=82) = 6, (select count(*) from _lote_questoes where cc_id=82)::text;
insert into _relatorio select 'staging_cc83_4', (select count(*) from _lote_questoes where cc_id=83) = 4, (select count(*) from _lote_questoes where cc_id=83)::text;
insert into _relatorio select 'staging_cc84_7', (select count(*) from _lote_questoes where cc_id=84) = 7, (select count(*) from _lote_questoes where cc_id=84)::text;
insert into _relatorio select 'staging_cc85_5', (select count(*) from _lote_questoes where cc_id=85) = 5, (select count(*) from _lote_questoes where cc_id=85)::text;

insert into _relatorio select 'snapshot_cc82_uteis_4', (select cc82_uteis from _snapshot_antes) = 4, (select cc82_uteis from _snapshot_antes)::text;
insert into _relatorio select 'snapshot_cc83_uteis_6', (select cc83_uteis from _snapshot_antes) = 6, (select cc83_uteis from _snapshot_antes)::text;
insert into _relatorio select 'snapshot_cc84_uteis_3', (select cc84_uteis from _snapshot_antes) = 3, (select cc84_uteis from _snapshot_antes)::text;
insert into _relatorio select 'snapshot_cc85_uteis_5', (select cc85_uteis from _snapshot_antes) = 5, (select cc85_uteis from _snapshot_antes)::text;
...
insert into _relatorio select 'pos_dificuldade_5_11_6',
...
insert into _relatorio select 'pos_cc82_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=82) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=82)::text;
insert into _relatorio select 'pos_cc83_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=83) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=83)::text;
insert into _relatorio select 'pos_cc84_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=84) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=84)::text;
insert into _relatorio select 'pos_cc85_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=85) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=85)::text;

insert into _relatorio select 'pos_real_inalterado',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=82 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc82_real from _snapshot_antes)
  and (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=83 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc83_real from _snapshot_antes)
  and (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=84 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc84_real from _snapshot_antes)
  and (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=85 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc85_real from _snapshot_antes),
  'ok';
