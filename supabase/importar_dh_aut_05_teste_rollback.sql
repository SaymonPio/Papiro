-- TESTE DE ROLLBACK do Lote DH-AUT-05 de Direitos Humanos e Cidadania — 21
-- questoes AUTORAL_PAPIRO + 105 alternativas + 21 vinculos (cc89,90,91).
--
-- Mirror exato de supabase/importar_dh_aut_05.sql, substituindo os
...
-- Precondicoes.
insert into _relatorio select 'staging_total_21', (select count(*) from _lote_questoes) = 21, (select count(*) from _lote_questoes)::text;
...
insert into _relatorio select 'staging_cc89_7', (select count(*) from _lote_questoes where cc_id=89) = 7, (select count(*) from _lote_questoes where cc_id=89)::text;
insert into _relatorio select 'staging_cc90_7', (select count(*) from _lote_questoes where cc_id=90) = 7, (select count(*) from _lote_questoes where cc_id=90)::text;
insert into _relatorio select 'staging_cc91_7', (select count(*) from _lote_questoes where cc_id=91) = 7, (select count(*) from _lote_questoes where cc_id=91)::text;

insert into _relatorio select 'snapshot_cc89_uteis_3', (select cc89_uteis from _snapshot_antes) = 3, (select cc89_uteis from _snapshot_antes)::text;
insert into _relatorio select 'snapshot_cc90_uteis_3', (select cc90_uteis from _snapshot_antes) = 3, (select cc90_uteis from _snapshot_antes)::text;
insert into _relatorio select 'snapshot_cc91_uteis_3', (select cc91_uteis from _snapshot_antes) = 3, (select cc91_uteis from _snapshot_antes)::text;

insert into _relatorio select 'snapshot_q147_hash_ok', (select q147_hash from _snapshot_antes) = 'b0c2c84e52e2ca7bdcbed957467fda6e', (select q147_hash from _snapshot_antes);
insert into _relatorio select 'snapshot_q148_hash_ok', (select q148_hash from _snapshot_antes) = '7ac4306a25cc846e97c80a4215a553fa', (select q148_hash from _snapshot_antes);
insert into _relatorio select 'snapshot_q149_hash_ok', (select q149_hash from _snapshot_antes) = 'd8dd965a249edbdd3f74e72919ed1fba', (select q149_hash from _snapshot_antes);
...
insert into _relatorio select 'pos_questoes_novas_21',
  (select count(*) from public.questoes where id in (select questao_id from _mapa_ids)) = 21,
...
insert into _relatorio select 'pos_alternativas_105',
  (select count(*) from public.alternativas where questao_id in (select questao_id from _mapa_ids)) = 105,
...
insert into _relatorio select 'pos_dificuldade_5_10_6',
  (select count(*) from public.questoes where id in (select questao_id from _mapa_ids) and dificuldade='facil') = 5
  and (select count(*) from public.questoes where id in (select questao_id from _mapa_ids) and dificuldade='media') = 10
  and (select count(*) from public.questoes where id in (select questao_id from _mapa_ids) and dificuldade='dificil') = 6,
...
insert into _relatorio select 'pos_gabarito_A5_B4_C4_D4_E4',
...
insert into _relatorio select 'pos_vinculos_novos_21',
  (select count(*) from public.questao_unidades_pedagogicas where questao_id in (select questao_id from _mapa_ids)) = 21,
...
insert into _relatorio select 'pos_cc89_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=89) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=89)::text;
insert into _relatorio select 'pos_cc90_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=90) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=90)::text;
insert into _relatorio select 'pos_cc91_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=91) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=91)::text;

insert into _relatorio select 'pos_real_inalterado',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=89 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc89_real from _snapshot_antes)
  and (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=90 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc90_real from _snapshot_antes)
  and (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=91 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc91_real from _snapshot_antes),
  'ok';

insert into _relatorio select 'pos_dh_uteis_delta_21',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id = qup.unidade_pedagogica_id join public.curso_conteudos cc on cc.id = up.curso_conteudo_id join public.assuntos a on a.id = cc.assunto_id where a.materia_id = 11) = (select dh_uteis from _snapshot_antes) + 21,
...
insert into _relatorio select 'pos_q147_hash_inalterado', (select md5(explicacao) from public.questoes where id = 147) = (select q147_hash from _snapshot_antes), (select md5(explicacao) from public.questoes where id = 147);
insert into _relatorio select 'pos_q148_hash_inalterado', (select md5(explicacao) from public.questoes where id = 148) = (select q148_hash from _snapshot_antes), (select md5(explicacao) from public.questoes where id = 148);
insert into _relatorio select 'pos_q149_hash_inalterado', (select md5(explicacao) from public.questoes where id = 149) = (select q149_hash from _snapshot_antes), (select md5(explicacao) from public.questoes where id = 149);

insert into _relatorio select 'pos_total_questoes_delta_21', (select count(*) from public.questoes) = (select total_questoes from _snapshot_antes) + 21, (select count(*) from public.questoes)::text;
insert into _relatorio select 'pos_total_alternativas_delta_105', (select count(*) from public.alternativas) = (select total_alternativas from _snapshot_antes) + 105, (select count(*) from public.alternativas)::text;
insert into _relatorio select 'pos_total_vinculos_delta_21', (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 21, (select count(*) from public.questao_unidades_pedagogicas)::text;
