-- TESTE DE ROLLBACK — AJUSTE CIRURGICO DO RESUMO VISUAL (fluxo completo
-- OLD -> TARGET -> REVERT -> OLD_FINAL -> ROLLBACK; NAO aplica nada
-- permanente; ver ajustar_resumo_visual_dgf_u1_xv_xix.sql para o apply
-- real) — unidade "Direitos Individuais e Coletivos Fundamentais"
-- (BM RS) — atualiza o componente `resumo_visual`
...
begin
  ...
  from public.aula_versoes av
  join _snapshot_old_resumo_dgf t on t.aula_versao_id = av.id
  where av.id = '52756262-8be4-4fb2-a9f3-2f5a158ea1d4'::uuid;
  ...
  raise notice 'OLD_FINAL OK: estrutura e status identicos ao estado OLD original (resumo_visual com 7 pontos, sem Locomocao/XIX) — reversao comprovada';
end $$;
