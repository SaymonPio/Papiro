...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_crase.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_crase.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (6, '57bf73b4-ca89-4809-bc8d-7bf1ae7fa4c2', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_15_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 5,
    format('materia_id=%s assunto_id=%s (esperado 6/5)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '57bf73b4-ca89-4809-bc8d-7bf1ae7fa4c2' and curso_conteudo_id = 15 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 15 — depende de curadoria_unidades_crase.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 15) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 15 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 15)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=5', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 15', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '57bf73b4-ca89-4809-bc8d-7bf1ae7fa4c2';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_crase) ===';
