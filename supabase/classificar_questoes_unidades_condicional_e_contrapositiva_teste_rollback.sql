...
-- questoes de Condicional e contrapositiva (curso_conteudos.id = 6)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/condicional_e_contrapositiva.unidades.json + config/condicional_e_contrapositiva.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_condicional_e_contrapositiva.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_condicional_e_contrapositiva.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (80, '42f5f55c-350a-4fb6-904c-184cde415d1e', 1, 'alta'),
  (309, '42f5f55c-350a-4fb6-904c-184cde415d1e', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_6_materia_assunto',
    v_materia_id is not distinct from 18 and v_assunto_id is not distinct from 39,
    format('materia_id=%s assunto_id=%s (esperado 18/39)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '42f5f55c-350a-4fb6-904c-184cde415d1e' and curso_conteudo_id = 6 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 6 — depende de curadoria_unidades_condicional_e_contrapositiva.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 6) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 6 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 6)));
  else
  ...
  insert into _relatorio values (
    'total_candidatas_2',
    v_total_candidatas = 2,
    format('total_candidatas=%s (esperado 2)', v_total_candidatas)
  );
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=18/assunto_id=39', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_2_distintas',
    v_distintas = 2,
    format('mapa cobre %s questoes distintas (esperado 2)', v_distintas)
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 2 ativas', v_fora_do_candidato)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 6', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 6;
  insert into _relatorio values ('total_vinculos_2', v_total_vinculos = 2, format('total_vinculos=%s (esperado 2)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 6;
  insert into _relatorio values ('questoes_classificadas_2', v_questoes_classificadas = 2, format('questoes_classificadas=%s (esperado 2)', v_questoes_classificadas));
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '42f5f55c-350a-4fb6-904c-184cde415d1e';
  insert into _relatorio values ('u1_2_questoes', v_qtd_tmp = 2, format('u1_questoes=%s (esperado 2)', v_qtd_tmp));
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_2',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 2,
    'total de vinculos deve crescer exatamente 2 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_condicional_e_contrapositiva) ===';
