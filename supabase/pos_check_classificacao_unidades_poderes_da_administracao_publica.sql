-- Pos-check SOMENTE LEITURA da classificacao de questoes de Poderes da
-- Administracao Publica (curso_conteudos.id = 59) — gerado seguindo o
-- mesmo template de scripts/curadoria-pedagogica/gerar-pos-check.mjs. Os
...
--
-- NOTA: conteudo predominantemente doutrinario. Q207 (poder hierarquico) e
-- Q208 (poder disciplinar) permanecem sem dispositivo legal atribuido.
-- artigos_esperados contem apenas CF art. 84, IV (Q209, poder regulamentar)
-- e Lei no 4.717/1965, art. 2o, paragrafo unico, "a" e "e" (Q360, abuso de
-- poder) — o texto legal da alinea "a" usa literalmente "incompetencia";
-- a associacao com "excesso de poder" e doutrinaria, nao do texto legal.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 59, com
...
--    ordem 1 (Poderes da Administração Pública) = 4
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 59
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 59 aponta para questao
...
--    8cb82a8e-e0f4-4d46-a4a5-7443f31912a4): curso_conteudos=93,
