...
-- supabase/mapa_classificacao_dignidade_humana.sql,
...
--
-- ATENCAO: sobreposicao tematica documentada (nao duplicata),
-- CROSS-MATERIA, com o conteudo ja concluido 57 (Constituicao Federal
-- de 1988, materia Legislacao Especifica) — Q794 e Q651 la testam o
-- mesmo art. 1o da CF (Q794 mesma tese de Q648/Q715 aqui; Q651 com
-- comando EXCETO). O conteudo 57 nao e tocado por este arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_tratado_de_marraqueche.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_dignidade_humana.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 70bd2ed8-e947-4050-b1aa-e1ceb2c0be1f — não cria unidade nova).
begin;
...
select
  (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 57) as unidades_57,
  (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 57) as vinculos_57;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (13, '70bd2ed8-e947-4050-b1aa-e1ceb2c0be1f'::uuid, 1, 'alta'),
  (648, '70bd2ed8-e947-4050-b1aa-e1ceb2c0be1f', 1, 'alta'),
  (715, '70bd2ed8-e947-4050-b1aa-e1ceb2c0be1f', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 98
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 98 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 12 then
    raise exception 'Precondicao falhou: conteudo 98 materia_id=% assunto_id=% (esperado 11/12)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 98) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 98 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '70bd2ed8-e947-4050-b1aa-e1ceb2c0be1f' and curso_conteudo_id = 98 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_dignidade_humana.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 98
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 57) then
    raise exception 'Precondicao falhou: curso_conteudos 57 (sobreposicao tematica Constituicao Federal de 1988) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 12);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=12', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 98);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 98', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 98) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 98 != 1';
  end if;
  ...
  -- Pos-condicao extra: artigos_esperados deve ser exatamente ['art. 1º, III'].
  select artigos_esperados into v_artigos_esperados
  from public.unidades_pedagogicas
  where id = '70bd2ed8-e947-4050-b1aa-e1ceb2c0be1f';
  if v_artigos_esperados is distinct from array['art. 1º, III']::text[] then
    raise exception 'Pos-condicao falhou: artigos_esperados=% (esperado [art. 1º, III])', v_artigos_esperados;
  end if;

  -- Pos-condicao extra: conteudo 57 (sobreposicao tematica cross-materia) permanece intocado.
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 57) <> (select unidades_57 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 57 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 57) <> (select vinculos_57 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 57 mudou — nao deveria ser tocado';
  end if;

  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / artigos_esperados = [art. 1º, III] / conteudo 57 intocado.';
end $$;
