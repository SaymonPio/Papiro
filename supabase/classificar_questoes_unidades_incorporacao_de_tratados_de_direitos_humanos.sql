...
-- supabase/mapa_classificacao_incorporacao_de_tratados_de_direitos_humanos.sql,
...
-- Emenda Constitucional) — Q165 deste conteudo cobre substancialmente
-- a mesma regra do art. 5o, §3o ja usada para Q265 do conteudo 78, mas
-- com enunciados diferentes e nenhuma duplicata exata de ID. O
-- conteudo 78 nao e tocado por este arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_incorporacao_de_tratados_de_direitos_humanos.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 98b10517-14a2-4efe-8360-960cae263ad5 — não cria unidade nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (165, '98b10517-14a2-4efe-8360-960cae263ad5'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 93 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 101 then
    raise exception 'Precondicao falhou: conteudo 93 materia_id=% assunto_id=% (esperado 11/101)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 93) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 93 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '98b10517-14a2-4efe-8360-960cae263ad5' and curso_conteudo_id = 93 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_incorporacao_de_tratados_de_direitos_humanos.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=101', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 93', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 93) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 93 != 1';
  end if;
  ...
  -- ['art. 5º, §3º', 'art. 84, VIII', 'art. 49, I'].
  select artigos_esperados into v_artigos_esperados
  from public.unidades_pedagogicas
  where id = '98b10517-14a2-4efe-8360-960cae263ad5';
  if v_artigos_esperados is distinct from array['art. 5º, §3º','art. 84, VIII','art. 49, I']::text[] then
    raise exception 'Pos-condicao falhou: artigos_esperados=% (esperado [art. 5º, §3º / art. 84, VIII / art. 49, I])', v_artigos_esperados;
  end if;
  ...
  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / artigos_esperados = [art. 5º §3º / art. 84 VIII / art. 49 I] / conteudo 78 intocado.';
end $$;
