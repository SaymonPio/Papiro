...
-- supabase/mapa_classificacao_comissao_interamericana_de_direitos_humanos.sql,
...
--
-- ATENCAO: sobreposicao tematica documentada (nao duplicata) com os
-- conteudos ja concluidos 72 (Pacto de San Jose), 85 (Sistema
-- Interamericano), 86 (OEA) e 88 (Corte Interamericana). Nenhum deles
-- e tocado por este arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_organizacao_dos_estados_americanos.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_comissao_interamericana_de_direitos_humanos.sql
...
select
  (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 72) as unidades_72,
  (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 72) as vinculos_72,
  ...
  (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 86) as unidades_86,
  (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 86) as vinculos_86,
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (150, '1b84fd2f-93e4-46c1-868f-c8402e73bdf9'::uuid, 1, 'alta'),
  (151, '1b84fd2f-93e4-46c1-868f-c8402e73bdf9', 1, 'alta'),
  (152, '1b84fd2f-93e4-46c1-868f-c8402e73bdf9', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 87
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 87 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 92 then
    raise exception 'Precondicao falhou: conteudo 87 materia_id=% assunto_id=% (esperado 11/92)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 87) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 87 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '1b84fd2f-93e4-46c1-868f-c8402e73bdf9' and curso_conteudo_id = 87 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_comissao_interamericana_de_direitos_humanos.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 87
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 72) then
    raise exception 'Precondicao falhou: curso_conteudos 72 (sobreposicao Pacto San Jose) nao existe mais — verificar integridade';
  end if;
  if not exists (select 1 from public.curso_conteudos where id = 85) then
    raise exception 'Precondicao falhou: curso_conteudos 85 (sobreposicao Sistema Interamericano) nao existe mais — verificar integridade';
  end if;
  if not exists (select 1 from public.curso_conteudos where id = 86) then
    raise exception 'Precondicao falhou: curso_conteudos 86 (sobreposicao OEA) nao existe mais — verificar integridade';
  end if;
  if not exists (select 1 from public.curso_conteudos where id = 88) then
    raise exception 'Precondicao falhou: curso_conteudos 88 (sobreposicao Corte Interamericana) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 92);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=92', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 87);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 87', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 87) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 87 != 1';
  end if;
  ...
  -- Pos-condicao extra: conteudos sobrepostos (72, 85, 86, 88) permanecem intocados.
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 72) <> (select unidades_72 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 72 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 72) <> (select vinculos_72 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 72 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 85) <> (select unidades_85 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 85 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 85) <> (select vinculos_85 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 85 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 86) <> (select unidades_86 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 86 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 86) <> (select vinculos_86 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 86 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 88) <> (select unidades_88 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 88 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 88) <> (select vinculos_88 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 88 mudou — nao deveria ser tocado';
  end if;

  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / conteudos 72, 85, 86 e 88 intocados.';
end $$;
