...
-- supabase/mapa_classificacao_improbidade.sql, validado pelo harness
-- classificar_questoes_unidades_improbidade_teste_rollback.sql (tudo_ok =
-- true esperado antes de rodar este arquivo).
--
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (42,  '9d4c7a1e-3f68-4b52-8a91-6c0d5e2f7b84'::uuid, 2, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 55 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 64 then
    raise exception 'Precondicao falhou: conteudo 55 materia_id=% assunto_id=% (esperado 10/64)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 55 and ativa = true) <> 2 then
    raise exception 'Precondicao falhou: nao ha exatamente 2 unidades pedagogicas ativas para o conteudo 55';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '60927a85-1b4a-480a-b8da-8eb318520692' and curso_conteudo_id = 55 and ordem = 1 and ativa)
  or not exists (select 1 from public.unidades_pedagogicas where id = '9d4c7a1e-3f68-4b52-8a91-6c0d5e2f7b84' and curso_conteudo_id = 55 and ordem = 2 and ativa)
  then
    raise exception 'Precondicao falhou: uma ou ambas as 2 unidades oficiais nao conferem (id/ordem/conteudo/ativa) — curadoria_unidades_improbidade.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=64', v_invalidas;
  end if;
  ...
  if v_fora_do_candidato <> 0 then
    raise exception 'Mapa invalido: % linha(s) fora do conjunto candidato de 15', v_fora_do_candidato;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 55', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 55;
  if v_total_vinculos <> 16 then
    raise exception 'Pos-condicao falhou: total_vinculos=% (esperado 16)', v_total_vinculos;
  end if;
  ...
  if v_multiunidade is distinct from array[732]::bigint[] then
    raise exception 'Pos-condicao falhou: multiunidade=% (esperado {732})', v_multiunidade;
  end if;
  ...
  if v_u1_qtd <> 9 then
    raise exception 'Pos-condicao falhou: u1_questoes=% (esperado 9)', v_u1_qtd;
  end if;
  ...
  if v_u2_qtd <> 7 then
    raise exception 'Pos-condicao falhou: u2_questoes=% (esperado 7)', v_u2_qtd;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 55 and ativa = true) <> 2 then
    raise exception 'Pos-condicao falhou: unidades ativas do conteudo 55 != 2';
  end if;
  ...
  if (select count(*) from public.questao_unidades_pedagogicas) <> (select total_vinculos from _snapshot_antes) + 16 then
    raise exception 'Pos-condicao falhou: total de vinculos nao cresceu exatamente 16';
  end if;

  raise notice 'Pos-condicoes OK: 15 questoes / 16 vinculos / 1 multiunidade (732) aplicados com sucesso.';
end $$;
