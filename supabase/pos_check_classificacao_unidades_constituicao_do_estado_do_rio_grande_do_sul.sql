-- Pos-check SOMENTE LEITURA da classificacao de questoes de Constituição do Estado do Rio Grande do Sul
-- (curso_conteudos.id = 50) — gerado seguindo o mesmo template de
...
-- classificar_questoes_unidades_constituicao_do_estado_do_rio_grande_do_sul.sql (a versao
...
-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 50, com
...
--    ordem 1 (Constituição do Estado do Rio Grande do Sul) = 14
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 50
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 50 aponta para questao
...
--    83636594-c69f-4de0-bf46-0e75c2ec981c): curso_conteudos=93,
