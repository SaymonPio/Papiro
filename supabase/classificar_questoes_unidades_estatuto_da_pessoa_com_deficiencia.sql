...
-- supabase/mapa_classificacao_estatuto_da_pessoa_com_deficiencia.sql,
...
--
-- ATENCAO: este arquivo cobre exclusivamente o curso_conteudo_id 75
-- ("Estatuto da Pessoa com Deficiência", dispositivos SUBSTANTIVOS
-- proprios da Lei 13.146/2015). NAO confundir com curso_conteudo_id 71
-- ("Pessoa com deficiência", ja concluido, que cita a Lei 13.146/2015
-- apenas como lei alteradora da Lei 7.853/1989) — nenhuma sobreposicao
-- de artigo entre os dois, conteudo 71 nao e tocado por este arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_pacto_internacional_sobre_direitos_civis_e_politicos.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_estatuto_da_pessoa_com_deficiencia.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (126, '128d9183-6188-4ca1-abdb-fc5c57e78d28'::uuid, 1, 'alta'),
  (257, '128d9183-6188-4ca1-abdb-fc5c57e78d28', 1, 'alta'),
  (258, '128d9183-6188-4ca1-abdb-fc5c57e78d28', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 75
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 75 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 94 then
    raise exception 'Precondicao falhou: conteudo 75 materia_id=% assunto_id=% (esperado 11/94)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 75) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 75 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '128d9183-6188-4ca1-abdb-fc5c57e78d28' and curso_conteudo_id = 75 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_estatuto_da_pessoa_com_deficiencia.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 75
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 71) then
    raise exception 'Precondicao falhou: curso_conteudos 71 (homonimo tematico) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 94);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=94', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 75);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 75', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 75) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 75 != 1';
  end if;
  ...
  -- Pos-condicao extra: conteudo 71 (homonimo tematico) permanece intocado.
  ...
  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / conteudo 71 intocado.';
end $$;
