-- TESTE DE ROLLBACK REAL da IMPORTACAO LOTE09A_LEGISLACAO_ESPECIFICA.
...
begin
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_8_PODERES_DA_ADMINISTRACAO_PUBLICA', v_uteis = 8, v_uteis::text);
  ...
  insert into _relatorio values ('TARGET', 'gabaritos_PODERES_DA_ADMINISTRACAO_PUBLICA', v_gabaritos = 'BBAA', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_PODER_DE_POLICIA', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '2f0d3b9c-fe22-4173-a712-cfb9e1060b8c';
  insert into _relatorio values ('TARGET', 'gabaritos_PODER_DE_POLICIA', v_gabaritos = 'BABCBA', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_ESTATUTO_NACIONAL_DA_IGUALDADE_RACIAL', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '5bf890e1-8e09-4f7a-9410-7f5e5168d9c4';
  insert into _relatorio values ('TARGET', 'gabaritos_ESTATUTO_NACIONAL_DA_IGUALDADE_RACIAL', v_gabaritos = 'BBBBC', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_DIREITO_ADMINISTRATIVO', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '1172a885-1419-4ad8-b728-1a0c7492c133';
  insert into _relatorio values ('TARGET', 'gabaritos_DIREITO_ADMINISTRATIVO', v_gabaritos = 'AA', v_gabaritos);
...
begin
  ...
  if v_total = v_ok then
    raise notice 'TESTE DE ROLLBACK: %/% checks OK — LOTE09A_LEGISLACAO_ESPECIFICA_ROLLBACK_TEST_APROVADO', v_ok, v_total;
  else
