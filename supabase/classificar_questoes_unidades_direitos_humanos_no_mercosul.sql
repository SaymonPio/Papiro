...
-- supabase/mapa_classificacao_direitos_humanos_no_mercosul.sql,
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_casos_do_brasil_na_corte_interamericana_de_direitos_humanos.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_direitos_humanos_no_mercosul.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente fec87f6c-c735-46f1-8bd1-7bbaf6f56e93 — não cria unidade nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (162, 'fec87f6c-c735-46f1-8bd1-7bbaf6f56e93'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 91 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 91 then
    raise exception 'Precondicao falhou: conteudo 91 materia_id=% assunto_id=% (esperado 11/91)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 91) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 91 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'fec87f6c-c735-46f1-8bd1-7bbaf6f56e93' and curso_conteudo_id = 91 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_direitos_humanos_no_mercosul.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=91', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 91', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 91) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 91 != 1';
  end if;
  ...
  where id = 'fec87f6c-c735-46f1-8bd1-7bbaf6f56e93';
  ...
  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / artigos_esperados = [art. 1].';
end $$;
