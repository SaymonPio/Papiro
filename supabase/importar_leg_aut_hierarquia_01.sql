-- IMPORTACAO LEG-AUT-HIERARQUIA-01 — 7 questoes autorais para a unidade
-- "Hierarquia e disciplina" (curso_conteudo_id=52), curso Brigada
-- Militar RS. Fecha o deficit desta unidade (3 -> 10 uteis).
--
-- Padrao identico ao lote AUT anterior (INFO-AUT-01): staging de
...
--
-- NAO altera nenhuma das 3 questoes existentes na unidade (Q12, Q45, Q303).
-- NAO altera unidades/conteudos/materias/aulas.
--
-- ROLLBACK-TESTADO em importar_leg_aut_hierarquia_01_teste_rollback.sql
-- REVERSAO REAL disponivel em reverter_leg_aut_hierarquia_01.sql
...
begin
  ...
  if v_uteis <> 3 then raise exception 'PRECOND: uteis atuais=% esperado 3', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='cbde0aeb-3df8-4c41-a82f-91b83b529668' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 1 then raise exception 'PRECOND: REAL atual=% esperado 1', v_real; end if;
...
Está(ão) correta(s):')
  ...
  raise notice 'PRECONDICOES OK: unidade/conteudo/materia corretos, 3 uteis (1 REAL + 2 AUTORAL), gap curso_questoes=0, 0 duplicidade de enunciado';
end $$;
...
begin
  ...
  where qup.unidade_pedagogica_id='cbde0aeb-3df8-4c41-a82f-91b83b529668' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 1 then raise exception 'POSCOND: REAL=% esperado 1', v_real; end if;
  if v_autoral <> 9 then raise exception 'POSCOND: AUTORAL=% esperado 9', v_autoral; end if;
  ...
  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos
  from _mapa_ids m;
  if v_gabaritos <> 'ECBACDC' then raise exception 'POSCOND: gabaritos=% esperado ECBACDC', v_gabaritos; end if;

  select string_agg((select dificuldade from public.questoes where id=m.questao_id), ',' order by m.ordem)
    into v_dificuldades
  from _mapa_ids m;
  if v_dificuldades <> 'facil,media,media,media,dificil,dificil,media' then raise exception 'POSCOND: dificuldades=% esperado facil,media,media,media,dificil,dificil,media', v_dificuldades; end if;
  ...
  if v_vinc_novos <> 7 then raise exception 'POSCOND: %/7 vinculos novos confirmados', v_vinc_novos; end if;

  select count(*) into v_vinc_novos from _mapa_ids m
    where not exists(select 1 from public.questao_unidades_pedagogicas qup where qup.questao_id=m.questao_id and qup.unidade_pedagogica_id <> 'cbde0aeb-3df8-4c41-a82f-91b83b529668');
  if v_vinc_novos <> 7 then raise exception 'POSCOND: alguma das 7 tem vinculo em unidade errada'; end if;
  ...
  if v_cq_novos <> 7 then raise exception 'POSCOND: %/7 curso_questoes novas confirmadas', v_cq_novos; end if;

  raise notice 'POSCONDICOES OK: +7 questoes, +35 alternativas, +7 vinculos, +7 curso_questoes; unidade agora com 10 uteis (1 REAL + 9 AUTORAL); gabaritos ECBACDC confirmados; dificuldades confirmadas';
end $$;
