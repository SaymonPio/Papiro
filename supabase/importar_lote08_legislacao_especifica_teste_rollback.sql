-- TESTE DE ROLLBACK REAL da IMPORTACAO LOTE08_LEGISLACAO_ESPECIFICA.
-- Executa, dentro de UMA transacao controlada que termina em ROLLBACK
-- externo, a sequencia completa: OLD (baseline) -> APPLY (mesma logica do
-- apply real) -> TARGET (verifica) -> REVERSAO REAL (mesma logica do
-- reverter real) -> OLD_FINAL (verifica). Nenhuma linha e alterada
-- permanentemente — tudo e desfeito por ROLLBACK.
...
begin
  ...
  insert into _relatorio values ('TARGET', 'questoes_delta_23', v_questoes - v_snap.total_questoes = 23, (v_questoes - v_snap.total_questoes)::text);
  insert into _relatorio values ('TARGET', 'alternativas_delta_115', v_alternativas - v_snap.total_alternativas = 115, (v_alternativas - v_snap.total_alternativas)::text);
  insert into _relatorio values ('TARGET', 'vinculos_delta_23', v_vinculos - v_snap.total_vinculos = 23, (v_vinculos - v_snap.total_vinculos)::text);
  insert into _relatorio values ('TARGET', 'curso_questoes_delta_23', v_curso_questoes - v_snap.total_curso_questoes = 23, (v_curso_questoes - v_snap.total_curso_questoes)::text);

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='d6f03d69-8943-4867-a2af-e37482d4ca99' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_RESPONSABILIDADE_CIVIL_DO_ESTADO', v_uteis = 10, v_uteis::text);
  ...
  insert into _relatorio values ('TARGET', 'gabaritos_RESPONSABILIDADE_CIVIL_DO_ESTADO', v_gabaritos = 'ABCBBCC', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_GARANTIAS_E_REMEDIOS_CONSTITUCIONAIS', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = 'f3a6d9c2-8b41-4e0a-9c7d-2b5e8f1a4d63';
  insert into _relatorio values ('TARGET', 'gabaritos_GARANTIAS_E_REMEDIOS_CONSTITUCIONAIS', v_gabaritos = 'CD', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_DEFESA_DO_ESTADO_E_DAS_INSTITUICOES_DEMO', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = 'ecdb1d62-ef44-4407-b899-85911402bc90';
  insert into _relatorio values ('TARGET', 'gabaritos_DEFESA_DO_ESTADO_E_DAS_INSTITUICOES_DEMO', v_gabaritos = 'BEBA', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_SUJEITOS_SANCOES_PROCESSO_DE_IMPROBIDADE', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '9d4c7a1e-3f68-4b52-8a91-6c0d5e2f7b84';
  insert into _relatorio values ('TARGET', 'gabaritos_SUJEITOS_SANCOES_PROCESSO_DE_IMPROBIDADE', v_gabaritos = 'BAC', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_ATOS_DE_IMPROBIDADE_ADMINISTRATIVA', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '60927a85-1b4a-480a-b8da-8eb318520692';
  insert into _relatorio values ('TARGET', 'gabaritos_ATOS_DE_IMPROBIDADE_ADMINISTRATIVA', v_gabaritos = 'D', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_LEI_MARIA_DA_PENHA_REDE_DE_JUSTICA', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '53dc06a1-cd16-4004-a76b-8201d95a91c4';
  insert into _relatorio values ('TARGET', 'gabaritos_LEI_MARIA_DA_PENHA_REDE_DE_JUSTICA', v_gabaritos = 'BABC', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_ATOS_ADMINISTRATIVOS', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '85323ce5-772b-4bf1-bc32-a79e2316158b';
  insert into _relatorio values ('TARGET', 'gabaritos_ATOS_ADMINISTRATIVOS', v_gabaritos = 'CB', v_gabaritos);
  ...
  insert into _relatorio values ('TARGET', 'vinculos_23_corretos', v_vinc_ok = 23, v_vinc_ok::text);
  ...
  insert into _relatorio values ('TARGET', 'curso_questoes_23', v_cq_ok = 23, v_cq_ok::text);

  insert into _relatorio values ('TARGET', 'ativa_23de23',
    (select count(*) from _mapa_ids m join public.questoes q on q.id=m.questao_id where q.ativa) = 23, 'ativa');
  insert into _relatorio values ('TARGET', 'origem_papiro_23de23',
    (select count(*) from _mapa_ids m join public.questoes q on q.id=m.questao_id where coalesce(lower(q.banca),'') like '%papiro%') = 23, 'banca papiro');
end $$;
...
begin
  ...
  insert into _relatorio values ('REVERSAO', 'vinculos_removidos_23', v_count = 23, v_count::text);
end $$;
...
begin
  ...
  insert into _relatorio values ('REVERSAO', 'curso_questoes_removidas_23', v_count = 23, v_count::text);
end $$;
...
begin
  ...
  get diagnostics v_count = row_count;
  insert into _relatorio values ('REVERSAO', 'alternativas_removidas_115', v_count = 115, v_count::text);
end $$;
...
begin
  ...
  get diagnostics v_count = row_count;
  insert into _relatorio values ('REVERSAO', 'questoes_removidas_23', v_count = 23, v_count::text);
end $$;
...
begin
  ...
  if v_total = v_ok then
    raise notice 'TESTE DE ROLLBACK: %/% checks OK — LOTE08_LEGISLACAO_ESPECIFICA_ROLLBACK_TEST_APROVADO', v_ok, v_total;
  else
