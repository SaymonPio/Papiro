...
-- Responsabilidade civil do Estado (curso_conteudos.id = 61) — gerado
...
-- classificar_questoes_unidades_responsabilidade_civil_do_estado.sql (a
...
--
-- NOTA: CF art. 37, §6º, unico dispositivo desta unidade. Q214 reforcada
-- pelo Tema 940/STF (RE 1.027.633/2019) — documentado apenas em
-- escopo/_nota/mapa, sem entrada ficticia em artigos_esperados.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 61, com
...
--    ordem 1 (Responsabilidade civil do Estado) = 3
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 61
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 61 aponta para questao
...
--    d6f03d69-8943-4867-a2af-e37482d4ca99): curso_conteudos=93,
