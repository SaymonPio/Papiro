...
-- questoes de Pacto de San José da Costa Rica (curso_conteudos.id = 72)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/pacto_de_san_jose_da_costa_rica.unidades.json + config/pacto_de_san_jose_da_costa_rica.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_pacto_de_san_jose_da_costa_rica.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_pacto_de_san_jose_da_costa_rica.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (56, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (130, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (140, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (144, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (819, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (820, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (821, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (822, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (823, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (824, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta'),
  (825, 'b918b069-8364-4412-80a6-07d78b369317', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_72_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 90,
    format('materia_id=%s assunto_id=%s (esperado 11/90)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'b918b069-8364-4412-80a6-07d78b369317' and curso_conteudo_id = 72 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 72 — depende de curadoria_unidades_pacto_de_san_jose_da_costa_rica.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 72) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 72 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 72)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 72
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  insert into _relatorio values (
    'total_candidatas_11',
    v_total_candidatas = 11,
    format('total_candidatas=%s (esperado 11)', v_total_candidatas)
  );
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 90);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=90', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 11 ativas', v_fora_do_candidato)
  );

  select count(*) into v_unidade_fora
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 72);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 72', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'b918b069-8364-4412-80a6-07d78b369317';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_pacto_de_san_jose_da_costa_rica) ===';
