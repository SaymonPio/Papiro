...
-- supabase/mapa_classificacao_entendimentos_do_stf_e_stj.sql,
...
--
-- ATENCAO: sobreposicao normativa/tematica documentada (nao duplicata)
-- com curso_conteudo_id 69 ("Jurisprudencia do STF e STJ", materia_id
-- 10, ja concluido — mesma SV 11, questao diferente) e com
-- curso_conteudo_id 76 ("Direitos fundamentais", ja concluido — mesmo
-- art. 5o XI, texto literal vs. interpretacao jurisprudencial). Nenhum
-- dos dois e tocado por este arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_convencao_interamericana_para_prevenir_e_punir_a_tortura.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_entendimentos_do_stf_e_stj.sql
...
select
  (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 69) as unidades_conteudo_69,
  (select count(*) from public.questao_unidades_pedagogicas qup
     join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
     where u.curso_conteudo_id = 69) as vinculos_conteudo_69,
  (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 76) as unidades_conteudo_76,
  (select count(*) from public.questao_unidades_pedagogicas qup
     join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
     where u.curso_conteudo_id = 76) as vinculos_conteudo_76;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (146, '572c64ec-c7bb-412e-ab3d-94435ed7df12'::uuid, 1, 'alta'),
  (253, '572c64ec-c7bb-412e-ab3d-94435ed7df12', 1, 'alta'),
  (254, '572c64ec-c7bb-412e-ab3d-94435ed7df12', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 80
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 80 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 96 then
    raise exception 'Precondicao falhou: conteudo 80 materia_id=% assunto_id=% (esperado 11/96)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 80) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 80 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '572c64ec-c7bb-412e-ab3d-94435ed7df12' and curso_conteudo_id = 80 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_entendimentos_do_stf_e_stj.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 80
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 69) then
    raise exception 'Precondicao falhou: curso_conteudos 69 (sobreposicao SV 11) nao existe mais — verificar integridade';
  end if;
  if not exists (select 1 from public.curso_conteudos where id = 76) then
    raise exception 'Precondicao falhou: curso_conteudos 76 (sobreposicao art. 5o XI) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 96);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=96', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 80);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 80', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 80) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 80 != 1';
  end if;
  ...
  -- Pos-condicao extra: conteudos sobrepostos (69 e 76) permanecem intocados.
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 69) <> (select unidades_conteudo_69 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 69 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 69) <> (select vinculos_conteudo_69 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 69 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 76) <> (select unidades_conteudo_76 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 76 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 76) <> (select vinculos_conteudo_76 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 76 mudou — nao deveria ser tocado';
  end if;

  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / conteudos 69 e 76 intocados.';
end $$;
