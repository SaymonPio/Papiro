-- Pos-check SOMENTE LEITURA da classificacao de questoes de Abuso de Autoridade
-- (curso_conteudos.id = 62) — gerado seguindo o mesmo template de
...
-- classificar_questoes_unidades_abuso_de_autoridade.sql (a versao
...
-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 62, com
...
--    ordem 1 (Abuso de Autoridade) = 11
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 62
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 62 aponta para questao
...
--    7f7298d6-c0b8-4907-8f72-1072a0796a5f): curso_conteudos=93,
