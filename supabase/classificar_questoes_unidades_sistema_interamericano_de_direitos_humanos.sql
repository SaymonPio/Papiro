...
-- supabase/mapa_classificacao_sistema_interamericano_de_direitos_humanos.sql,
...
-- conteudos ja concluidos 72 (Pacto de San Jose, CADH substantiva) e 88
-- (Corte Interamericana, composicao/decisoes), e com o futuro conteudo
-- 87 (Comissao Interamericana, ainda pendente, ordem 45 — NAO
-- antecipado nem tocado por este arquivo).
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_pactos_internacionais_de_direitos_humanos.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_sistema_interamericano_de_direitos_humanos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (183, 'd815fc1f-82d3-4411-9dc5-63dae5373d2b'::uuid, 1, 'alta'),
  (184, 'd815fc1f-82d3-4411-9dc5-63dae5373d2b', 1, 'alta'),
  (185, 'd815fc1f-82d3-4411-9dc5-63dae5373d2b', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 85
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 85 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 86 then
    raise exception 'Precondicao falhou: conteudo 85 materia_id=% assunto_id=% (esperado 11/86)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 85) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 85 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'd815fc1f-82d3-4411-9dc5-63dae5373d2b' and curso_conteudo_id = 85 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_sistema_interamericano_de_direitos_humanos.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 85
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 72) then
    raise exception 'Precondicao falhou: curso_conteudos 72 (sobreposicao CADH) nao existe mais — verificar integridade';
  end if;
  if not exists (select 1 from public.curso_conteudos where id = 87) then
    raise exception 'Precondicao falhou: curso_conteudos 87 (futuro, Comissao Interamericana) nao existe mais — verificar integridade';
  end if;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 86);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=86', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 85);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 85', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 85) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 85 != 1';
  end if;
  ...
  -- Pos-condicao extra: conteudos sobrepostos (72, 87, 88) permanecem intocados.
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 87) <> (select unidades_87 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 87 mudou — nao deveria ser tocado (ainda pendente)';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 87) <> (select vinculos_87 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 87 mudou — nao deveria ser tocado (ainda pendente)';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / conteudos 72, 87 e 88 intocados.';
end $$;
