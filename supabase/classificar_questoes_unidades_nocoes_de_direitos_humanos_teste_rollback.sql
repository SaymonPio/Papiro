...
-- questoes de Noções de Direitos Humanos (curso_conteudos.id = 81)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/nocoes_de_direitos_humanos.unidades.json + config/nocoes_de_direitos_humanos.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_nocoes_de_direitos_humanos.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_nocoes_de_direitos_humanos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (168, 'df8d133f-ddd6-4b85-941d-60b4d4967c06', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_81_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 89,
    format('materia_id=%s assunto_id=%s (esperado 11/89)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'df8d133f-ddd6-4b85-941d-60b4d4967c06' and curso_conteudo_id = 81 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 81 — depende de curadoria_unidades_nocoes_de_direitos_humanos.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 81) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 81 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 81)));
  else
  ...
  insert into _relatorio values (
    'total_candidatas_7',
    v_total_candidatas = 7,
    format('total_candidatas=%s (esperado 7)', v_total_candidatas)
  );
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=89', v_invalidas)
  );
  ...
  select exists (select 1 from _mapa where questao_id in (624,697)) into v_contem_excluida;
  insert into _relatorio values (
    'mapa_nao_contem_excluidas',
    not coalesce(v_contem_excluida, false),
    'as questoes excluidas intencionalmente (624, 697) nao devem constar no mapa'
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 7 ativas', v_fora_do_candidato)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 81', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'df8d133f-ddd6-4b85-941d-60b4d4967c06';
  ...
  select exists (
    ...
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 81 and qup.questao_id = 624
  ) into v_excluida_tmp;
  insert into _relatorio values ('624_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 624 (fora de escopo — testa o Estatuto Nacional da Igualdade Racial (Lei no 12.288/2010, art. 52, caput), ja classificado nos curso_conteudo_id 67 e 97; materialmente incompativel com Nocoes de Direitos Humanos; sinalizada para saneamento futuro de assunto_id) nao deve ter sido classificada por este arquivo');

  select exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 81 and qup.questao_id = 697
  ) into v_excluida_tmp;
  insert into _relatorio values ('697_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 697 (fora de escopo — testa o Estatuto do Idoso (Lei no 10.741/2003, art. 34, caput); materialmente incompativel com Nocoes de Direitos Humanos; sinalizada para saneamento futuro de assunto_id) nao deve ter sido classificada por este arquivo');
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_nocoes_de_direitos_humanos) ===';
