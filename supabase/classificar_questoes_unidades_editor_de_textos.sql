...
-- config/editor_de_textos.mapa.json,
-- validado pelo harness
-- classificar_questoes_unidades_editor_de_textos_teste_rollback.sql
-- (tudo_ok = true confirmado antes de rodar este arquivo).
--
-- Segundo conteúdo de Informática curado nesta metodologia após Windows
-- (ordem 78). artigos_esperados desta unidade é NULL (metodologia não
-- juridica). 2 exclusões intencionais: Q791 (PROBLEMA_DE_FIDELIDADE_IMAGEM_
-- Q791, confirmado por fonte primária — prova original da Fundatec/GM
-- Imbé 2023, Questão 16, exigia reconhecimento visual de ícone via imagem
-- embutida, substituída no Papiro por descrição textual) e Q832
-- (DUPLICATA_Q832, mesma questão-fonte de Q91). Ambas permanecem ativas,
-- intactas, sem vínculo, não corrigidas nesta operação. As 13 questões
-- vinculadas são 13 rows REAL (Fundatec) — 0 AUTORAL neste corpus.
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_editor_de_textos.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- 71df17f6-18a9-49c8-a8da-025329e43bc7 — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (61, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (91, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (92, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (96, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (104, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (110, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (341, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (627, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (628, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (629, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (701, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (769, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta'),
  (831, '71df17f6-18a9-49c8-a8da-025329e43bc7'::uuid, 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 36 nao existe';
  end if;
  if v_materia_id is distinct from 9 or v_assunto_id is distinct from 61 then
    raise exception 'Precondicao falhou: conteudo 36 materia_id=% assunto_id=% (esperado 9/61)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '71df17f6-18a9-49c8-a8da-025329e43bc7' and curso_conteudo_id = 36 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_editor_de_textos.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 36) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 36 — decisao aprovada foi manter unidade unica';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 36
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
    and q.materia_id = cm.materia_id
    and (cc.assunto_id is null or q.assunto_id = cc.assunto_id);

  if v_total_candidatas <> 15 then
    raise exception 'Precondicao falhou: total de candidatas ativas = % (esperado 15)', v_total_candidatas;
  end if;
  ...
  -- Q791 e Q832 (exclusoes intencionais) nao podem ja estar classificadas aqui.
  if exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 36 and qup.questao_id in (791, 832)
  ) then
    raise exception 'Precondicao falhou: a questao 791 (PROBLEMA_DE_FIDELIDADE_IMAGEM_Q791) ou 832 (DUPLICATA_Q832) ja possui vinculo neste conteudo — nao deveria';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 9 and q.assunto_id = 61);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=9/assunto_id=61', v_invalidas;
  end if;

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  if v_distintas <> 13 then
    raise exception 'Mapa invalido: cobre % questoes distintas (esperado 13)', v_distintas;
  end if;

  select exists (select 1 from _mapa where questao_id in (791, 832)) into v_contem_excluida;
  if coalesce(v_contem_excluida, false) then
    raise exception 'Mapa invalido: as questoes excluidas intencionalmente (791, 832) nao deveriam constar no mapa';
  end if;
  ...
  if v_fora_do_candidato <> 0 then
    raise exception 'Mapa invalido: % linha(s) fora do conjunto candidato de 15 ativas', v_fora_do_candidato;
  end if;

  select count(*) into v_unidade_fora
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 36);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 36', v_unidade_fora;
  end if;
end $$;
...
declare
  ...
  v_excluidas_vinculos int;
begin
  select count(*) into v_total_vinculos
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 36;
  if v_total_vinculos <> 13 then
    raise exception 'Pos-condicao falhou: total_vinculos=% (esperado 13)', v_total_vinculos;
  end if;
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 36;
  if v_questoes_classificadas <> 13 then
    raise exception 'Pos-condicao falhou: questoes_classificadas=% (esperado 13)', v_questoes_classificadas;
  end if;
  ...
  select count(*) into v_excluidas_vinculos
  from public.questao_unidades_pedagogicas
  where questao_id in (791, 832);
  if v_excluidas_vinculos <> 0 then
    raise exception 'Pos-condicao falhou: questoes 791/832 (excluidas intencionalmente) ganharam vinculo (%), deveriam permanecer sem vinculo', v_excluidas_vinculos;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 36) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 36 != 1';
  end if;
  ...
  if (select count(*) from public.questao_unidades_pedagogicas) <> (select total_vinculos from _snapshot_antes) + 13 then
    raise exception 'Pos-condicao falhou: total de vinculos nao cresceu exatamente 13';
  end if;

  -- Pos-condicao extra: artigos_esperados deve ser NULL.
  select artigos_esperados into v_artigos_esperados
  from public.unidades_pedagogicas
  where id = '71df17f6-18a9-49c8-a8da-025329e43bc7';
  ...
  raise notice 'Pos-condicoes OK: 13 questoes classificadas / 13 vinculos / 0 multiunidade / Q791+Q832 sem vinculo / artigos_esperados NULL.';
end $$;
