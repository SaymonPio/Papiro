...
-- supabase/mapa_classificacao_poder_de_policia.sql,
...
-- mesmo padrão de supabase/classificar_questoes_unidades_poderes_da_administracao_publica.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_poder_de_policia.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (204, '2f0d3b9c-fe22-4173-a712-cfb9e1060b8c'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 60 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 69 then
    raise exception 'Precondicao falhou: conteudo 60 materia_id=% assunto_id=% (esperado 10/69)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 60) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 60 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '2f0d3b9c-fe22-4173-a712-cfb9e1060b8c' and curso_conteudo_id = 60 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_poder_de_policia.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=69', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 60', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 60) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 60 != 1';
  end if;
