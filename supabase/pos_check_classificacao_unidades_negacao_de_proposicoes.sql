...
-- Negação de proposições (curso_conteudos.id = 3).
--
...
-- exclusoes. 4 questoes classificadas: 3 rows REAL (Fundatec) + 1
-- AUTORAL (Q312). Pendencias externas de taxonomia (Q77, Q88, Q311 em
-- Leis de De Morgan; Q287 em Quantificadores) NAO fazem parte deste
-- conteudo e NAO devem ser tocadas.

-- 1) A unidade pedagogica do conteudo 3. Esperado: 1 linha, ordem 1,
...
-- 4) QIDs vinculados exatos. Esperado: 81, 86, 312, 337.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 3
  );
...
-- 8) Confirma banca de todas as 4 candidatas (3 REAL + 1 AUTORAL) e
--    vinculo neste conteudo. Esperado: 4 linhas, todas vinculo=1.
select q.id, q.banca, q.ano,
  (select count(*) from public.questao_unidades_pedagogicas qup2 join public.unidades_pedagogicas u2 on u2.id=qup2.unidade_pedagogica_id where qup2.questao_id=q.id and u2.curso_conteudo_id=3) as vinculo_neste_conteudo
from public.questoes q
where q.ativa = true and q.materia_id = 18 and q.assunto_id = 35
order by q.id;

-- 9) Confirma que as 4 questoes externas (pendencias de taxonomia)
--    permanecem intactas: ativas, com seu assunto_id original, e SEM
--    vinculo neste conteudo. Esperado: assunto_id 77/88/311=34, 287=33;
--    vinculos_neste_conteudo=0 para todas.
select id, ativa, assunto_id,
  (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where qup.questao_id = q.id and u.curso_conteudo_id = 3) as vinculos_neste_conteudo
from public.questoes q
where id in (77, 88, 311, 287)
order by id;

-- 10) Confirma artigos_esperados NULL na unidade e estado geral do
--     sistema.
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 3) as artigos_esperados_null,
