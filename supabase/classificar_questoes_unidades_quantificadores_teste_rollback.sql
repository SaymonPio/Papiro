...
-- questoes de Quantificadores (curso_conteudos.id = 7)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/quantificadores.unidades.json + config/quantificadores.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_quantificadores.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_quantificadores.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (83, 'b46a6a8e-d7bf-4c19-81f3-5f0fd3352f8c', 1, 'alta'),
  (288, 'b46a6a8e-d7bf-4c19-81f3-5f0fd3352f8c', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_7_materia_assunto',
    v_materia_id is not distinct from 18 and v_assunto_id is not distinct from 33,
    format('materia_id=%s assunto_id=%s (esperado 18/33)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'b46a6a8e-d7bf-4c19-81f3-5f0fd3352f8c' and curso_conteudo_id = 7 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 7 — depende de curadoria_unidades_quantificadores.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 7) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 7 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 7)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=18/assunto_id=33', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 7', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'b46a6a8e-d7bf-4c19-81f3-5f0fd3352f8c';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_quantificadores) ===';
