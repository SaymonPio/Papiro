...
-- questoes de Equivalências lógicas (curso_conteudos.id = 5)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/equivalencias_logicas.unidades.json + config/equivalencias_logicas.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_equivalencias_logicas.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_equivalencias_logicas.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (79, '56df08f8-0f22-48c1-a64d-df11ebfc5ae9', 1, 'alta'),
  (82, '56df08f8-0f22-48c1-a64d-df11ebfc5ae9', 1, 'alta'),
  (310, '56df08f8-0f22-48c1-a64d-df11ebfc5ae9', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_5_materia_assunto',
    v_materia_id is not distinct from 18 and v_assunto_id is not distinct from 41,
    format('materia_id=%s assunto_id=%s (esperado 18/41)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '56df08f8-0f22-48c1-a64d-df11ebfc5ae9' and curso_conteudo_id = 5 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 5 — depende de curadoria_unidades_equivalencias_logicas.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 5) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 5 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 5)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=18/assunto_id=41', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 5', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '56df08f8-0f22-48c1-a64d-df11ebfc5ae9';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_equivalencias_logicas) ===';
