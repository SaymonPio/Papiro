-- Pos-check SOMENTE LEITURA da classificacao de questoes do Sistema
-- Interamericano de Direitos Humanos (curso_conteudos.id = 85) —
...
--
-- NOTA: CADH, art. 33 (orgaos do sistema). Preambulo da CADH
-- (complementaridade, Q185) documentado apenas em escopo/mapa, sem
-- artigo artificial.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 85, com
--    titulo/escopo/artigos aplicados pela curadoria. Esperado: 1 linha(s),
--    ordem 1, artigos_esperados = ["art. 33"], ativa.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 85
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 85 aponta para questao
...
--    d815fc1f-82d3-4411-9dc5-63dae5373d2b): curso_conteudos=93,
...
-- 8) Conteudos 72 (Pacto de San Jose, ja concluido), 87 (Comissao
--    Interamericana, ainda pendente) e 88 (Corte Interamericana, ja
--    concluido) permanecem intocados por esta operacao. Esperado
--    (baseline ao vivo em 2026-08-22, antes desta operacao):
--    unidades_72=1, vinculos_72=11, unidades_87=1, vinculos_87=0,
--    unidades_88=1, vinculos_88=4.
select
...
-- 9) Confirma exatamente 1 artigo_esperado ("art. 33").
--    Esperado: contem_art33=true, total=1.
select
