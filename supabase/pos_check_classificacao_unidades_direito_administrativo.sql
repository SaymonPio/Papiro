-- Pos-check SOMENTE LEITURA da classificacao de questoes de Direito Administrativo
-- (curso_conteudos.id = 58) — gerado seguindo o mesmo template de
...
-- classificar_questoes_unidades_direito_administrativo.sql (a versao
...
--
-- NOTA: sobreposicoes tematicas conhecidas e mantidas por decisao do
-- usuario: Q197 (autotutela) tambem testada no curso_conteudo_id 54
-- (Atos administrativos); Q721/Q774/Q796 usam CF art. 37, tambem
-- parcialmente coberto pelo curso_conteudo_id 57 (Constituicao Federal de
-- 1988). Nenhuma questao duplicada entre conteudos.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 58, com
...
--    ordem 1 (Direito Administrativo) = 8
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 58
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 58 aponta para questao
...
--    1172a885-1419-4ad8-b728-1a0c7492c133): curso_conteudos=93,
