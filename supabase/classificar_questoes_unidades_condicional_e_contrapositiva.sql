-- Aplicação REAL do mapa aprovado em config/condicional_e_contrapositiva.mapa.json,
...
--
-- Quinto conteúdo do bloco final de Raciocínio Lógico/Matemática, após
-- Equivalências lógicas (ordem 87). artigos_esperados desta unidade é
-- NULL (metodologia não jurídica). 0 exclusões — as 2 questões ativas
-- LIVE foram classificadas (1 row REAL Fundatec + 1 AUTORAL Q309).
--
-- DIVERGÊNCIA FILA×LIVE (cronologia, não erro): a fila histórica
-- (ordem-curadoria.json, ordem 88) documenta questoes_ativas=3, retrato
-- de antes do saneamento taxonômico dedicado da Q84 (commit 6256b12,
-- assunto_id 39 -> 36, reclassificada para Proposições e conectivos por
-- testar apenas a condição de falsidade da condicional, sem exigir
-- contrapositiva). O estado LIVE nesta curadoria final é 2 candidatas.
--
-- AUDITORIA TAXONÔMICA (executada e aprovada antes deste apply):
-- confirmou que Q80 e Q309 exigem genuinamente a contrapositiva
-- (P→Q≡¬Q→¬P), identidade distinta de (a) condição de falsidade
-- isolada da condicional (Proposições e conectivos, destino da Q84),
-- (b) equivalência disjuntiva P→Q≡¬P∨Q (Equivalências lógicas, Q82/
-- Q310), e (c) negação da condicional P∧¬Q (Negação de proposições).
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_condicional_e_contrapositiva.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- 42f5f55c-350a-4fb6-904c-184cde415d1e — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (80, '42f5f55c-350a-4fb6-904c-184cde415d1e'::uuid, 1, 'alta'),
  (309, '42f5f55c-350a-4fb6-904c-184cde415d1e'::uuid, 1, 'alta');

-- Lock deterministico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 6
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 6 nao existe';
  end if;
  if v_materia_id is distinct from 18 or v_assunto_id is distinct from 39 then
    raise exception 'Precondicao falhou: conteudo 6 materia_id=% assunto_id=% (esperado 18/39)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '42f5f55c-350a-4fb6-904c-184cde415d1e' and curso_conteudo_id = 6 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_condicional_e_contrapositiva.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 6) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 6 — decisao aprovada foi manter unidade unica';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 6
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if v_total_candidatas <> 2 then
    raise exception 'Precondicao falhou: total de candidatas ativas LIVE = % (esperado 2 — apos saneamento taxonomico da Q84 no commit 6256b12)', v_total_candidatas;
  end if;

  select count(*) into v_classificacoes_previas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 6
  ...
  -- Q84 (reclassificada para Proposicoes e conectivos no commit
  -- 6256b12) NAO faz parte desta ordem: precisa continuar com
  -- assunto_id=36 (nao 39) e sem nenhum vinculo neste conteudo.
  select assunto_id into v_assunto_84 from public.questoes where id = 84;
  if v_assunto_84 is distinct from 36 then
    raise exception 'Precondicao falhou: Q84 assunto_id=% (esperado 36 — ja deveria estar reclassificada antes desta curadoria)', v_assunto_84;
  end if;

  select count(*) into v_vinculos_84_aqui
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 6 and qup.questao_id = 84;
  if v_vinculos_84_aqui <> 0 then
    raise exception 'Precondicao falhou: Q84 possui vinculo neste conteudo (Condicional e contrapositiva), nao deveria';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 18 and q.assunto_id = 39);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=18/assunto_id=39', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 6);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 6', v_unidade_fora;
  end if;
end $$;
...
declare
  ...
  v_assunto_84 bigint;
  v_vinculos_84_conteudo6 int;
  v_vinculos_84_conteudo1 int;
begin
  ...
  -- Pos-condicao extra: Q84 (ja reclassificada no commit 6256b12)
  ...
  select assunto_id into v_assunto_84 from public.questoes where id = 84;
  if v_assunto_84 is distinct from 36 then
    raise exception 'Pos-condicao falhou: assunto_id da Q84 foi alterado indevidamente (%), esperado 36', v_assunto_84;
  end if;

  select count(*) into v_vinculos_84_conteudo6
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 6 and qup.questao_id = 84;
  if v_vinculos_84_conteudo6 <> 0 then
    raise exception 'Pos-condicao falhou: Q84 ganhou vinculo neste conteudo (Condicional e contrapositiva), nao deveria';
  end if;

  select count(*) into v_vinculos_84_conteudo1
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 1 and qup.questao_id = 84;
  if v_vinculos_84_conteudo1 <> 1 then
    raise exception 'Pos-condicao falhou: Q84 deveria manter exatamente 1 vinculo em Proposicoes e conectivos (conteudo 1), atual=%', v_vinculos_84_conteudo1;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 6) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 6 != 1';
  end if;
  ...
  where id = '42f5f55c-350a-4fb6-904c-184cde415d1e';
  ...
  raise notice 'Pos-condicoes OK: 2 questoes classificadas (Q80/Q309) / 2 vinculos / 0 multiunidade / artigos_esperados NULL / Q84 intocada (assunto_id=36, 1 vinculo em Proposicoes e conectivos).';
end $$;
