...
-- questoes de Acentuação gráfica (curso_conteudos.id = 26)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/acentuacao_grafica.unidades.json + config/acentuacao_grafica.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_acentuacao_grafica.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_acentuacao_grafica.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (216, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_26_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 44,
    format('materia_id=%s assunto_id=%s (esperado 6/44)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'd100427d-d567-43e8-8ec8-81d44e5e3afe' and curso_conteudo_id = 26 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 26 — depende de curadoria_unidades_acentuacao_grafica.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 26) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 26 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 26)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=44', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 26', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'd100427d-d567-43e8-8ec8-81d44e5e3afe';
  insert into _relatorio values ('u1_15_questoes', v_qtd_tmp = 15, format('u1_questoes=%s (esperado 15)', v_qtd_tmp));
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_acentuacao_grafica) ===';
