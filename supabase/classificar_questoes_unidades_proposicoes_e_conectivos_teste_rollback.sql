...
-- questoes de Proposições e conectivos (curso_conteudos.id = 1)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/proposicoes_e_conectivos.unidades.json + config/proposicoes_e_conectivos.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_proposicoes_e_conectivos.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_proposicoes_e_conectivos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (74, '6683c484-74a7-4b07-9cda-1a72190e6445', 1, 'alta'),
  (87, '6683c484-74a7-4b07-9cda-1a72190e6445', 1, 'alta'),
  (313, '6683c484-74a7-4b07-9cda-1a72190e6445', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_1_materia_assunto',
    v_materia_id is not distinct from 18 and v_assunto_id is not distinct from 36,
    format('materia_id=%s assunto_id=%s (esperado 18/36)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '6683c484-74a7-4b07-9cda-1a72190e6445' and curso_conteudo_id = 1 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 1 — depende de curadoria_unidades_proposicoes_e_conectivos.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 1) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 1 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 1)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=18/assunto_id=36', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 1', v_unidade_fora)
  );
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_proposicoes_e_conectivos) ===';
