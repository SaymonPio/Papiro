...
-- config/fonemas_e_digrafos.mapa.json,
...
--
-- Quarto conteudo de Lingua Portuguesa da fila. artigos_esperados
-- desta unidade e NULL (metodologia nao juridica). 1 exclusao
-- intencional: Q885 (FORA_DE_ESCOPO_ACENTUACAO_TONICIDADE — cobra
-- classificacao de tonicidade/paroxitona, habilidade do conteudo 26,
-- ja concluido; nao reaberto aqui, apenas registrado como possivel
-- destino futuro).
--
-- ATENCAO: sobreposicao tematica (nao duplicata) com o conteudo ja
-- concluido 20 (Ortografia) — a expressao "Corpo de Bombeiros
-- Militar" de Q886 vem do mesmo artigo ja usado integralmente em
-- Q891 (Ortografia). Os conteudos 20, 22 (Classes de palavras) e 26
-- (Acentuação gráfica), todos ja concluidos, nao sao tocados por
-- este arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_acentuacao_grafica.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_fonemas_e_digrafos.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 61bd0288-38fe-45bc-9c1e-51c1ecc9a515 — não cria unidade
-- nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (114, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515'::uuid, 1, 'alta'),
  (277, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (278, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (320, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (686, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (687, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (756, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (757, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (785, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (808, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (884, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (886, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
  (887, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 34 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 56 then
    raise exception 'Precondicao falhou: conteudo 34 materia_id=% assunto_id=% (esperado 6/56)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 34) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 34 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '61bd0288-38fe-45bc-9c1e-51c1ecc9a515' and curso_conteudo_id = 34 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_fonemas_e_digrafos.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 34
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 26) then
    raise exception 'Precondicao falhou: curso_conteudos 26 (Acentuação gráfica — possivel destino futuro de Q885) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 56);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=56', v_invalidas;
  end if;
  ...
  select exists (select 1 from _mapa where questao_id = 885) into v_contem_excluida;
  if coalesce(v_contem_excluida, false) then
    raise exception 'Mapa invalido: contem a questao 885, que deve permanecer excluida (FORA_DE_ESCOPO_ACENTUACAO_TONICIDADE)';
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 34);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 34', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 34) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 34 != 1';
  end if;
  ...
  where id = '61bd0288-38fe-45bc-9c1e-51c1ecc9a515';
  ...
  -- Pos-condicao extra: Q885 (exclusao intencional) permanece SEM vinculo neste conteudo.
  select exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 34 and qup.questao_id = 885
  ) into v_885_classificada;
  if coalesce(v_885_classificada, false) then
    raise exception 'Pos-condicao falhou: questao 885 (FORA_DE_ESCOPO_ACENTUACAO_TONICIDADE) foi classificada indevidamente neste conteudo';
  end if;
  if not exists (select 1 from public.questoes where id = 885 and ativa = true) then
    raise exception 'Pos-condicao falhou: questao 885 nao esta mais ativa=true — nao deveria ter sido alterada';
  end if;

  -- Pos-condicao extra: conteudos 20, 22 e 26 (sobreposicao tematica) permanecem intocados.
  ...
  raise notice 'Pos-condicoes OK: 13 questoes classificadas / 13 vinculos / 0 multiunidade / artigos_esperados NULL / Q885 excluida e intacta / conteudos 20, 22 e 26 intocados.';
end $$;
