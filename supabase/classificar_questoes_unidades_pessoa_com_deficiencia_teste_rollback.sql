...
-- questoes de Pessoa com deficiência (curso_conteudos.id = 71)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/pessoa_com_deficiencia.unidades.json + config/pessoa_com_deficiencia.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_pessoa_com_deficiencia.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_pessoa_com_deficiencia.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (30, '435543fe-bdc2-452a-be2d-ffa414c5e27d', 1, 'alta'),
  (55, '435543fe-bdc2-452a-be2d-ffa414c5e27d', 1, 'alta'),
  (141, '435543fe-bdc2-452a-be2d-ffa414c5e27d', 1, 'alta'),
  (352, '435543fe-bdc2-452a-be2d-ffa414c5e27d', 1, 'alta'),
  (353, '435543fe-bdc2-452a-be2d-ffa414c5e27d', 1, 'alta'),
  (827, '435543fe-bdc2-452a-be2d-ffa414c5e27d', 1, 'alta'),
  (828, '435543fe-bdc2-452a-be2d-ffa414c5e27d', 1, 'alta'),
  (829, '435543fe-bdc2-452a-be2d-ffa414c5e27d', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_71_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 27,
    format('materia_id=%s assunto_id=%s (esperado 11/27)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '435543fe-bdc2-452a-be2d-ffa414c5e27d' and curso_conteudo_id = 71 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 71 — depende de curadoria_unidades_pessoa_com_deficiencia.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 71) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 71 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 71)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 71
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 27);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=27', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 71);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 71', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '435543fe-bdc2-452a-be2d-ffa414c5e27d';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_pessoa_com_deficiencia) ===';
