...
-- config/windows.mapa.json,
-- validado pelo harness
-- classificar_questoes_unidades_windows_teste_rollback.sql
-- (tudo_ok = true confirmado antes de rodar este arquivo).
--
-- Primeiro conteúdo de Informática curado nesta metodologia após Internet
-- e navegadores (ordem 77). artigos_esperados desta unidade é NULL
-- (metodologia não jurídica). 1 exclusão intencional — Q836, DUPLICATA_Q836
-- (mesma questão-fonte de Q101, permanece ativa/intacta/sem vínculo, não
-- corrigida nesta operação). As 14 questões vinculadas são 14 rows REAL
-- (Fundatec) + 1 AUTORAL (Q31, cobertura suplementar) — 13 eventos reais
-- únicos em 11 provas/cadernos independentes (Q101 é a canônica da
-- duplicata com Q836).
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_windows.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- 936d2d9d-0f83-4a25-8066-9e035a12ca16 — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (31, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (60, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (101, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (102, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (103, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (108, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (338, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (339, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (494, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (643, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (644, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (645, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (711, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta'),
  (835, '936d2d9d-0f83-4a25-8066-9e035a12ca16'::uuid, 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 35 nao existe';
  end if;
  if v_materia_id is distinct from 9 or v_assunto_id is distinct from 28 then
    raise exception 'Precondicao falhou: conteudo 35 materia_id=% assunto_id=% (esperado 9/28)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '936d2d9d-0f83-4a25-8066-9e035a12ca16' and curso_conteudo_id = 35 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_windows.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 35) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 35 — decisao aprovada foi manter unidade unica';
  end if;
  ...
  -- Q836 (duplicata excluida intencionalmente) nao pode ja estar classificada aqui.
  if exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 35 and qup.questao_id = 836
  ) then
    raise exception 'Precondicao falhou: a questao 836 (DUPLICATA_Q836) ja possui vinculo neste conteudo — nao deveria';
  end if;
end $$;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=9/assunto_id=28', v_invalidas;
  end if;
  ...
  select exists (select 1 from _mapa where questao_id = 836) into v_contem_excluida;
  if coalesce(v_contem_excluida, false) then
    raise exception 'Mapa invalido: a questao excluida intencionalmente (836) nao deveria constar no mapa';
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 35', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  select count(*) into v_q836_vinculos
  from public.questao_unidades_pedagogicas
  where questao_id = 836;
  if v_q836_vinculos <> 0 then
    raise exception 'Pos-condicao falhou: questao 836 (DUPLICATA_Q836) ganhou vinculo (%), deveria permanecer sem vinculo', v_q836_vinculos;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 35) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 35 != 1';
  end if;
  ...
  where id = '936d2d9d-0f83-4a25-8066-9e035a12ca16';
  ...
  raise notice 'Pos-condicoes OK: 14 questoes classificadas / 14 vinculos / 0 multiunidade / Q836 sem vinculo / artigos_esperados NULL.';
end $$;
