...
-- supabase/mapa_classificacao_prevencao_da_tortura.sql,
...
-- conteudo ja concluido 79 (Convencao Interamericana para Prevenir e
-- Punir a Tortura) — Q250 la testa tese semelhante ("circunstancias
-- excepcionais nao justificam tortura") usando o sistema interamericano
-- (art. 5), enquanto Q29 deste conteudo usa o sistema universal da ONU
-- (art. 2, item 2) — diferenciacao pedagogica deliberada, sem duplicata
-- exata de ID. O conteudo 79 nao e tocado por este arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_dignidade_humana.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_prevencao_da_tortura.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 1c05566b-5c71-4baa-b965-3577b8ffdc17 — não cria unidade nova).
begin;
...
select
  (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 79) as unidades_79,
  (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 79) as vinculos_79;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (29, '1c05566b-5c71-4baa-b965-3577b8ffdc17'::uuid, 1, 'alta'),
  (263, '1c05566b-5c71-4baa-b965-3577b8ffdc17', 1, 'alta'),
  (264, '1c05566b-5c71-4baa-b965-3577b8ffdc17', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 99
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 99 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 26 then
    raise exception 'Precondicao falhou: conteudo 99 materia_id=% assunto_id=% (esperado 11/26)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 99) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 99 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '1c05566b-5c71-4baa-b965-3577b8ffdc17' and curso_conteudo_id = 99 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_prevencao_da_tortura.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 99
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 79) then
    raise exception 'Precondicao falhou: curso_conteudos 79 (sobreposicao tematica Convencao Interamericana Tortura) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 26);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=26', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 99);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 99', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 99) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 99 != 1';
  end if;
  ...
  where id = '1c05566b-5c71-4baa-b965-3577b8ffdc17';
  if v_artigos_esperados is distinct from array['art. 2, item 2','art. 1']::text[] then
    raise exception 'Pos-condicao falhou: artigos_esperados=% (esperado [art. 2, item 2 / art. 1])', v_artigos_esperados;
  end if;

  -- Pos-condicao extra: conteudo 79 (sobreposicao tematica) permanece intocado.
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 79) <> (select unidades_79 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 79 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 79) <> (select vinculos_79 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 79 mudou — nao deveria ser tocado';
  end if;

  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / artigos_esperados = [art. 2, item 2 / art. 1] / conteudo 79 intocado.';
end $$;
