-- TESTE DE ROLLBACK — REVISAO FINAL DE VINCULOS (fluxo completo OLD ->
-- TARGET -> REVERT -> OLD_FINAL -> ROLLBACK; NAO aplica nada permanente;
-- ver mover_e_excluir_questoes_dgf_final.sql para o apply real) —
-- curadoria Fase 9.0.1.
--
...
-- ================= REVERT (restaura vinculos e curso_questoes a partir dos snapshots OLD) =================
insert into public.questao_unidades_pedagogicas (questao_id, unidade_pedagogica_id, classificado_por, criado_em)
select questao_id, unidade_pedagogica_id, classificado_por, criado_em
from _snapshot_old_vinculos_dgf_final;

insert into public.curso_questoes (questao_id, curso_id, prioridade)
select questao_id, curso_id, prioridade
from _snapshot_old_curso_questoes_dgf_final;

-- ================= OLD_FINAL (confirma identidade com OLD) =================
do $$
declare
  v_qtd_vinculos_restaurados int;
  v_qtd_vinculos_snapshot int;
  v_qtd_cq_restaurados int;
  v_qtd_cq_snapshot int;
begin
  select count(*) into v_qtd_vinculos_snapshot from _snapshot_old_vinculos_dgf_final;
  select count(*) into v_qtd_vinculos_restaurados
  from public.questao_unidades_pedagogicas qup
  join _snapshot_old_vinculos_dgf_final t
    on t.questao_id = qup.questao_id
   and t.unidade_pedagogica_id = qup.unidade_pedagogica_id
   and t.classificado_por is not distinct from qup.classificado_por
   and t.criado_em = qup.criado_em;
  if v_qtd_vinculos_restaurados <> v_qtd_vinculos_snapshot then
    raise exception 'OLD_FINAL: nem todos os % vinculos originais retornaram identicos (restaurados: %)', v_qtd_vinculos_snapshot, v_qtd_vinculos_restaurados;
  end if;

  select count(*) into v_qtd_cq_snapshot from _snapshot_old_curso_questoes_dgf_final;
  select count(*) into v_qtd_cq_restaurados
  from public.curso_questoes cq
  join _snapshot_old_curso_questoes_dgf_final t
    on t.questao_id = cq.questao_id
   and t.curso_id = cq.curso_id
   and t.prioridade = cq.prioridade;
  if v_qtd_cq_restaurados <> v_qtd_cq_snapshot then
    raise exception 'OLD_FINAL: nem todas as % linhas de curso_questoes originais retornaram identicas (restauradas: %)', v_qtd_cq_snapshot, v_qtd_cq_restaurados;
  end if;

  raise notice 'OLD_FINAL OK: os 6 vinculos e as 2 linhas de curso_questoes originais retornaram identicos, byte-a-byte — reversao comprovada';
end $$;
