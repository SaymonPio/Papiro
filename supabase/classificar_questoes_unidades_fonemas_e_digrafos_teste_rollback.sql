...
-- questoes de Fonemas e dígrafos (curso_conteudos.id = 34)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/fonemas_e_digrafos.unidades.json + config/fonemas_e_digrafos.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_fonemas_e_digrafos.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_fonemas_e_digrafos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (114, '61bd0288-38fe-45bc-9c1e-51c1ecc9a515', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_34_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 56,
    format('materia_id=%s assunto_id=%s (esperado 6/56)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '61bd0288-38fe-45bc-9c1e-51c1ecc9a515' and curso_conteudo_id = 34 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 34 — depende de curadoria_unidades_fonemas_e_digrafos.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 34) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 34 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 34)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=56', v_invalidas)
  );
  ...
  select exists (select 1 from _mapa where questao_id in (885)) into v_contem_excluida;
  insert into _relatorio values (
    'mapa_nao_contem_excluidas',
    not coalesce(v_contem_excluida, false),
    'as questoes excluidas intencionalmente (885) nao devem constar no mapa'
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 34', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '61bd0288-38fe-45bc-9c1e-51c1ecc9a515';
  ...
  insert into _relatorio values ('885_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 885 (FORA_DE_ESCOPO_ACENTUACAO_TONICIDADE — cobra classificacao quanto a tonicidade (identificacao de palavras paroxitonas: vacina, aumento), habilidade do conteudo 26 (Acentuacao grafica, ja concluido), sem envolver nenhum conceito de fonema, digrafo ou encontro vocalico/consonantal. Provavel destino futuro: conteudo 26, ja concluido, NAO reaberto nesta operacao. Nao realocada nesta etapa.) nao deve ter sido classificada por este arquivo');
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_fonemas_e_digrafos) ===';
