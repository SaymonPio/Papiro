-- Pos-check SOMENTE LEITURA da classificacao de questoes da Comissao
-- Interamericana de Direitos Humanos (curso_conteudos.id = 87) —
...
--
-- NOTA: CADH — art. 33, art. 41 (caput e "f"), art. 44. Estatuto da
-- CIDH art. 1 (fonte complementar da Q152) documentado apenas em
-- escopo/mapa/_nota, nao entra em artigos_esperados.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 87, com
--    titulo/escopo/artigos aplicados pela curadoria. Esperado: 1 linha(s),
--    ordem 1, artigos_esperados com 4 itens, ativa.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 87
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 87 aponta para questao
...
--    1b84fd2f-93e4-46c1-868f-c8402e73bdf9): curso_conteudos=93,
...
-- 8) Conteudos 72, 85, 86 e 88 (ja concluidos) permanecem intocados por
--    esta operacao. Esperado (baseline ao vivo em 2026-08-22, antes
--    desta operacao): unidades_72=1, vinculos_72=11, unidades_85=1,
--    vinculos_85=3, unidades_86=1, vinculos_86=3, unidades_88=1,
--    vinculos_88=4.
select
...
-- 9) Confirma exatamente os 4 artigos_esperados aprovados.
--    Esperado: contem_art33=true, contem_art41_caput=true,
--    contem_art41_f=true, contem_art44=true, total=4.
select
  (artigos_esperados @> array['art. 33']::text[]) as contem_art33,
  (artigos_esperados @> array['art. 41, caput']::text[]) as contem_art41_caput,
  (artigos_esperados @> array['art. 41, "f"']::text[]) as contem_art41_f,
  (artigos_esperados @> array['art. 44']::text[]) as contem_art44,
