...
-- config/coordenacao_e_subordinacao.mapa.json,
...
--
-- Vigésimo primeiro conteúdo de Língua Portuguesa da fila (ordem 74).
...
-- questões REAIS neste corpus). GAP_DE_BANCO_COORDENACAO_E_SUBORDINACAO
-- quantitativo real extremo, confirmado por microauditoria direcionada
-- prévia (nenhuma candidata REAL encontrada em outro assunto, incluindo
-- exame aprofundado de Q123/Transitividade Verbal e Termos Integrantes,
-- confirmada como corretamente classificada lá).
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_coordenacao_e_subordinacao.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- bfeaf283-09fc-4f14-9d0c-41ff0db4eab7 — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (225, 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7'::uuid, 1, 'alta'),
  (226, 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7'::uuid, 1, 'alta'),
  (227, 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7'::uuid, 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 28 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 50 then
    raise exception 'Precondicao falhou: conteudo 28 materia_id=% assunto_id=% (esperado 6/50)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7' and curso_conteudo_id = 28 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_coordenacao_e_subordinacao.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 28) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 28 — decisao aprovada foi manter unidade unica';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=50', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 28', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 28) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 28 != 1';
  end if;
  ...
  where id = 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7';
