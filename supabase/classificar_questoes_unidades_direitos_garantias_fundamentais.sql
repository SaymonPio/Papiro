...
-- validado pelo harness classificar_questoes_unidades_direitos_garantias_
-- fundamentais_teste_rollback.sql (tudo_ok = true esperado antes de rodar
-- este arquivo).
--
...
-- fundamentais.sql precisa já ter sido aplicado.
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (46,  '0c5d1d64-0cae-406e-be19-b03d387bee8a'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 47 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 71 then
    raise exception 'Precondicao falhou: conteudo 47 materia_id=% assunto_id=% (esperado 10/71)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 47 and ativa = true) <> 2 then
    raise exception 'Precondicao falhou: nao ha exatamente 2 unidades pedagogicas ativas para o conteudo 47';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '0c5d1d64-0cae-406e-be19-b03d387bee8a' and curso_conteudo_id = 47 and ordem = 1 and ativa)
  or not exists (select 1 from public.unidades_pedagogicas where id = 'f3a6d9c2-8b41-4e0a-9c7d-2b5e8f1a4d63' and curso_conteudo_id = 47 and ordem = 2 and ativa)
  then
    raise exception 'Precondicao falhou: uma ou ambas as 2 unidades oficiais nao conferem (id/ordem/conteudo/ativa) — curadoria_unidades_direitos_garantias_fundamentais.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=71', v_invalidas;
  end if;

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  if v_distintas <> 21 then
    raise exception 'Mapa invalido: cobre % questoes distintas (esperado 21)', v_distintas;
  end if;
  ...
  if v_fora_do_candidato <> 0 then
    raise exception 'Mapa invalido: % linha(s) fora do conjunto candidato de 21', v_fora_do_candidato;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 47', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 47;
  if v_questoes_classificadas <> 21 then
    raise exception 'Pos-condicao falhou: questoes_classificadas=% (esperado 21)', v_questoes_classificadas;
  end if;
  ...
  if v_multiunidade is distinct from array[46]::bigint[] then
    raise exception 'Pos-condicao falhou: multiunidade=% (esperado {46})', v_multiunidade;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 47 and ativa = true) <> 2 then
    raise exception 'Pos-condicao falhou: unidades ativas do conteudo 47 != 2';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 21 questoes / 22 vinculos / 1 multiunidade (46) aplicados com sucesso.';
end $$;
