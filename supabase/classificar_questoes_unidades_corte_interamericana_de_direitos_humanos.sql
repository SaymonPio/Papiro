...
-- supabase/mapa_classificacao_corte_interamericana_de_direitos_humanos.sql,
...
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_corte_interamericana_de_direitos_humanos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (156, '9ab2c28c-2c1d-4d15-b134-a191ff946529'::uuid, 1, 'media'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 88 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 106 then
    raise exception 'Precondicao falhou: conteudo 88 materia_id=% assunto_id=% (esperado 11/106)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 88) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 88 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '9ab2c28c-2c1d-4d15-b134-a191ff946529' and curso_conteudo_id = 88 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_corte_interamericana_de_direitos_humanos.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=106', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 88', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 88) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 88 != 1';
  end if;
