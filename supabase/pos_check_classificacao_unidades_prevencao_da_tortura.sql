-- Pos-check SOMENTE LEITURA da classificacao de questoes de Prevencao
-- da tortura (curso_conteudos.id = 99) — gerado seguindo o mesmo
...
-- classificar_questoes_unidades_prevencao_da_tortura.sql (a versao que
...
--
-- NOTA: Convencao contra a Tortura da ONU (1984), art. 2, item 2
-- (Q29). Art. 2, item 3 (ordem superior) documentado apenas em
-- escopo/mapa/_nota, NAO no array. Q263 sem artigo (formulacao
-- principiologica). OPCAT, art. 1 (Q264) — Lei 12.847/2013
-- documentada apenas como implementacao domestica em escopo/_nota.
-- Sobreposicao tematica (nao duplicata) com o conteudo 79 (Convencao
-- Interamericana para Prevenir e Punir a Tortura, ja concluido).

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 99, com
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 99
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 99 aponta para questao
...
--    1c05566b-5c71-4baa-b965-3577b8ffdc17): curso_conteudos=93,
...
-- 8) Conteudo 79 (Convencao Interamericana para Prevenir e Punir a
--    Tortura, ja concluido) permanece intocado por esta operacao —
--    sobreposicao apenas tematica, nao duplicata. Esperado (baseline ao
--    vivo em 2026-08-22, antes desta operacao): unidades_79=1,
--    vinculos_79=3.
select
...
-- 9) Confirma exatamente os 2 artigos_esperados aprovados, e que o
--    art. 2, item 3 (ordem superior) NAO foi incluido.
--    Esperado: contem_art2_item2=true, contem_art1=true,
--    contem_art2_item3=false, total=2.
select
  (artigos_esperados @> array['art. 2, item 2']::text[]) as contem_art2_item2,
  (artigos_esperados @> array['art. 1']::text[]) as contem_art1,
  (artigos_esperados @> array['art. 2, item 3']::text[]) as contem_art2_item3,
