...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_pontuacao.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_pontuacao.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (237, 'dca4fe2e-50e9-41db-abeb-0ef6b388c5af', 1, 'alta'),
  (238, 'dca4fe2e-50e9-41db-abeb-0ef6b388c5af', 1, 'alta'),
  (239, 'dca4fe2e-50e9-41db-abeb-0ef6b388c5af', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_16_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 48,
    format('materia_id=%s assunto_id=%s (esperado 6/48)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'dca4fe2e-50e9-41db-abeb-0ef6b388c5af' and curso_conteudo_id = 16 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 16 — depende de curadoria_unidades_pontuacao.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 16) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 16 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 16)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 16
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 48);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=48', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 16);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 16', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'dca4fe2e-50e9-41db-abeb-0ef6b388c5af';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_pontuacao) ===';
