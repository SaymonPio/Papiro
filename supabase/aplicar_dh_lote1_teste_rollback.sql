-- ============================================================================
-- AUDITORIA GLOBAL -- DIREITOS HUMANOS -- LOTE 1 (50 QUESTÕES)
-- Aplicação de 50 explicações pedagógicas (materia_id 11)
-- IDs: 13,28,29,30,55,56,57,58,59,124,125,126,127,128,130,131,138,139,140,141,144,145,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173
-- HARNESS TRANSACIONAL -- TERMINA SEMPRE EM ROLLBACK, NADA PERSISTE.
-- ============================================================================
--
-- Gerado automaticamente por scripts/generate-dh-lote1-harness.mjs a partir de
-- scripts/dh-lote1-explicacoes.mjs.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- Staging: id -> nova explicacao (fonte: scripts/dh-lote1-explicacoes.mjs).
-- ----------------------------------------------------------------------------
create temporary table _dh1_novas_explicacoes (id bigint primary key, explicacao text) on commit drop;
insert into _dh1_novas_explicacoes (id, explicacao) values
(13, 'GABARITO: alternativa A
...
create temporary table _dh1_snap_questoes on commit drop as
select id, (to_jsonb(q) - 'explicacao' - 'atualizado_em') as dados_imutaveis
from public.questoes q
where q.id in (13,28,29,30,55,56,57,58,59,124,125,126,127,128,130,131,138,139,140,141,144,145,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173);

-- 2) alternativas completas das 50.
create temporary table _dh1_snap_alternativas on commit drop as
select questao_id, jsonb_agg(to_jsonb(a) order by a.ordem) as alternativas
from public.alternativas a
where a.questao_id in (13,28,29,30,55,56,57,58,59,124,125,126,127,128,130,131,138,139,140,141,144,145,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173)
group by questao_id;

-- 3) hash de explicacao de TODAS as questoes do banco.
create temporary table _dh1_snap_hash_todas on commit drop as
select id, md5(coalesce(explicacao, '')) as hash_explicacao
from public.questoes;

-- 4) contagens globais.
create temporary table _dh1_snap_global on commit drop as
select
...
create temporary table _dh1_asserts (ordem serial primary key, descricao text, ok boolean) on commit drop;
...
declare
  v_qtd int;
begin
  if (select count(*) from _dh1_novas_explicacoes) <> 50 then
    raise exception 'PRECONDICAO FALHOU: staging nao tem exatamente 50 explicacoes';
  end if;

  select count(*) into v_qtd from public.questoes where id in (select id from _dh1_novas_explicacoes);
  if v_qtd <> 50 then
  ...
  if exists (
    ...
    join _dh1_novas_explicacoes s on s.id = q.id
...
-- ----------------------------------------------------------------------------
create temporary table _dh1_ids_afetados (id bigint primary key) on commit drop;
...
begin
  with atualizado as (
    ...
    from _dh1_novas_explicacoes s
    where q.id = s.id
    returning q.id
  )
  insert into _dh1_ids_afetados (id) select id from atualizado;
...
declare
  ...
  v_sem_correta int;
begin
  insert into _dh1_asserts (descricao, ok)
  select 'exatamente 50 questoes afetadas pelo UPDATE', (select count(*) from _dh1_ids_afetados) = 50;

  insert into _dh1_asserts (descricao, ok)
  select 'os ids afetados sao exatamente os 50 esperados',
    (select array_agg(id order by id) from _dh1_ids_afetados) = ARRAY[13,28,29,30,55,56,57,58,59,124,125,126,127,128,130,131,138,139,140,141,144,145,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173]::bigint[];
  ...
  select 'enunciado/alternativas-relacao/gabarito/materia/assunto/ativa/banca/concurso/ano/fonte preservados nas 50 (comparacao jsonb byte-a-byte, exceto explicacao/atualizado_em)',
    not exists (
      select 1 from public.questoes q
      join _dh1_snap_questoes s on s.id = q.id
  ...
  select 'alternativas das 50 continuam byte-identicas (texto/correta/ordem)',
    not exists (
      select 1
      from _dh1_snap_alternativas s
      join (
        select questao_id, jsonb_agg(to_jsonb(a) order by a.ordem) as alternativas
        from public.alternativas a
        where a.questao_id in (select id from _dh1_novas_explicacoes)
        group by questao_id
  ...
  insert into _dh1_asserts (descricao, ok) values ('as 50 continuam com exatamente 1 alternativa correta cada', v_sem_correta = 0);
  ...
  with alt_stats as (
    ...
    where q.id in (select id from _dh1_novas_explicacoes)
    group by q.id
  ),
  classificado as (
    select q.id,
  ...
  insert into _dh1_asserts (descricao, ok) values ('as 50 passam no classificador canonico como EXPLICACAO_COMPLETA', v_completas = 50);

  insert into _dh1_asserts (descricao, ok)
  select 'nenhuma outra questao do banco teve explicacao alterada',
    not exists (
      select 1 from public.questoes q
      join _dh1_snap_hash_todas s on s.id = q.id
      where md5(coalesce(q.explicacao, '')) <> s.hash_explicacao
        and q.id <> ALL(ARRAY[13,28,29,30,55,56,57,58,59,124,125,126,127,128,130,131,138,139,140,141,144,145,146,147,148,149,150,151,152,153,154,155,156,157,158,159,160,161,162,163,164,165,166,167,168,169,170,171,172,173]::bigint[])
    );

  select count(*) into v_total_depois from public.questoes;
  insert into _dh1_asserts (descricao, ok) values ('total de questoes no banco inalterado (nenhuma criada/excluida)', v_total_depois = (select total_questoes_antes from _dh1_snap_global));

  select count(*) into v_ativas_depois from public.questoes where ativa = true;
  insert into _dh1_asserts (descricao, ok) values ('total de questoes ATIVAS do Papiro inalterado', v_ativas_depois = (select total_ativas_antes from _dh1_snap_global));
end $$;
...
declare
  ...
  v_ok integer;
begin
  for r in select descricao, ok from _dh1_asserts order by ordem loop
    if r.ok then
  ...
  select count(*), count(*) filter (where ok) into v_total, v_ok from _dh1_asserts;
