...
-- questoes de Hierarquia e disciplina (curso_conteudos.id = 52)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/hierarquia_e_disciplina.unidades.json + config/hierarquia_e_disciplina.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_hierarquia_e_disciplina.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_hierarquia_e_disciplina.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (12, 'cbde0aeb-3df8-4c41-a82f-91b83b529668', 1, 'alta'),
  (45, 'cbde0aeb-3df8-4c41-a82f-91b83b529668', 1, 'alta'),
  (303, 'cbde0aeb-3df8-4c41-a82f-91b83b529668', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_52_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 11,
    format('materia_id=%s assunto_id=%s (esperado 10/11)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'cbde0aeb-3df8-4c41-a82f-91b83b529668' and curso_conteudo_id = 52 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 52 — depende de curadoria_unidades_hierarquia_e_disciplina.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 52) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 52 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 52)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 52
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 11);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=11', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 52);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 52', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'cbde0aeb-3df8-4c41-a82f-91b83b529668';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_hierarquia_e_disciplina) ===';
