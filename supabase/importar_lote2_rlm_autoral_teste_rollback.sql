-- Harness de teste (SEMPRE termina em ROLLBACK) da importacao do Lote 2
...
-- pacote_importacao_rlm_lote2_autoral.json (29 candidatas, status =
...
begin
  ...
  insert into _relatorio values ('distribuicao_cc1', v_qtd_cc1 = 1, format('cc1: %s novos vinculos (esperado 1)', v_qtd_cc1));

  select count(*) into v_qtd_cc2
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 2 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc2', v_qtd_cc2 = 4, format('cc2: %s novos vinculos (esperado 4)', v_qtd_cc2));
  ...
  insert into _relatorio values ('distribuicao_cc6', v_qtd_cc6 = 4, format('cc6: %s novos vinculos (esperado 4)', v_qtd_cc6));

  select count(*) into v_qtd_cc7
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 7 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc7', v_qtd_cc7 = 4, format('cc7: %s novos vinculos (esperado 4)', v_qtd_cc7));
  ...
  insert into _relatorio values ('distribuicao_cc11', v_qtd_cc11 = 4, format('cc11: %s novos vinculos (esperado 4)', v_qtd_cc11));
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (importar_lote2_rlm_autoral) ===';
