...
-- supabase/mapa_classificacao_sistema_internacional_de_protecao_dos_direitos_humanos.sql,
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_entendimentos_do_stf_e_stj.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_sistema_internacional_de_protecao_dos_direitos_humanos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (186, '522f7c40-b95e-4c91-b0d6-cf4a6f012c16'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 82 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 83 then
    raise exception 'Precondicao falhou: conteudo 82 materia_id=% assunto_id=% (esperado 11/83)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 82) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 82 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '522f7c40-b95e-4c91-b0d6-cf4a6f012c16' and curso_conteudo_id = 82 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_sistema_internacional_de_protecao_dos_direitos_humanos.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=83', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 82', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 82) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 82 != 1';
  end if;
