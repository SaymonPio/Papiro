...
-- supabase/mapa_classificacao_abuso_de_autoridade_direitos_humanos_70.sql,
...
--
-- ATENCAO: este arquivo cobre exclusivamente o curso_conteudo_id 70
-- ("Abuso de Autoridade", materia_id 11/Direitos Humanos e Cidadania).
-- NAO confundir com curso_conteudo_id 62 (mesmo nome, materia_id
-- 10/Legislacao Especifica, ja concluido, arquivos com slug
-- abuso_de_autoridade sem sufixo) — slug deliberadamente diferenciado
-- (abuso_de_autoridade_direitos_humanos_70) para nao colidir.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_direito_a_cidadania_na_constituicao_federal.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_abuso_de_autoridade_direitos_humanos_70.sql
...
create temporary table _snapshot_conteudo_62_antes on commit drop as
select
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (57, 'd4d8a1fc-4c52-4c85-879c-031d0085be88'::uuid, 1, 'alta'),
  (128, 'd4d8a1fc-4c52-4c85-879c-031d0085be88', 1, 'alta'),
  (291, 'd4d8a1fc-4c52-4c85-879c-031d0085be88', 1, 'alta'),
  (354, 'd4d8a1fc-4c52-4c85-879c-031d0085be88', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 70 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 103 then
    raise exception 'Precondicao falhou: conteudo 70 materia_id=% assunto_id=% (esperado 11/103)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 70) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 70 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'd4d8a1fc-4c52-4c85-879c-031d0085be88' and curso_conteudo_id = 70 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_abuso_de_autoridade_direitos_humanos_70.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 70
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  -- Precondicao extra: conteudo 62 (homonimo) deve ja existir concluido e intocado.
  if not exists (select 1 from public.curso_conteudos where id = 62) then
    raise exception 'Precondicao falhou: curso_conteudos 62 (homonimo) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 103);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=103', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 70);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 70', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 70) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 70 != 1';
  end if;
  ...
  -- Pos-condicao extra: conteudo 62 (homonimo) permanece intocado.
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 62) <> (select unidades_conteudo_62 from _snapshot_conteudo_62_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 62 (homonimo) mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 62) <> (select vinculos_conteudo_62 from _snapshot_conteudo_62_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 62 (homonimo) mudou — nao deveria ser tocado';
  end if;

  raise notice 'Pos-condicoes OK: 4 questoes / 4 vinculos / 0 multiunidade / conteudo 62 intocado.';
end $$;
