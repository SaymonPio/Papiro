...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_windows.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_windows.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (31, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (60, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (101, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (102, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (103, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (108, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (338, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (339, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (494, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (643, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (644, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (645, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (711, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta'),
  (835, '936d2d9d-0f83-4a25-8066-9e035a12ca16', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_35_materia_assunto',
    v_materia_id is not distinct from 9 and v_assunto_id is not distinct from 28,
    format('materia_id=%s assunto_id=%s (esperado 9/28)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '936d2d9d-0f83-4a25-8066-9e035a12ca16' and curso_conteudo_id = 35 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 35 — depende de curadoria_unidades_windows.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 35) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 35 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 35)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 35
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 9 and q.assunto_id = 28);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=9/assunto_id=28', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_14_distintas',
    v_distintas = 14,
    format('mapa cobre %s questoes distintas (esperado 14)', v_distintas)
  );

  select exists (select 1 from _mapa where questao_id in (836)) into v_contem_excluida;
  insert into _relatorio values (
    'mapa_nao_contem_excluidas',
    not coalesce(v_contem_excluida, false),
    'as questoes excluidas intencionalmente (836) nao devem constar no mapa'
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 35);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 35', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 35;
  insert into _relatorio values ('total_vinculos_14', v_total_vinculos = 14, format('total_vinculos=%s (esperado 14)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 35;
  insert into _relatorio values ('questoes_classificadas_14', v_questoes_classificadas = 14, format('questoes_classificadas=%s (esperado 14)', v_questoes_classificadas));
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '936d2d9d-0f83-4a25-8066-9e035a12ca16';
  insert into _relatorio values ('u1_14_questoes', v_qtd_tmp = 14, format('u1_questoes=%s (esperado 14)', v_qtd_tmp));

  select exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 35 and qup.questao_id = 836
  ) into v_excluida_tmp;
  insert into _relatorio values ('836_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 836 (DUPLICATA_Q836 — mesmo enunciado, mesmas 5 alternativas e mesmo gabarito (B) de Q101 ("praia ou serra?.pdf" como nome inválido pelo caractere ?), mesmo bloco de Informática do concurso SUSEPE/Polícia Penal RS 01/2022 (cargos diferentes, mesma prova de origem). Q836 possui glitch textual "arquivoinválido" (sem espaço), enquanto Q101 é a versão mais limpa e foi escolhida como questão canônica vinculada. Q836 permanece ATIVA, INTACTA e SEM vínculo neste conteúdo — não corrigida, não desativada, não contabilizada como segunda incidência independente do fenômeno de caracteres proibidos.) nao deve ter sido classificada por este arquivo');
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_14',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 14,
    'total de vinculos deve crescer exatamente 14 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_windows) ===';
