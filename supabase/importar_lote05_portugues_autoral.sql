-- IMPORTACAO LOTE05_PORTUGUES_AUTORAL — 17 questoes autorais para 5 unidades de
-- Lingua Portuguesa (curso Brigada Militar RS): Vozes verbais (+3), Regencia
-- verbal e nominal (+2), Transitividade Verbal e Termos Integrantes (+5),
-- Conectores (+4), Coesao textual (+3).
--
-- Human sign-off explicito, revisao final unica (mandato "PAPIRO — LOTE 05 —
-- HUMAN REVIEW FINAL + IMPORTACAO AUTOMATIZADA"). Nao selecionadas/reserva
-- (7): VOZ-04, REG-02, REG-04, TRANS-05, CONECT-05, CONECT-06, COES-04 —
-- NENHUMA importada nesta fase. Redacao oficial permanece fora deste lote.
--
...
-- automaticamente).
--
-- NAO altera nenhuma questao existente nas 5 unidades.
-- NAO altera unidades/conteudos/materias/aulas.
--
-- Texto final e o texto persistido em
-- outputs/curadoria-autoral/review/LOTE05-PORTUGUES-HUMAN-REVIEW.md,
-- congelado em outputs/curadoria-autoral/review/LOTE05-FROZEN-17.json
-- (corpus_hash 10d7bff71554da4d004cf4b33c13b5f3d6d9f898750e2d4c94ff80c1e483b65c).
--
-- ROLLBACK-TESTADO em importar_lote05_portugues_autoral_teste_rollback.sql
-- REVERSAO REAL disponivel em reverter_lote05_portugues_autoral.sql
...
begin
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Vozes verbais=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_autoral + v_real <> 10 then raise exception 'POSCOND: uteis(real+autoral) unidade Vozes verbais=% esperado 10', v_autoral + v_real; end if;
  ...
  if v_gabaritos <> 'AAB' then raise exception 'POSCOND: gabaritos unidade Vozes verbais=% esperado AAB', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='735f736a-37c0-477f-a555-dcd73d243d21' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Regência verbal e nominal=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='735f736a-37c0-477f-a555-dcd73d243d21' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_autoral + v_real <> 10 then raise exception 'POSCOND: uteis(real+autoral) unidade Regência verbal e nominal=% esperado 10', v_autoral + v_real; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '735f736a-37c0-477f-a555-dcd73d243d21';
  if v_gabaritos <> 'BA' then raise exception 'POSCOND: gabaritos unidade Regência verbal e nominal=% esperado BA', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='981e5d2c-3b59-48a0-a699-a53c03e500ee' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Transitividade Verbal e Termos Integrantes=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='981e5d2c-3b59-48a0-a699-a53c03e500ee' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_autoral + v_real <> 10 then raise exception 'POSCOND: uteis(real+autoral) unidade Transitividade Verbal e Termos Integrantes=% esperado 10', v_autoral + v_real; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '981e5d2c-3b59-48a0-a699-a53c03e500ee';
  if v_gabaritos <> 'DBBBA' then raise exception 'POSCOND: gabaritos unidade Transitividade Verbal e Termos Integrantes=% esperado DBBBA', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='d1e31767-d27d-431b-ba59-7a2008c7473d' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Conectores=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='d1e31767-d27d-431b-ba59-7a2008c7473d' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_autoral + v_real <> 10 then raise exception 'POSCOND: uteis(real+autoral) unidade Conectores=% esperado 10', v_autoral + v_real; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = 'd1e31767-d27d-431b-ba59-7a2008c7473d';
  if v_gabaritos <> 'CBCB' then raise exception 'POSCOND: gabaritos unidade Conectores=% esperado CBCB', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='29a4bec1-2c3a-40f3-a86f-fa6bda25d04f' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Coesão textual=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='29a4bec1-2c3a-40f3-a86f-fa6bda25d04f' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_autoral + v_real <> 10 then raise exception 'POSCOND: uteis(real+autoral) unidade Coesão textual=% esperado 10', v_autoral + v_real; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '29a4bec1-2c3a-40f3-a86f-fa6bda25d04f';
  if v_gabaritos <> 'CDA' then raise exception 'POSCOND: gabaritos unidade Coesão textual=% esperado CDA', v_gabaritos; end if;
  ...
  raise notice 'POSCONDICOES OK: +17 questoes, +85 alternativas, +17 vinculos, +17 curso_questoes; todas as 5 unidades com gabaritos e contagens confirmadas';
end $$;
