...
-- questoes de Lei de Tortura (curso_conteudos.id = 73)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/lei_de_tortura.unidades.json + config/lei_de_tortura.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_lei_de_tortura.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_lei_de_tortura.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (59, '392fd9fe-a3d2-4062-b912-0a299b414429', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_73_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 102,
    format('materia_id=%s assunto_id=%s (esperado 11/102)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '392fd9fe-a3d2-4062-b912-0a299b414429' and curso_conteudo_id = 73 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 73 — depende de curadoria_unidades_lei_de_tortura.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 73) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 73 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 73)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=102', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 73', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '392fd9fe-a3d2-4062-b912-0a299b414429';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_lei_de_tortura) ===';
