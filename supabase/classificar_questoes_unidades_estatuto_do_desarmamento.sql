...
-- supabase/mapa_classificacao_estatuto_do_desarmamento.sql, validado pelo harness
-- classificar_questoes_unidades_estatuto_do_desarmamento_teste_rollback.sql (tudo_ok =
...
-- mesmo padrão de supabase/classificar_questoes_unidades_constituicao_do_estado_do_rio_grande_do_sul.sql.
--
-- NOTA: Q142 e Q853 são duplicatas confirmadas (mesmo fato, mesmas 5
-- alternativas na mesma ordem) — ambas classificadas por decisão explícita
-- do usuário nesta curadoria; nenhuma foi desativada ou alterada. O
-- saneamento de duplicatas fica para um fluxo separado, futuro.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_estatuto_do_desarmamento.sql precisa já
-- ter sido aplicado.
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (50, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f'::uuid, 1, 'alta'),
  (142, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (299, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (664, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (728, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (729, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (730, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (776, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (777, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (853, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (854, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
  (855, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 56
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 56 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 68 then
    raise exception 'Precondicao falhou: conteudo 56 materia_id=% assunto_id=% (esperado 10/68)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 56) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 56 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f' and curso_conteudo_id = 56 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_estatuto_do_desarmamento.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 56
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
    and q.materia_id = cm.materia_id
    and (cc.assunto_id is null or q.assunto_id = cc.assunto_id);

  if v_total_candidatas <> 12 then
    raise exception 'Precondicao falhou: total de candidatas ativas = % (esperado 12)', v_total_candidatas;
  end if;

  select count(*) into v_classificacoes_previas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 56
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 68);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=68', v_invalidas;
  end if;

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  if v_distintas <> 12 then
    raise exception 'Mapa invalido: cobre % questoes distintas (esperado 12)', v_distintas;
  end if;
  ...
  if v_fora_do_candidato <> 0 then
    raise exception 'Mapa invalido: % linha(s) fora do conjunto candidato de 12 ativas', v_fora_do_candidato;
  end if;

  select count(*) into v_unidade_fora
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 56);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 56', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 56;
  if v_total_vinculos <> 12 then
    raise exception 'Pos-condicao falhou: total_vinculos=% (esperado 12)', v_total_vinculos;
  end if;
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 56;
  if v_questoes_classificadas <> 12 then
    raise exception 'Pos-condicao falhou: questoes_classificadas=% (esperado 12)', v_questoes_classificadas;
  end if;
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 56) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 56 != 1';
  end if;
  ...
  if (select count(*) from public.questao_unidades_pedagogicas) <> (select total_vinculos from _snapshot_antes) + 12 then
    raise exception 'Pos-condicao falhou: total de vinculos nao cresceu exatamente 12';
  end if;

  raise notice 'Pos-condicoes OK: 12 questoes / 12 vinculos / 0 multiunidade.';
end $$;
