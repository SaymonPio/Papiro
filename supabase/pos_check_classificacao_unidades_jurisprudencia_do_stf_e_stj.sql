-- Pos-check SOMENTE LEITURA da classificacao de questoes de Jurisprudência
-- do STF e STJ (curso_conteudos.id = 69) — gerado seguindo o mesmo
-- template de scripts/curadoria-pedagogica/gerar-pos-check.mjs. Os totais
-- da consulta 7 foram obtidos ao vivo (leitura direta, somente SELECT)
...
--
-- NOTA: conteudo de natureza jurisprudencial (nao um diploma unico).
-- artigos_esperados contem SOMENTE dispositivos constitucionais/legais
-- efetivamente interpretados pelos precedentes (arts. 5º XI/LVII, 39 §4º,
-- 40 §4º-B e 144 §9º da Constituição Federal; art. 283 do Código de
-- Processo Penal). Súmula Vinculante 11, Tema 280/RE 603.616, ADCs 43/44/54,
-- ADO 26/MI 4733 e Tema 541/ARE 654.432 ficam documentados apenas em texto
-- (escopo/mapa), nunca como "artigo" fictício. CORREÇÃO DE AUDITORIA: a
-- unidade reaproveitada é a placeholder já existente
-- (8e1d1204-1f66-431c-a95f-6403e77882dc); um UUID novo havia sido proposto
-- por engano e foi corrigido antes do apply.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 69, com
...
--    ordem 1 (Jurisprudência do STF e STJ) = 5
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 69
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 69 aponta para questao
...
--    8e1d1204-1f66-431c-a95f-6403e77882dc): curso_conteudos=93,
