...
-- Vozes verbais (curso_conteudos.id = 32).
--
...
-- 1) A unidade pedagogica do conteudo 32. Esperado: 1 linha, ordem 1,
...
-- 4) QIDs vinculados exatos. Esperado: 243, 244, 245.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 32
  );
...
-- 9) Confirma que Q72, Q280, Q283, Q284, Q332 (candidatas descartadas na
--    microauditoria) permanecem exatamente onde estavam, sem vinculo
--    neste conteudo. Esperado: 0 linhas.
select qup.questao_id, qup.unidade_pedagogica_id
from public.questao_unidades_pedagogicas qup
join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
where u.curso_conteudo_id = 32 and qup.questao_id in (72, 280, 283, 284, 332);

-- 10) Confirma Q693 (pendencia externa) permanece intacta, ativa e sem
--     vinculo em qualquer lugar. Esperado: ativa=true, vinculos=0.
select id, ativa,
  (select count(*) from public.questao_unidades_pedagogicas where questao_id = q.id) as vinculos
from public.questoes q where id = 693;

-- 11) Confirma artigos_esperados NULL e estado geral do sistema.
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 32) as artigos_esperados_null,
