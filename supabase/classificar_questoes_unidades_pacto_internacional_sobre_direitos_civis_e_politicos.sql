...
-- supabase/mapa_classificacao_pacto_internacional_sobre_direitos_civis_e_politicos.sql,
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_programa_nacional_de_direitos_humanos_pndh_3.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_pacto_internacional_sobre_direitos_civis_e_politicos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (58, '8d4e4b20-37ac-4df0-a7ac-57cb016c44d6'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 74 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 85 then
    raise exception 'Precondicao falhou: conteudo 74 materia_id=% assunto_id=% (esperado 11/85)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 74) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 74 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '8d4e4b20-37ac-4df0-a7ac-57cb016c44d6' and curso_conteudo_id = 74 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_pacto_internacional_sobre_direitos_civis_e_politicos.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=85', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 74', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 74) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 74 != 1';
  end if;
