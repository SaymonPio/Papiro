...
-- questoes de Segurança da informação (curso_conteudos.id = 40)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/seguranca_da_informacao.unidades.json + config/seguranca_da_informacao.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_seguranca_da_informacao.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_seguranca_da_informacao.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (11, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (15, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (32, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (294, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (639, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (640, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (641, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (642, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (706, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (707, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (708, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (709, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta'),
  (710, 'a042af68-babf-4385-bcc4-2fa69e9b27cd', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_40_materia_assunto',
    v_materia_id is not distinct from 9 and v_assunto_id is not distinct from 10,
    format('materia_id=%s assunto_id=%s (esperado 9/10)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'a042af68-babf-4385-bcc4-2fa69e9b27cd' and curso_conteudo_id = 40 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 40 — depende de curadoria_unidades_seguranca_da_informacao.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 40) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 40 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 40)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=9/assunto_id=10', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 40', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'a042af68-babf-4385-bcc4-2fa69e9b27cd';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_seguranca_da_informacao) ===';
