...
-- supabase/mapa_classificacao_atos_administrativos.sql, validado pelo harness
-- classificar_questoes_unidades_atos_administrativos_teste_rollback.sql (tudo_ok =
...
-- mesmo padrão de supabase/classificar_questoes_unidades_seguranca_publica.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_atos_administrativos.sql precisa já
-- ter sido aplicado.
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (44, '85323ce5-772b-4bf1-bc32-a79e2316158b'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 54 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 67 then
    raise exception 'Precondicao falhou: conteudo 54 materia_id=% assunto_id=% (esperado 10/67)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 54) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 54 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '85323ce5-772b-4bf1-bc32-a79e2316158b' and curso_conteudo_id = 54 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_atos_administrativos.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=67', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 54', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 54) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 54 != 1';
  end if;
