-- TESTE DE ROLLBACK REAL da IMPORTACAO LOTE07_LEGISLACAO_INSTITUCIONAL.
...
begin
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_LOB', v_uteis = 10, v_uteis::text);
  ...
  insert into _relatorio values ('TARGET', 'gabaritos_LOB', v_gabaritos = 'CAAAAABB', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_PLANO_DE_CARREIRA', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200';
  insert into _relatorio values ('TARGET', 'gabaritos_PLANO_DE_CARREIRA', v_gabaritos = 'ABCBAA', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_ESTATUTO', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = 'bf13f365-3dd9-4d22-9ad7-f369a298eb19';
  insert into _relatorio values ('TARGET', 'gabaritos_ESTATUTO', v_gabaritos = 'BBCAA', v_gabaritos);
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  insert into _relatorio values ('TARGET', 'uteis_10_REGULAMENTO_DISCIPLINAR', v_uteis = 10, v_uteis::text);

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '454f8501-7818-4dc4-b22a-337247678c58';
  insert into _relatorio values ('TARGET', 'gabaritos_REGULAMENTO_DISCIPLINAR', v_gabaritos = 'ABCD', v_gabaritos);
...
begin
  ...
  if v_total = v_ok then
    raise notice 'TESTE DE ROLLBACK: %/% checks OK — LOTE07_LEGISLACAO_INSTITUCIONAL_ROLLBACK_TEST_APROVADO', v_ok, v_total;
  else
