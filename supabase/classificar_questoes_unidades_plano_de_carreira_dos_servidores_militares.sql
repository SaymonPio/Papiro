...
-- supabase/mapa_classificacao_plano_de_carreira_dos_servidores_militares.sql,
...
-- mesmo padrão de supabase/classificar_questoes_unidades_poder_de_policia.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_plano_de_carreira_dos_servidores_militares.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (201, '9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 65 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 77 then
    raise exception 'Precondicao falhou: conteudo 65 materia_id=% assunto_id=% (esperado 10/77)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 65) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 65 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200' and curso_conteudo_id = 65 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_plano_de_carreira_dos_servidores_militares.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=77', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 65', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 65) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 65 != 1';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 4 questoes / 4 vinculos / 0 multiunidade.';
end $$;
