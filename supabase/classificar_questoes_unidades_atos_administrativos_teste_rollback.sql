...
-- questoes de Atos administrativos (curso_conteudos.id = 54)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/atos_administrativos.unidades.json + config/atos_administrativos.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_atos_administrativos.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_atos_administrativos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (44, '85323ce5-772b-4bf1-bc32-a79e2316158b', 1, 'alta'),
  (267, '85323ce5-772b-4bf1-bc32-a79e2316158b', 1, 'alta'),
  (268, '85323ce5-772b-4bf1-bc32-a79e2316158b', 1, 'alta'),
  (358, '85323ce5-772b-4bf1-bc32-a79e2316158b', 1, 'alta'),
  (646, '85323ce5-772b-4bf1-bc32-a79e2316158b', 1, 'alta'),
  (714, '85323ce5-772b-4bf1-bc32-a79e2316158b', 1, 'alta'),
  (840, '85323ce5-772b-4bf1-bc32-a79e2316158b', 1, 'alta'),
  (841, '85323ce5-772b-4bf1-bc32-a79e2316158b', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_54_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 67,
    format('materia_id=%s assunto_id=%s (esperado 10/67)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '85323ce5-772b-4bf1-bc32-a79e2316158b' and curso_conteudo_id = 54 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 54 — depende de curadoria_unidades_atos_administrativos.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 54) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 54 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 54)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 54
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 67);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=67', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 54);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 54', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '85323ce5-772b-4bf1-bc32-a79e2316158b';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_atos_administrativos) ===';
