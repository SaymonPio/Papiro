-- IMPORTACAO LOTE02_PORTUGUES_AUTORAL — 24 questoes autorais para 4 unidades de
-- Lingua Portuguesa (curso Brigada Militar RS): Reescrita de frases e
-- textos (+8), Interpretacao de textos (+7), Regencia verbal e nominal
-- (+5), Vozes verbais (+4).
--
-- Human sign-off explicito (Fase 3A.3 -> 3A.3.1 -> 3A.4). Reservas
-- (Interpretacao Q1, Regencia Q5/Q6/Q7) NAO importadas nesta fase.
-- REGENCIA permanece com deficit 2 (COVERAGE_GAP_REGENCIA_NOMINAL).
-- VOZES permanece com deficit 3 (COVERAGE_GAPS_PRESERVED).
--
-- Padrao identico aos lotes AUT anteriores (CV-AUT-01, INFO-AUT-01,
-- LEG-AUT-HIERARQUIA-01): staging de questoes/explicacoes/alternativas,
-- insercao por INSERT direto em questoes/alternativas (nunca em
-- questao_unidades_pedagogicas), vinculo exclusivamente via
...
--
-- ROLLBACK-TESTADO em importar_lote02_portugues_autoral_teste_rollback.sql
-- REVERSAO REAL disponivel em reverter_lote02_portugues_autoral.sql
...
Quais afirmativas estão corretas?$DUP24$)
  ...
  raise notice 'PRECONDICOES GLOBAIS OK: 0 duplicidade de enunciado entre as 24';
end $$;

-- ================= INSERT das 24 questoes =================
...
begin
  ...
  if v_questoes - v_snap.total_questoes <> 24 then raise exception 'POSCOND: questoes criadas=% esperado 24', v_questoes - v_snap.total_questoes; end if;
  if v_alternativas - v_snap.total_alternativas <> 120 then raise exception 'POSCOND: alternativas criadas=% esperado 120', v_alternativas - v_snap.total_alternativas; end if;
  if v_vinculos - v_snap.total_vinculos <> 24 then raise exception 'POSCOND: vinculos criados=% esperado 24', v_vinculos - v_snap.total_vinculos; end if;
  if v_curso_questoes - v_snap.total_curso_questoes <> 24 then raise exception 'POSCOND: curso_questoes criadas=% esperado 24', v_curso_questoes - v_snap.total_curso_questoes; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='5cc30e49-890f-4b83-b96f-31724024ee24' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Reescrita de frases e textos=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='5cc30e49-890f-4b83-b96f-31724024ee24' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 1 then raise exception 'POSCOND: REAL unidade Reescrita de frases e textos=% esperado 1', v_real; end if;
  if v_autoral <> 9 then raise exception 'POSCOND: AUTORAL unidade Reescrita de frases e textos=% esperado 9', v_autoral; end if;
  ...
  if v_gabaritos <> 'BCBBCAAA' then raise exception 'POSCOND: gabaritos unidade Reescrita de frases e textos=% esperado BCBBCAAA', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='138aafa7-066c-40fd-9fa5-7f1b90406db2' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Interpretação de textos=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='138aafa7-066c-40fd-9fa5-7f1b90406db2' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 1 then raise exception 'POSCOND: REAL unidade Interpretação de textos=% esperado 1', v_real; end if;
  if v_autoral <> 9 then raise exception 'POSCOND: AUTORAL unidade Interpretação de textos=% esperado 9', v_autoral; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '138aafa7-066c-40fd-9fa5-7f1b90406db2';
  if v_gabaritos <> 'CCCBEAB' then raise exception 'POSCOND: gabaritos unidade Interpretação de textos=% esperado CCCBEAB', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='735f736a-37c0-477f-a555-dcd73d243d21' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 8 then raise exception 'POSCOND: uteis unidade Regência verbal e nominal=% esperado 8', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='735f736a-37c0-477f-a555-dcd73d243d21' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 1 then raise exception 'POSCOND: REAL unidade Regência verbal e nominal=% esperado 1', v_real; end if;
  if v_autoral <> 7 then raise exception 'POSCOND: AUTORAL unidade Regência verbal e nominal=% esperado 7', v_autoral; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '735f736a-37c0-477f-a555-dcd73d243d21';
  if v_gabaritos <> 'ACACD' then raise exception 'POSCOND: gabaritos unidade Regência verbal e nominal=% esperado ACACD', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 7 then raise exception 'POSCOND: uteis unidade Vozes verbais=% esperado 7', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 0 then raise exception 'POSCOND: REAL unidade Vozes verbais=% esperado 0', v_real; end if;
  if v_autoral <> 7 then raise exception 'POSCOND: AUTORAL unidade Vozes verbais=% esperado 7', v_autoral; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '1cb6bef3-1d8d-4cae-9ed9-eae9fe4d79b9';
  if v_gabaritos <> 'CBBD' then raise exception 'POSCOND: gabaritos unidade Vozes verbais=% esperado CBBD', v_gabaritos; end if;
  ...
  if v_vinc_ok <> 24 then raise exception 'POSCOND: %/24 vinculos corretos (unidade unica) confirmados', v_vinc_ok; end if;
  ...
  if v_cq_ok <> 24 then raise exception 'POSCOND: %/24 curso_questoes novas confirmadas', v_cq_ok; end if;

  raise notice 'POSCONDICOES OK: +24 questoes, +120 alternativas, +24 vinculos, +24 curso_questoes; todas as 4 unidades com gabaritos e contagens confirmadas';
end $$;
