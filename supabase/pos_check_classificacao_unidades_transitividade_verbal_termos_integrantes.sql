...
-- Transitividade Verbal e Termos Integrantes (curso_conteudos.id = 27)
...
-- 1) A unidade pedagogica do conteudo 27. Esperado: 1 linha, ordem 1,
...
-- 2) Contagem de questoes classificadas. Esperado: ordem 1 = 5.
...
-- 3) Total de vinculos e questoes distintas. Esperado: 5 / 5.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 27
  );
...
-- 7) Q332 permanece intacta (nenhuma alteracao de texto); Q34 mantem
--    proveniencia incompleta (pendencia registrada, nao saneada).
select
  (select length(enunciado) from public.questoes where id = 332) as q332_len_enunciado,
  (select concurso from public.questoes where id = 34) as q34_concurso,
  (select ano from public.questoes where id = 34) as q34_ano;

-- 8) Confirma artigos_esperados NULL e estado geral do sistema.
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 27) as artigos_esperados_null,
