-- Harness de teste (SEMPRE termina em ROLLBACK) da importacao do Lote 3 (final)
-- AUTORAL_PAPIRO de Raciocinio Logico — 29 questoes novas + 145 alternativas
-- + 29 vinculos.
--
-- Fonte da verdade: scripts/curadoria-pedagogica/relatorios/
-- pacote_importacao_rlm_lote3_alvo10.json (29 candidatas, status =
-- PRONTA_PARA_IMPORTAR). Origem: AUTORAL_PAPIRO (banca 'Papiro') em todas
-- as 29 — nenhuma REAL. Nenhuma questao para Negacao de proposicoes nem
-- Leis de De Morgan.
--
-- Gerado programaticamente a partir do pacote JSON (evita erro de
-- transcricao manual de apostrofos/aspas).
--
...
begin
  ...
  insert into _relatorio values ('staging_29_questoes', v_cnt = 29, format('staging tem %s questoes (esperado 29)', v_cnt));

  select count(*) into v_dup
  from _lote_questoes lq
  where exists (select 1 from public.questoes q where q.enunciado = lq.enunciado);
  insert into _relatorio values ('nenhuma_duplicata_exata', v_dup = 0, format('%s enunciado(s) identicos ja existem no banco', v_dup));

  select count(*) into v_dup
  from _lote_questoes lq
  where not exists (select 1 from public.unidades_pedagogicas u where u.id = lq.unidade_id and u.ativa);
  insert into _relatorio values ('unidades_alvo_existem_ativas', v_dup = 0, format('%s unidade(s)-alvo inexistente(s) ou inativa(s)', v_dup));
  ...
  insert into _relatorio values ('assunto_compativel_com_unidade', v_dup = 0, format('%s linha(s) com assunto_id divergente da unidade-alvo', v_dup));
end $$;
...
begin
  ...
  insert into _relatorio values ('29_questoes_novas_inseridas', v_novas_questoes = 29, format('questoes novas=%s (esperado 29)', v_novas_questoes));

  select count(*) into v_novas_alternativas from public.alternativas where questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('145_alternativas_inseridas', v_novas_alternativas = 145, format('alternativas novas=%s (esperado 145)', v_novas_alternativas));
  ...
  insert into _relatorio values ('exatamente_1_correta_por_questao', v_sem_1_correta = 0, format('%s questao(oes) sem exatamente 1 alternativa correta', v_sem_1_correta));
  ...
  insert into _relatorio values ('29_vinculos_criados', v_novos_vinculos = 29, format('vinculos novos=%s (esperado 29)', v_novos_vinculos));
  ...
  insert into _relatorio values ('zero_multiunidade', v_multiunidade = 0, format('%s questao(oes) com mais de 1 vinculo', v_multiunidade));

  select count(*) into v_nao_ativas from public.questoes where id in (select questao_id from _mapa_ids) and ativa <> true;
  insert into _relatorio values ('29_ativas', v_nao_ativas = 0, format('%s questao(oes) nao ativas', v_nao_ativas));
  ...
  insert into _relatorio values ('29_origem_autoral', v_nao_autoral = 0, format('%s questao(oes) com banca != papiro (esperado 0, todas autoral)', v_nao_autoral));
  ...
  insert into _relatorio values ('29_com_explicacao', v_sem_explicacao = 0, format('%s questao(oes) sem explicacao', v_sem_explicacao));
  ...
  insert into _relatorio values ('distribuicao_cc1', v_qtd_cc1 = 3, format('cc1: %s novos vinculos (esperado 3)', v_qtd_cc1));

  select count(*) into v_qtd_cc2
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 2 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc2', v_qtd_cc2 = 3, format('cc2: %s novos vinculos (esperado 3)', v_qtd_cc2));

  select count(*) into v_qtd_cc3
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 3 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc3', v_qtd_cc3 = 2, format('cc3: %s novos vinculos (esperado 2)', v_qtd_cc3));

  select count(*) into v_qtd_cc5
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 5 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc5', v_qtd_cc5 = 3, format('cc5: %s novos vinculos (esperado 3)', v_qtd_cc5));

  select count(*) into v_qtd_cc6
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 6 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc6', v_qtd_cc6 = 3, format('cc6: %s novos vinculos (esperado 3)', v_qtd_cc6));

  select count(*) into v_qtd_cc7
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 7 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc7', v_qtd_cc7 = 3, format('cc7: %s novos vinculos (esperado 3)', v_qtd_cc7));

  select count(*) into v_qtd_cc8
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 8 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc8', v_qtd_cc8 = 3, format('cc8: %s novos vinculos (esperado 3)', v_qtd_cc8));

  select count(*) into v_qtd_cc9
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 9 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc9', v_qtd_cc9 = 3, format('cc9: %s novos vinculos (esperado 3)', v_qtd_cc9));

  select count(*) into v_qtd_cc10
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 10 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc10', v_qtd_cc10 = 3, format('cc10: %s novos vinculos (esperado 3)', v_qtd_cc10));

  select count(*) into v_qtd_cc11
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 11 and qup.questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('distribuicao_cc11', v_qtd_cc11 = 3, format('cc11: %s novos vinculos (esperado 3)', v_qtd_cc11));

  insert into _relatorio values ('total_questoes_cresceu_29',
    (select count(*) from public.questoes) = (select total_questoes from _snapshot_antes) + 29, 'contagem total de questoes deve crescer exatamente 29');
  insert into _relatorio values ('total_alternativas_cresceu_145',
    (select count(*) from public.alternativas) = (select total_alternativas from _snapshot_antes) + 145, 'contagem total de alternativas deve crescer exatamente 145');
  insert into _relatorio values ('total_vinculos_cresceu_29',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 29, 'contagem total de vinculos deve crescer exatamente 29');
  ...
  insert into _relatorio values ('curso_conteudos_inalterados',
    (select count(*) from public.curso_conteudos) = (select total_conteudos from _snapshot_antes), 'nenhum curso_conteudo deve ser criado/alterado');
...
do $$
declare v_tudo_ok boolean; r record;
begin
  select bool_and(ok) into v_tudo_ok from _relatorio;
  raise notice '=== RELATORIO DO TESTE (importar_lote3_rlm_autoral) ===';
