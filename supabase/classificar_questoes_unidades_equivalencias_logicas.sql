-- Aplicação REAL do mapa aprovado em config/equivalencias_logicas.mapa.json,
...
--
-- Quarto conteúdo do bloco final de Raciocínio Lógico/Matemática, após
-- Tabela-verdade (ordem 85) e Leis de De Morgan (ordem 86, encerrada
-- sem corpus próprio por sobreposição curricular com Negação de
-- proposições). artigos_esperados desta unidade é NULL (metodologia
-- não jurídica). 0 exclusões — as 3 questões ativas foram classificadas
-- (2 rows REAL Fundatec + 1 AUTORAL Q310).
--
-- AUDITORIA TAXONÔMICA (executada e aprovada antes deste apply):
-- confirmou identidade pedagógica própria de Equivalências lógicas,
-- distinta de Tabela-verdade (Q79 compara fórmulas contra um
-- comportamento-alvo, não constrói/completa a tabela de uma única
-- expressão), de Negação de proposições (nenhuma candidata pede
-- "negue isto") e da futura Condicional e contrapositiva (Q82/Q310
-- testam a equivalência DISJUNTIVA P→Q≡¬P∨Q, diferente da
-- CONTRAPOSITIVA P→Q≡¬Q→¬P). Microauditoria externa encontrou Q80 e
-- Q309 (assunto_id 39, Condicional e contrapositiva) contendo a
-- palavra "equivalente", mas confirmou que testam a contrapositiva —
-- corretamente classificadas lá. Nenhuma dessas 2 questões externas é
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_equivalencias_logicas.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- 56df08f8-0f22-48c1-a64d-df11ebfc5ae9 — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (79, '56df08f8-0f22-48c1-a64d-df11ebfc5ae9'::uuid, 1, 'alta'),
  (82, '56df08f8-0f22-48c1-a64d-df11ebfc5ae9'::uuid, 1, 'alta'),
  (310, '56df08f8-0f22-48c1-a64d-df11ebfc5ae9'::uuid, 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 5 nao existe';
  end if;
  if v_materia_id is distinct from 18 or v_assunto_id is distinct from 41 then
    raise exception 'Precondicao falhou: conteudo 5 materia_id=% assunto_id=% (esperado 18/41)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '56df08f8-0f22-48c1-a64d-df11ebfc5ae9' and curso_conteudo_id = 5 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_equivalencias_logicas.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 5) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 5 — decisao aprovada foi manter unidade unica';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 5
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  -- Q80/Q309 (candidatas externas descartadas pela microauditoria) nao
  -- devem ter sido tocadas: continuam sem vinculo neste conteudo (nao
  -- fazem parte do mapa) e mantem seu assunto_id original (39).
  if exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 5 and qup.questao_id in (80, 309)
  ) then
    raise exception 'Precondicao falhou: alguma das questoes externas (80/309) ja possui vinculo neste conteudo — nao deveria';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 18 and q.assunto_id = 41);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=18/assunto_id=41', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 5);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 5', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  -- Pos-condicao extra: as 2 questoes externas (Q80/Q309) continuam sem
  -- vinculo neste conteudo e com seu assunto_id original intacto (39) —
  -- nenhuma reclassificacao foi feita por este arquivo.
  select count(*) into v_externas_vinculos
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 5 and qup.questao_id in (80, 309);
  if v_externas_vinculos <> 0 then
    raise exception 'Pos-condicao falhou: questoes externas (80/309) ganharam vinculo neste conteudo (%), nao deveriam', v_externas_vinculos;
  end if;
  ...
  if v_assunto_80 is distinct from 39 or v_assunto_309 is distinct from 39 then
    raise exception 'Pos-condicao falhou: assunto_id de questao externa foi alterado indevidamente (80=%,309=%)', v_assunto_80, v_assunto_309;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 5) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 5 != 1';
  end if;
  ...
  where id = '56df08f8-0f22-48c1-a64d-df11ebfc5ae9';
  ...
  raise notice 'Pos-condicoes OK: 3 questoes classificadas / 3 vinculos / 0 multiunidade / artigos_esperados NULL / questoes externas (80/309) intactas e sem vinculo neste conteudo.';
end $$;
