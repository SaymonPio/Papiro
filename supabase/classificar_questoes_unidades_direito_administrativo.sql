...
-- supabase/mapa_classificacao_direito_administrativo.sql, validado pelo harness
-- classificar_questoes_unidades_direito_administrativo_teste_rollback.sql (tudo_ok =
...
-- mesmo padrão de supabase/classificar_questoes_unidades_atos_administrativos.sql.
--
-- NOTA: Q197 (autotutela) e Q721/Q774/Q796 (CF art. 37) representam
-- sobreposicoes tematicas conhecidas com os curso_conteudo_id 54 (Atos
-- administrativos) e 57 (Constituicao Federal de 1988) — nao sao questoes
-- duplicadas, mantidas neste conteudo por decisao explicita do usuario.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_direito_administrativo.sql precisa já
-- ter sido aplicado.
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (195, '1172a885-1419-4ad8-b728-1a0c7492c133'::uuid, 1, 'alta'),
  (196, '1172a885-1419-4ad8-b728-1a0c7492c133', 1, 'alta'),
  (197, '1172a885-1419-4ad8-b728-1a0c7492c133', 1, 'alta'),
  (721, '1172a885-1419-4ad8-b728-1a0c7492c133', 1, 'alta'),
  (722, '1172a885-1419-4ad8-b728-1a0c7492c133', 1, 'alta'),
  (774, '1172a885-1419-4ad8-b728-1a0c7492c133', 1, 'alta'),
  (796, '1172a885-1419-4ad8-b728-1a0c7492c133', 1, 'alta'),
  (845, '1172a885-1419-4ad8-b728-1a0c7492c133', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 58
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 58 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 65 then
    raise exception 'Precondicao falhou: conteudo 58 materia_id=% assunto_id=% (esperado 10/65)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 58) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 58 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '1172a885-1419-4ad8-b728-1a0c7492c133' and curso_conteudo_id = 58 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_direito_administrativo.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 58
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
    and q.materia_id = cm.materia_id
    and (cc.assunto_id is null or q.assunto_id = cc.assunto_id);

  if v_total_candidatas <> 8 then
    raise exception 'Precondicao falhou: total de candidatas ativas = % (esperado 8)', v_total_candidatas;
  end if;

  select count(*) into v_classificacoes_previas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 58
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 65);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=65', v_invalidas;
  end if;

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  if v_distintas <> 8 then
    raise exception 'Mapa invalido: cobre % questoes distintas (esperado 8)', v_distintas;
  end if;
  ...
  if v_fora_do_candidato <> 0 then
    raise exception 'Mapa invalido: % linha(s) fora do conjunto candidato de 8 ativas', v_fora_do_candidato;
  end if;

  select count(*) into v_unidade_fora
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 58);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 58', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 58;
  if v_total_vinculos <> 8 then
    raise exception 'Pos-condicao falhou: total_vinculos=% (esperado 8)', v_total_vinculos;
  end if;
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 58;
  if v_questoes_classificadas <> 8 then
    raise exception 'Pos-condicao falhou: questoes_classificadas=% (esperado 8)', v_questoes_classificadas;
  end if;
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 58) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 58 != 1';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 8 questoes / 8 vinculos / 0 multiunidade.';
end $$;
