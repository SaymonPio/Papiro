...
begin
  if not exists (
    select 1 from public.questoes where id = 674 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = '43bc2941a2f90cdc8b367ce373cd9056'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = 'c30e2067e61681de0e2aced6c8849064'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 740 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = '19d35b3cfbf0ec998b284a4df180eea1'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = '93b438b82f833e7d6f6d2aef0e7c0a59'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 741 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = '7c92e2d0d14b62bbed633639af38f0eb'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = 'feba0e5fa410b8bafe1d50221caa5ef8'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 742 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = 'd98c2e3bd0f484c8b634a2c59497d6be'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = 'c97417868354b40990d0d9e44cde5fa1'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 781 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = '6338935c9b900249f8fc5d02cad5a365'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = '87796b0a55e5196be4b20db9210252c3'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 782 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = '47fdbe2c41be9528d25aa221bb384e69'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = '3398270d73eed339110948e382c689f7'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 783 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = 'ad3f1463a6afc418adc6d4cc2380dab7'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = 'c4a85a02a5e327f472415ddca5267cb7'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 803 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = '667900455c55fa7d518a679c95551cbd'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = '3489cefc445861e55d3496faa0f1c2a9'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 804 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = 'a69eed5a0c994fce34b750d34b3085a0'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = 'fc68aebe7b690a40dc583a12ee23eca0'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 867 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = 'ff609499af6e528b07fa63b1d9ae2fbc'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = '6b8b43a471b1673ce1ed0cb9ac601ede'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 868 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = 'd2ad721021eb598ec70b86d5fa6c055f'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = '50e943be8bc8a9796c5861244b386c08'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 869 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = '7f498bfc1b660a30f7ad70d582bf7029'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = '636bc42deb0765bf529b79e23756396e'
  ) then
  ...
  if not exists (
    select 1 from public.questoes where id = 870 and ativa = true
      and md5(enunciado || '|' || coalesce(fonte,'') || '|' || coalesce(banca,'') || '|' || coalesce(concurso,'') || '|' || materia_id::text || '|' || coalesce(assunto_id::text,'') || '|' || ativa::text) = 'e783f7f2309f5daf2422847c244a1b4f'
      and md5(regexp_replace(explicacao, E'\r\n', E'\n', 'g')) = '2bafe200c95b07d24c13f8dd6f65f43d'
  ) then
...
begin
  update public.alternativas set texto = 'Nos crimes de extorsão mediante sequestro, o membro do Ministério Público ou o delegado de polícia poderá requisitar, de quaisquer órgãos do Poder Público ou de empresas da iniciativa privada, dados e informações cadastrais da vítima ou de suspeitos.' where id = 3347 and questao_id = 674;
  get diagnostics v_linhas = row_count;
  if v_linhas <> 1 then raise exception 'UPDATE alternativa 3347 afetou % linhas', v_linhas; end if;

  update public.alternativas set texto = 'O inquérito policial ou outro procedimento previsto em lei em curso somente poderá ser avocado ou redistribuído por superior hierárquico, mediante despacho fundamentado, por motivo de interesse público ou nas hipóteses de inobservância dos procedimentos previstos em regulamento da corporação que prejudique a eficácia da investigação.' where id = 3349 and questao_id = 674;
  get diagnostics v_linhas = row_count;
  if v_linhas <> 1 then raise exception 'UPDATE alternativa 3349 afetou % linhas', v_linhas; end if;

  update public.alternativas set texto = 'Conforme jurisprudência do Supremo Tribunal Federal, é possível a deflagração da persecução penal pela chamada denúncia anônima, desde que esta seja seguida de diligências realizadas para averiguar os fatos nela noticiados antes da instauração do inquérito policial.' where id = 3350 and questao_id = 674;
  get diagnostics v_linhas = row_count;
  if v_linhas <> 1 then raise exception 'UPDATE alternativa 3350 afetou % linhas', v_linhas; end if;

  update public.alternativas set texto = 'O juiz formará sua convicção pela livre apreciação da prova produzida em contraditório judicial, não podendo fundamentar sua decisão exclusivamente nos elementos informativos colhidos na investigação, ressalvadas as provas cautelares, não repetíveis e antecipadas.' where id = 3351 and questao_id = 674;
  get diagnostics v_linhas = row_count;
...
begin
  ...
  set enunciado = 'Durante sua atividade de proteção ao patrimônio em um parque municipal com grande área de vegetação, um guarda municipal encontrou um canteiro, de aproximadamente 30 m², em meio às árvores, com uma plantação de maconha. A pessoa responsável pela plantação informou que dependia da planta para fins medicinais e que, além disso, usava a plantação como meio para obter recursos financeiros para sua sobrevivência. Considerando o quadro apresentado e a legislação vigente, é correto afirmar que:', explicacao = 'GABARITO: alternativa D
...
No caso da questão, o cultivo não autorizado em parque público associado à venda para obtenção de renda não se enquadra nessas ressalvas.', atualizado_em = now()
  where id = 740;
  get diagnostics v_linhas = row_count;
...
declare v_linhas int;
begin
  update public.questoes set enunciado = 'Conforme a Lei nº 11.343/2006, para atender à finalidade de articular, integrar, organizar e coordenar as atividades relacionadas com a prevenção do uso indevido, a atenção e a reinserção social de usuários e dependentes de drogas, a repressão da produção não autorizada e do tráfico ilícito de drogas, o Sistema Nacional de Políticas Públicas sobre Drogas (Sisnad) também atua em articulação com os seguintes sistemas de políticas públicas:', atualizado_em = now() where id = 741;
  get diagnostics v_linhas = row_count;
...
begin
  update public.questoes
  set enunciado = 'Segundo a Lei de Drogas (Lei nº 11.343/2006) e seus crimes, analise as assertivas abaixo: I. O crime de tráfico privilegiado (art. 33, § 4º, da referida Lei) permite a redução da pena de um a dois terços quando o agente for primário, possuir bons antecedentes, não se dedicar a atividades criminosas nem integrar organização criminosa, constituindo direito subjetivo do réu que preencha tais requisitos. II. O usuário de drogas que adquire, guarda, tem em depósito, transporta ou traz consigo drogas para consumo pessoal pratica crime punível com pena privativa de liberdade, sendo cabível a prisão em flagrante e a posterior conversão em penas alternativas. III. O crime de associação para o tráfico (art. 35 da referida Lei) exige a reunião de duas ou mais pessoas com o fim de praticar, reiteradamente ou não, qualquer dos crimes previstos nos arts. 33 a 37 da Lei, configurando-se como crime formal que se consuma independentemente da prática efetiva do tráfico. IV. A Lei de Drogas estabelece que não há crime quando o agente planta, cultiva ou colhe plantas destinadas à preparação de pequena quantidade de droga para consumo pessoal, aplicando-se as mesmas penas previstas para o usuário. Quais estão INCORRETAS?', explicacao = 'GABARITO: alternativa E
...
4. Cultivo Pessoal (Art. 28, §1º): Submete-se às medidas do art. 28.', atualizado_em = now()
  where id = 742;
  get diagnostics v_linhas = row_count;
...
- Atualização STF (Tema 506): Para a cannabis/maconha de uso próprio, a advertência e o comparecimento a curso educativo são medidas de natureza extrapenal/administrativa, sem efeitos criminais.', atualizado_em = now()
  where id = 781;
  get diagnostics v_linhas = row_count;
...
declare v_linhas int;
begin
  update public.questoes set enunciado = 'A Lei nº 11.343/2006 prescreve medidas para prevenção do uso indevido, atenção e reinserção social de usuários e dependentes de drogas, estabelece normas para repressão à produção não autorizada e ao tráfico ilícito de drogas, define crimes e dá outras providências. Essa Lei instituiu o(a):', atualizado_em = now() where id = 782;
  get diagnostics v_linhas = row_count;
...
- Atualização STF (Cannabis): Ilícito administrativo punível com Advertência (I) e Curso educativo (III). Prestação de serviços (II) afastada para uso pessoal de maconha. 40g é presunção relativa.', atualizado_em = now()
  where id = 783;
  get diagnostics v_linhas = row_count;
...
declare v_linhas int;
begin
  update public.questoes set enunciado = 'Qual a base legal, além do Código Penal Brasileiro, para a atuação do Guarda Municipal que constatar que uma pessoa está vendendo substância tóxica entorpecente para outras pessoas em uma praça municipal?', atualizado_em = now() where id = 804;
  get diagnostics v_linhas = row_count;
...
declare v_linhas int;
begin
  update public.questoes set enunciado = 'Com base na Lei de Drogas, Lei nº 11.343/2006, analise as assertivas abaixo, assinalando V, se verdadeiras, ou F, se falsas. Quem oferece droga, eventualmente e sem objetivo de lucro, à pessoa de seu relacionamento, para juntos a consumirem: ( ) Não pratica crime por falta de previsão legal. ( ) Pratica crime previsto na Lei de Drogas. ( ) Pode ser penalizado com detenção. A ordem correta de preenchimento dos parênteses, de cima para baixo, é:', atualizado_em = now() where id = 868;
  get diagnostics v_linhas = row_count;
...
declare v_linhas int;
begin
  update public.questoes set enunciado = 'Nos termos da Lei nº 11.343/2006, quem adquirir, guardar, tiver em depósito, transportar ou trouxer consigo, para consumo pessoal, drogas sem autorização ou em desacordo com determinação legal ou regulamentar será submetido às seguintes penas: I. Advertência sobre os efeitos das drogas. II. Medida educativa de comparecimento a programa ou curso educativo. III. Lavratura de prisão em flagrante. Quais estão corretas?', atualizado_em = now() where id = 869;
  get diagnostics v_linhas = row_count;
...
declare v_linhas int;
begin
  update public.alternativas set texto = 'Todas as internações e altas de que trata esta lei deverão ser informadas, em, no máximo, de 72 horas, ao Ministério Público, à Defensoria Pública e a outros órgãos de fiscalização, por meio de sistema informatizado único.' where id = 4318 and questao_id = 870;
  get diagnostics v_linhas = row_count;
  if v_linhas <> 1 then raise exception 'UPDATE alternativa 4318 afetou % linhas', v_linhas; end if;

  update public.alternativas set texto = 'A internação involuntária perdurará apenas pelo tempo necessário à desintoxicação, no prazo máximo de 90 dias, tendo seu término determinado pelo médico responsável.' where id = 4320 and questao_id = 870;
  get diagnostics v_linhas = row_count;
...
begin
  ...
  select '740: explicacao contem art. 2º caput e paragrafo unico, sem ilicitude absoluta',
    ...
    = regexp_replace('GABARITO: alternativa D
...
Ressalvas: a própria lei admite hipótese de autorização legal ou regulamentar e preserva a ressalva ritualístico-religiosa mencionada no caput.
Além disso, a União pode autorizar plantio, cultura e colheita exclusivamente para fins medicinais ou científicos, em local e prazo predeterminados e mediante fiscalização.
No caso da questão, o cultivo não autorizado em parque público associado à venda para obtenção de renda não se enquadra nessas ressalvas.', E'\r\n', E'\n', 'g')
...
2. Usuário (Art. 28 e Art. 48, §2º): Não há pena de prisão nem flagrante.
3. Associação (Art. 35): Exige vínculo estável e permanente para os crimes dos arts. 33, caput/§1º e 34.
4. Cultivo Pessoal (Art. 28, §1º): Submete-se às medidas do art. 28.', E'\r\n', E'\n', 'g')
...
Usuário de Drogas e Maconha (Art. 28 da Lei 11.343/2006 e Tema 506/STF):
- Medida legal aplicável: Advertência sobre os efeitos das drogas (Art. 28, I).
- Atualização STF (Tema 506): Para a cannabis/maconha de uso próprio, a advertência e o comparecimento a curso educativo são medidas de natureza extrapenal/administrativa, sem efeitos criminais.', E'\r\n', E'\n', 'g')
...
Artigo 28 da Lei nº 11.343/2006 vs. Tema 506/STF:
- Texto da Lei: Advertência (I), Prestação de serviços à comunidade (II) e Curso educativo (III).
- Atualização STF (Cannabis): Ilícito administrativo punível com Advertência (I) e Curso educativo (III). Prestação de serviços (II) afastada para uso pessoal de maconha. 40g é presunção relativa.', E'\r\n', E'\n', 'g')
