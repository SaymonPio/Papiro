...
-- questoes de Responsabilidade civil do Estado (curso_conteudos.id = 61)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/responsabilidade_civil_do_estado.unidades.json + config/responsabilidade_civil_do_estado.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_responsabilidade_civil_do_estado.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_responsabilidade_civil_do_estado.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (213, 'd6f03d69-8943-4867-a2af-e37482d4ca99', 1, 'alta'),
  (214, 'd6f03d69-8943-4867-a2af-e37482d4ca99', 1, 'alta'),
  (215, 'd6f03d69-8943-4867-a2af-e37482d4ca99', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_61_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 76,
    format('materia_id=%s assunto_id=%s (esperado 10/76)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'd6f03d69-8943-4867-a2af-e37482d4ca99' and curso_conteudo_id = 61 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 61 — depende de curadoria_unidades_responsabilidade_civil_do_estado.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 61) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 61 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 61)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 61
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 76);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=76', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 61);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 61', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'd6f03d69-8943-4867-a2af-e37482d4ca99';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_responsabilidade_civil_do_estado) ===';
