...
-- questoes de Constituição do Estado do Rio Grande do Sul (curso_conteudos.id = 50)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/constituicao_do_estado_do_rio_grande_do_sul.unidades.json + config/constituicao_do_estado_do_rio_grande_do_sul.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_constituicao_do_estado_do_rio_grande_do_sul.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_constituicao_do_estado_do_rio_grande_do_sul.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (39, '83636594-c69f-4de0-bf46-0e75c2ec981c', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_50_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 80,
    format('materia_id=%s assunto_id=%s (esperado 10/80)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '83636594-c69f-4de0-bf46-0e75c2ec981c' and curso_conteudo_id = 50 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 50 — depende de curadoria_unidades_constituicao_do_estado_do_rio_grande_do_sul.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 50) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 50 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 50)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=80', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 50', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '83636594-c69f-4de0-bf46-0e75c2ec981c';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_constituicao_do_estado_do_rio_grande_do_sul) ===';
