...
-- supabase/mapa_classificacao_direito_a_cidadania_na_constituicao_federal.sql,
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_direitos_fundamentais.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_direito_a_cidadania_na_constituicao_federal.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (159, 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7'::uuid, 1, 'alta'),
  (160, 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7', 1, 'alta'),
  (161, 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7', 1, 'media'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 95 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 100 then
    raise exception 'Precondicao falhou: conteudo 95 materia_id=% assunto_id=% (esperado 11/100)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 95) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 95 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7' and curso_conteudo_id = 95 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_direito_a_cidadania_na_constituicao_federal.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=100', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 95', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 95) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 95 != 1';
  end if;
