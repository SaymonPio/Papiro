...
-- config/interpretacao_de_textos.mapa.json,
...
--
-- Décimo quarto conteúdo de Língua Portuguesa da fila. artigos_esperados
-- desta unidade é NULL (metodologia não jurídica). 0 exclusões
-- intencionais — todas as 3 questões ativas foram classificadas, após
-- saneamento de fidelidade de Q65 (commit ac67dd1, já concluído em
-- operação separada) e microauditoria global que confirmou ausência de
-- candidatas reais adicionais (GAP_DE_BANCO_INTERPRETACAO_DE_TEXTOS
-- quantitativo severo, não problema de taxonomia/duplicação/fidelidade).
--
-- Este arquivo NÃO altera enunciado/explicacao/alternativas/gabarito de
-- nenhuma questão — contém pós-condições extras para confirmar que o
-- saneamento de Q65 permanece intacto após o apply da classificação.
--
...
-- mesmo padrão de supabase/classificar_questoes_unidades_significacao_das_palavras.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_interpretacao_de_textos.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 138aafa7-066c-40fd-9fa5-7f1b90406db2 — não cria unidade
-- nova).
begin;
...
-- Snapshot do saneamento ja concluido (Q65), para confirmar nas
-- pos-condicoes que este apply nao o desfaz.
create temporary table _snapshot_saneamento_q65 on commit drop as
select
  (select enunciado from public.questoes where id = 65)  as q65_enunciado,
  (select explicacao from public.questoes where id = 65) as q65_explicacao;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (17, '138aafa7-066c-40fd-9fa5-7f1b90406db2'::uuid, 1, 'alta'),
  (65, '138aafa7-066c-40fd-9fa5-7f1b90406db2', 1, 'alta'),
  (306, '138aafa7-066c-40fd-9fa5-7f1b90406db2', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 12
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 12 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 15 then
    raise exception 'Precondicao falhou: conteudo 12 materia_id=% assunto_id=% (esperado 6/15)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 12) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 12 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '138aafa7-066c-40fd-9fa5-7f1b90406db2' and curso_conteudo_id = 12 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_interpretacao_de_textos.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 12
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  -- Q65 precisa estar com o saneamento de fidelidade ja aplicado.
  if position('[01]' in (select enunciado from public.questoes where id = 65)) = 0
     or position('[32]' in (select enunciado from public.questoes where id = 65)) = 0 then
    raise exception 'Precondicao falhou: saneamento da questao 65 (marcadores [01]/[32]) nao esta presente antes do apply';
  end if;
  if exists (select 1 from public.questao_unidades_pedagogicas where questao_id = 65) then
    raise exception 'Precondicao falhou: questao 65 ja possui vinculo pedagogico inesperado antes do apply';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 15);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=15', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 12);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 12', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 12) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 12 != 1';
  end if;
  ...
  where id = '138aafa7-066c-40fd-9fa5-7f1b90406db2';
  ...
  -- Pos-condicao extra: Q65 vinculada exatamente uma vez, e o saneamento
  -- (ac67dd1) permanece byte a byte identico ao snapshot antes deste apply.
  select count(*) into v_q65_vinculos from public.questao_unidades_pedagogicas where questao_id = 65;
  if v_q65_vinculos <> 1 then
    raise exception 'Pos-condicao falhou: questao 65 possui % vinculo(s) (esperado exatamente 1)', v_q65_vinculos;
  end if;
  if (select enunciado from public.questoes where id = 65) is distinct from (select q65_enunciado from _snapshot_saneamento_q65) then
    raise exception 'Pos-condicao falhou: enunciado da questao 65 foi alterado por este apply (saneamento ac67dd1 desfeito)';
  end if;
  if (select explicacao from public.questoes where id = 65) is distinct from (select q65_explicacao from _snapshot_saneamento_q65) then
    raise exception 'Pos-condicao falhou: explicacao da questao 65 foi alterada por este apply (saneamento ac67dd1 desfeito)';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 3 questoes classificadas / 3 vinculos / 0 multiunidade / artigos_esperados NULL / questao 65 vinculada exatamente 1 vez / saneamento de Q65 preservado intacto.';
end $$;
