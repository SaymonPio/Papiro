...
-- supabase/mapa_classificacao_estatuto_dos_militares_estaduais.sql,
...
-- mesmo padrão de supabase/classificar_questoes_unidades_defesa_do_estado_e_das_instituicoes_democraticas.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_estatuto_dos_militares_estaduais.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (20, 'bf13f365-3dd9-4d22-9ad7-f369a298eb19'::uuid, 1, 'alta'),
  (53, 'bf13f365-3dd9-4d22-9ad7-f369a298eb19', 1, 'alta'),
  (300, 'bf13f365-3dd9-4d22-9ad7-f369a298eb19', 1, 'alta'),
  (362, 'bf13f365-3dd9-4d22-9ad7-f369a298eb19', 1, 'alta'),
  (363, 'bf13f365-3dd9-4d22-9ad7-f369a298eb19', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 51
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 51 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 18 then
    raise exception 'Precondicao falhou: conteudo 51 materia_id=% assunto_id=% (esperado 10/18)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 51) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 51 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'bf13f365-3dd9-4d22-9ad7-f369a298eb19' and curso_conteudo_id = 51 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_estatuto_dos_militares_estaduais.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 51
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 18);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=18', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 51);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 51', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 51) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 51 != 1';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 5 questoes / 5 vinculos / 0 multiunidade.';
end $$;
