...
-- questoes de Coordenação e subordinação (curso_conteudos.id = 28)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/coordenacao_e_subordinacao.unidades.json + config/coordenacao_e_subordinacao.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_coordenacao_e_subordinacao.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_coordenacao_e_subordinacao.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (225, 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7', 1, 'alta'),
  (226, 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7', 1, 'alta'),
  (227, 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_28_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 50,
    format('materia_id=%s assunto_id=%s (esperado 6/50)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7' and curso_conteudo_id = 28 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 28 — depende de curadoria_unidades_coordenacao_e_subordinacao.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 28) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 28 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 28)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 28
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 50);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=50', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 28);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 28', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'bfeaf283-09fc-4f14-9d0c-41ff0db4eab7';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_coordenacao_e_subordinacao) ===';
