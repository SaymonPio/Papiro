...
-- questoes de Protocolo de Assunção sobre Direitos Humanos no Mercosul (curso_conteudos.id = 92)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul.unidades.json + config/protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (180, '655700d6-b585-468f-8d98-8143090cbafb', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_92_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 93,
    format('materia_id=%s assunto_id=%s (esperado 11/93)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '655700d6-b585-468f-8d98-8143090cbafb' and curso_conteudo_id = 92 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 92 — depende de curadoria_unidades_protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 92) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 92 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 92)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=93', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 92', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '655700d6-b585-468f-8d98-8143090cbafb';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul) ===';
