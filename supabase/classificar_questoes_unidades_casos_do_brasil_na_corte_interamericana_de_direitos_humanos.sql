...
-- supabase/mapa_classificacao_casos_do_brasil_na_corte_interamericana_de_direitos_humanos.sql,
...
-- conteudo ja concluido 88 (Corte Interamericana de Direitos Humanos —
-- estrutura institucional da Corte). Este conteudo 90 trata de casos
-- concretos do Brasil perante a Corte. Nenhum deles e tocado por este
-- arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_convencao_de_belem_do_para.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_casos_do_brasil_na_corte_interamericana_de_direitos_humanos.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 420c8c2f-6f80-422a-9e63-d64aace51465 — não cria unidade nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (147, '420c8c2f-6f80-422a-9e63-d64aace51465'::uuid, 1, 'alta'),
  (148, '420c8c2f-6f80-422a-9e63-d64aace51465', 1, 'alta'),
  (149, '420c8c2f-6f80-422a-9e63-d64aace51465', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 90
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 90 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 97 then
    raise exception 'Precondicao falhou: conteudo 90 materia_id=% assunto_id=% (esperado 11/97)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 90) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 90 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '420c8c2f-6f80-422a-9e63-d64aace51465' and curso_conteudo_id = 90 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_casos_do_brasil_na_corte_interamericana_de_direitos_humanos.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 90
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 88) then
    raise exception 'Precondicao falhou: curso_conteudos 88 (sobreposicao tematica Corte Interamericana) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 97);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=97', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 90);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 90', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 90) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 90 != 1';
  end if;
  ...
  -- Pos-condicao extra: artigos_esperados deve permanecer NULL (lista vazia aprovada).
  select artigos_esperados into v_artigos_esperados
  from public.unidades_pedagogicas
  where id = '420c8c2f-6f80-422a-9e63-d64aace51465';
  if v_artigos_esperados is not null then
    raise exception 'Pos-condicao falhou: artigos_esperados=% (esperado NULL — lista vazia aprovada, conteudo genuinamente jurisprudencial)', v_artigos_esperados;
  end if;

  -- Pos-condicao extra: conteudo 88 (sobreposicao tematica) permanece intocado.
  ...
  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / artigos_esperados NULL / conteudo 88 intocado.';
end $$;
