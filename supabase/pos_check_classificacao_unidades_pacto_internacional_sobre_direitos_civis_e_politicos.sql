...
-- Internacional sobre Direitos Civis e Politicos - PIDCP
-- (curso_conteudos.id = 74) — gerado seguindo o mesmo template de
...
--
-- NOTA: PIDCP (Decreto no 592/1992). Art. 11 mapeado com precisao
-- ("vedacao a prisao fundada exclusivamente na incapacidade de cumprir
-- obrigacao contratual", nao "descumprimento de obrigacao contratual"
-- em sentido generico). Art. 7o nao ampliado para nao derrogabilidade
-- (art. 4o, item 2, nao testado, nao entra em artigos_esperados).

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 74, com
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 74
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 74 aponta para questao
...
--    8d4e4b20-37ac-4df0-a7ac-57cb016c44d6): curso_conteudos=93,
...
-- 8) Confirma exatamente os 5 artigos_esperados aprovados.
--    Esperado: total_artigos_esperados=5, todos os campos true.
select
  (artigos_esperados @> array['art. 6º, item 1']::text[]) as contem_art6_item1,
  (artigos_esperados @> array['art. 7º']::text[]) as contem_art7,
  (artigos_esperados @> array['art. 9º, item 5']::text[]) as contem_art9_item5,
  (artigos_esperados @> array['art. 10, item 1']::text[]) as contem_art10_item1,
