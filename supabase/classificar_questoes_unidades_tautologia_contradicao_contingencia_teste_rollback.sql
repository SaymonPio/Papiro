...
-- questoes de Tautologia, contradição e contingência (curso_conteudos.id = 8)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/tautologia_contradicao_contingencia.unidades.json + config/tautologia_contradicao_contingencia.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_tautologia_contradicao_contingencia.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_tautologia_contradicao_contingencia.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (76, 'ae60f2db-49d0-4326-980c-df1617a0bc35', 1, 'alta'),
  (85, 'ae60f2db-49d0-4326-980c-df1617a0bc35', 1, 'alta'),
  (315, 'ae60f2db-49d0-4326-980c-df1617a0bc35', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_8_materia_assunto',
    v_materia_id is not distinct from 18 and v_assunto_id is not distinct from 37,
    format('materia_id=%s assunto_id=%s (esperado 18/37)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'ae60f2db-49d0-4326-980c-df1617a0bc35' and curso_conteudo_id = 8 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 8 — depende de curadoria_unidades_tautologia_contradicao_contingencia.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 8) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 8 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 8)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=18/assunto_id=37', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 8', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'ae60f2db-49d0-4326-980c-df1617a0bc35';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_tautologia_contradicao_contingencia) ===';
