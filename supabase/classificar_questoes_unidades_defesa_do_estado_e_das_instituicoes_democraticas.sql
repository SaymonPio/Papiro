...
-- supabase/mapa_classificacao_defesa_do_estado_e_das_instituicoes_democraticas.sql,
...
-- mesmo padrão de supabase/classificar_questoes_unidades_regulamento_disciplinar_da_brigada_militar.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_defesa_do_estado_e_das_instituicoes_democraticas.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (47, 'ecdb1d62-ef44-4407-b899-85911402bc90'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 48 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 74 then
    raise exception 'Precondicao falhou: conteudo 48 materia_id=% assunto_id=% (esperado 10/74)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 48) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 48 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'ecdb1d62-ef44-4407-b899-85911402bc90' and curso_conteudo_id = 48 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_defesa_do_estado_e_das_instituicoes_democraticas.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=74', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 48', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 48) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 48 != 1';
  end if;
