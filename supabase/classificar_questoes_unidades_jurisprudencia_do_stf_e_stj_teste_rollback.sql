...
-- questoes de Jurisprudência do STF e STJ (curso_conteudos.id = 69)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/jurisprudencia_do_stf_e_stj.unidades.json + config/jurisprudencia_do_stf_e_stj.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_jurisprudencia_do_stf_e_stj.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_jurisprudencia_do_stf_e_stj.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (198, '8e1d1204-1f66-431c-a95f-6403e77882dc', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_69_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 66,
    format('materia_id=%s assunto_id=%s (esperado 10/66)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '8e1d1204-1f66-431c-a95f-6403e77882dc' and curso_conteudo_id = 69 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 69 — depende de curadoria_unidades_jurisprudencia_do_stf_e_stj.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 69) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 69 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 69)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=66', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 69', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '8e1d1204-1f66-431c-a95f-6403e77882dc';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_jurisprudencia_do_stf_e_stj) ===';
