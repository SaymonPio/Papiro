...
-- questoes de Transitividade Verbal e Termos Integrantes (Objeto Direto e Indireto, Complemento Nominal e Agente da Passiva) (curso_conteudos.id = 27)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/transitividade_verbal_termos_integrantes.unidades.json + config/transitividade_verbal_termos_integrantes.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_transitividade_verbal_termos_integrantes.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_transitividade_verbal_termos_integrantes.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (34, '981e5d2c-3b59-48a0-a699-a53c03e500ee', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_27_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 30,
    format('materia_id=%s assunto_id=%s (esperado 6/30)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '981e5d2c-3b59-48a0-a699-a53c03e500ee' and curso_conteudo_id = 27 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 27 — depende de curadoria_unidades_transitividade_verbal_termos_integrantes.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 27) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 27 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 27)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=30', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 27', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '981e5d2c-3b59-48a0-a699-a53c03e500ee';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_transitividade_verbal_termos_integrantes) ===';
