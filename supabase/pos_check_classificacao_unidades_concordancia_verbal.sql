...
-- Concordancia verbal (curso_conteudos.id = 18) — gerado seguindo o
...
-- 1) A unidade pedagogica do conteudo 18. Esperado: 1 linha, ordem 1,
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 18
  );
...
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 18) as artigos_esperados_null,
