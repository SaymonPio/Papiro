...
-- questoes de Tratados de Direitos Humanos com força de Emenda Constitucional (curso_conteudos.id = 78)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/tratados_de_direitos_humanos_com_forca_de_emenda_constitucional.unidades.json + config/tratados_de_direitos_humanos_com_forca_de_emenda_constitucional.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_tratados_de_direitos_humanos_com_forca_de_emenda_constitucional.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_tratados_de_direitos_humanos_com_forca_de_emenda_constitucional.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (127, 'e996e440-4508-4878-8acd-c805b718b27c', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_78_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 99,
    format('materia_id=%s assunto_id=%s (esperado 11/99)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'e996e440-4508-4878-8acd-c805b718b27c' and curso_conteudo_id = 78 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 78 — depende de curadoria_unidades_tratados_de_direitos_humanos_com_forca_de_emenda_constitucional.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 78) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 78 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 78)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=99', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_3_distintas',
    v_distintas = 3,
    format('mapa cobre %s questoes distintas (esperado 3)', v_distintas)
  );

  select exists (select 1 from _mapa where questao_id in (351)) into v_contem_excluida;
  insert into _relatorio values (
    'mapa_nao_contem_excluidas',
    not coalesce(v_contem_excluida, false),
    'as questoes excluidas intencionalmente (351) nao devem constar no mapa'
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 78', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 78;
  insert into _relatorio values ('total_vinculos_3', v_total_vinculos = 3, format('total_vinculos=%s (esperado 3)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 78;
  insert into _relatorio values ('questoes_classificadas_3', v_questoes_classificadas = 3, format('questoes_classificadas=%s (esperado 3)', v_questoes_classificadas));
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'e996e440-4508-4878-8acd-c805b718b27c';
  insert into _relatorio values ('u1_3_questoes', v_qtd_tmp = 3, format('u1_questoes=%s (esperado 3)', v_qtd_tmp));
  ...
  insert into _relatorio values ('351_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 351 (Materialmente aderente ao conteudo substantivo da Convencao sobre os Direitos das Pessoas com Deficiencia (CDPD) — testa art. 2, art. 3, art. 5 item 4 e art. 7 item 3 da Convencao, nao o rito do art. 5o, SS3o da CF nem a distincao entre status de emenda constitucional e status supralegal, que sao o objeto real deste curso_conteudo_id. Destino pedagogico provavel: curso_conteudo_id 71 (Pessoa com Deficiencia), ja concluido — nao reaberto/alterado nesta curadoria. Questao permanece ativa e intacta.) nao deve ter sido classificada por este arquivo');
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_3',
    ...
    'total de vinculos deve crescer exatamente 3 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_tratados_de_direitos_humanos_com_forca_de_emenda_constitucional) ===';
