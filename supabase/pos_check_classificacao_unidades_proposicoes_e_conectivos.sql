...
-- Proposições e conectivos (curso_conteudos.id = 1).
--
...
-- exclusoes. 3 questoes classificadas: 2 rows REAL (Fundatec) + 1
-- AUTORAL (Q313). Notas de revisao taxonomica externas (Q314 em
-- Tabela-verdade; Q84 em Condicional e contrapositiva) NAO fazem parte
-- deste conteudo e NAO devem ser tocadas.

-- 1) A unidade pedagogica do conteudo 1. Esperado: 1 linha, ordem 1,
...
-- 4) QIDs vinculados exatos. Esperado: 74, 87, 313.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 1
  );
...
-- 8) Confirma banca de todas as 3 candidatas (2 REAL + 1 AUTORAL) e
--    vinculo neste conteudo. Esperado: 3 linhas, todas vinculo=1.
select q.id, q.banca, q.ano,
  (select count(*) from public.questao_unidades_pedagogicas qup2 join public.unidades_pedagogicas u2 on u2.id=qup2.unidade_pedagogica_id where qup2.questao_id=q.id and u2.curso_conteudo_id=1) as vinculo_neste_conteudo
from public.questoes q
where q.ativa = true and q.materia_id = 18 and q.assunto_id = 36
order by q.id;

-- 9) Confirma que as 2 questoes externas (notas de revisao taxonomica)
--    permanecem intactas: ativas, com seu assunto_id original, e SEM
--    vinculo neste conteudo. Esperado: assunto_id 314=38, 84=39;
--    vinculos_neste_conteudo=0 para ambas.
select id, ativa, assunto_id,
  (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where qup.questao_id = q.id and u.curso_conteudo_id = 1) as vinculos_neste_conteudo
from public.questoes q
where id in (314, 84)
order by id;
...
--     sistema.
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 1) as artigos_esperados_null,
