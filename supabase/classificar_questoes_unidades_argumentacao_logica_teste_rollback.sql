...
-- questoes de Argumentação lógica (curso_conteudos.id = 10)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/argumentacao_logica.unidades.json + config/argumentacao_logica.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_argumentacao_logica.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_argumentacao_logica.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (90, '5e2d5159-41da-4af7-b75d-4dc21239177d', 1, 'alta'),
  (285, '5e2d5159-41da-4af7-b75d-4dc21239177d', 1, 'alta'),
  (286, '5e2d5159-41da-4af7-b75d-4dc21239177d', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_10_materia_assunto',
    v_materia_id is not distinct from 18 and v_assunto_id is not distinct from 40,
    format('materia_id=%s assunto_id=%s (esperado 18/40)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '5e2d5159-41da-4af7-b75d-4dc21239177d' and curso_conteudo_id = 10 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 10 — depende de curadoria_unidades_argumentacao_logica.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 10) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 10 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 10)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=18/assunto_id=40', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 10', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '5e2d5159-41da-4af7-b75d-4dc21239177d';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_argumentacao_logica) ===';
