...
-- supabase/mapa_classificacao_pacto_internacional_sobre_direitos_economicos_sociais_e_culturais.sql,
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_estatuto_da_pessoa_com_deficiencia.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_pacto_internacional_sobre_direitos_economicos_sociais_e_culturais.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (125, 'f33221cc-f53c-4f91-88e4-a6d8440beca0'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 77 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 84 then
    raise exception 'Precondicao falhou: conteudo 77 materia_id=% assunto_id=% (esperado 11/84)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 77) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 77 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'f33221cc-f53c-4f91-88e4-a6d8440beca0' and curso_conteudo_id = 77 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_pacto_internacional_sobre_direitos_economicos_sociais_e_culturais.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=84', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 77', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 77) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 77 != 1';
  end if;
