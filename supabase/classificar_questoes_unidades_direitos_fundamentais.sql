...
-- supabase/mapa_classificacao_direitos_fundamentais.sql,
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_lei_de_tortura.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_direitos_fundamentais.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (28, '325c5ca6-8165-472f-b02b-eb7b18c6f71d'::uuid, 1, 'alta'),
  (139, '325c5ca6-8165-472f-b02b-eb7b18c6f71d', 1, 'alta'),
  (292, '325c5ca6-8165-472f-b02b-eb7b18c6f71d', 1, 'alta'),
  (342, '325c5ca6-8165-472f-b02b-eb7b18c6f71d', 1, 'alta'),
  (343, '325c5ca6-8165-472f-b02b-eb7b18c6f71d', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 76
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 76 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 25 then
    raise exception 'Precondicao falhou: conteudo 76 materia_id=% assunto_id=% (esperado 11/25)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 76) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 76 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '325c5ca6-8165-472f-b02b-eb7b18c6f71d' and curso_conteudo_id = 76 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_direitos_fundamentais.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 76
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 25);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=25', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 76);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 76', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 76) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 76 != 1';
  end if;
