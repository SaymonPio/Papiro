...
-- config/redacao_oficial.mapa.json,
...
--
-- Décimo terceiro conteúdo de Língua Portuguesa da fila. artigos_esperados
-- desta unidade é NULL (metodologia não jurídica). 0 exclusões
-- intencionais — todas as 4 questões ativas foram classificadas, após
-- auditoria normativa dedicada a Q118 (Decreto Federal nº 9.758/2019 ×
-- MRPR 2018 × Manual de Redação Oficial do Poder Executivo do RS) que
-- confirmou ausência de contradição material.
--
...
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_redacao_oficial.sql precisa
-- já ter sido aplicado (reaproveita unidade placeholder já existente
-- 29bfb433-4013-4164-85de-fd847963199d — não cria unidade nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (18, '29bfb433-4013-4164-85de-fd847963199d'::uuid, 1, 'alta'),
  (73, '29bfb433-4013-4164-85de-fd847963199d', 1, 'alta'),
  (117, '29bfb433-4013-4164-85de-fd847963199d', 1, 'alta'),
  (118, '29bfb433-4013-4164-85de-fd847963199d', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 30
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 30 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 16 then
    raise exception 'Precondicao falhou: conteudo 30 materia_id=% assunto_id=% (esperado 6/16)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 30) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 30 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '29bfb433-4013-4164-85de-fd847963199d' and curso_conteudo_id = 30 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_redacao_oficial.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 30
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 16);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=16', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 30);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 30', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 30) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 30 != 1';
  end if;
  ...
  where id = '29bfb433-4013-4164-85de-fd847963199d';
  ...
  raise notice 'Pos-condicoes OK: 4 questoes classificadas / 4 vinculos / 0 multiunidade / artigos_esperados NULL.';
end $$;
