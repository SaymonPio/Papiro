-- Pos-check SOMENTE LEITURA da classificacao de questoes de Hierarquia e
-- disciplina (curso_conteudos.id = 52) — gerado seguindo o mesmo template
...
-- classificar_questoes_unidades_hierarquia_e_disciplina.sql (a versao que
...
--
-- NOTA: SOBREPOSICAO reconhecida e documentada, sem movimentacao — Q12
-- (art. 12, caput) e estruturalmente proxima de Q20/Q300, ja classificadas
-- no curso_conteudo_id 51 (Estatuto dos Militares Estaduais); Q45 (alt5,
-- art. 15, caput) testa o mesmo ponto de Q53 (alt4) do mesmo conteudo 51.
-- Existe tambem redundancia conceitual com a Q212 do curso_conteudo_id 64
-- (RDBM, Decreto 43.245/2004, art. 3o) — diploma distinto. O conteudo 51
-- NAO foi reaberto ou alterado por esta operacao.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 52, com
...
--    ordem 1 (Hierarquia e disciplina) = 3
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 52
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 52 aponta para questao
...
--    cbde0aeb-3df8-4c41-a82f-91b83b529668): curso_conteudos=93,
