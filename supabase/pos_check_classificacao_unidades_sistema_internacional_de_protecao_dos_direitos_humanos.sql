...
-- Internacional de Protecao dos Direitos Humanos (curso_conteudos.id =
-- 82) — gerado seguindo o mesmo template de
...
--
-- NOTA: conteudo integralmente doutrinario (classificacao dos sistemas
-- universal/regionais e principio da complementaridade). artigos_esperados
-- = [] no config foi convertido pelo utilitario arrayPgTexto para NULL
-- no banco (comportamento ja estabelecido do toolkit) — consulta 9
-- confirma artigos_esperados IS NULL.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 82, com
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 82
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 82 aponta para questao
...
--    522f7c40-b95e-4c91-b0d6-cf4a6f012c16): curso_conteudos=93,
