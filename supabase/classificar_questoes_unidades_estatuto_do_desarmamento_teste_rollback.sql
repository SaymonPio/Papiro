...
-- questoes de Estatuto do Desarmamento (curso_conteudos.id = 56)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/estatuto_do_desarmamento.unidades.json + config/estatuto_do_desarmamento.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_estatuto_do_desarmamento.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_estatuto_do_desarmamento.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (50, 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_56_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 68,
    format('materia_id=%s assunto_id=%s (esperado 10/68)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f' and curso_conteudo_id = 56 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 56 — depende de curadoria_unidades_estatuto_do_desarmamento.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 56) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 56 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 56)));
  else
  ...
  insert into _relatorio values (
    'total_candidatas_12',
    v_total_candidatas = 12,
    format('total_candidatas=%s (esperado 12)', v_total_candidatas)
  );
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=68', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_12_distintas',
    v_distintas = 12,
    format('mapa cobre %s questoes distintas (esperado 12)', v_distintas)
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 12 ativas', v_fora_do_candidato)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 56', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 56;
  insert into _relatorio values ('total_vinculos_12', v_total_vinculos = 12, format('total_vinculos=%s (esperado 12)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 56;
  insert into _relatorio values ('questoes_classificadas_12', v_questoes_classificadas = 12, format('questoes_classificadas=%s (esperado 12)', v_questoes_classificadas));
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'd88a80ae-187b-4a24-a5cb-3b20ff32e26f';
  insert into _relatorio values ('u1_12_questoes', v_qtd_tmp = 12, format('u1_questoes=%s (esperado 12)', v_qtd_tmp));
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_12',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 12,
    'total de vinculos deve crescer exatamente 12 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_estatuto_do_desarmamento) ===';
