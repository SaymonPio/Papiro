-- Pos-check SOMENTE LEITURA da classificacao de questoes de Estatuto Estadual
-- da Igualdade Racial (curso_conteudos.id = 68) — gerado seguindo o mesmo
...
--
-- NOTA: Q132/Q852 e Q136/Q851 sao duplicatas confirmadas — ambas
-- classificadas por decisao explicita do usuario nesta curadoria; o
-- saneamento de duplicatas fica para um fluxo separado, futuro.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 68, com
...
--    ordem 1 (Estatuto Estadual da Igualdade Racial) = 10
...
--    Esperado: total_vinculos = 10, questoes_distintas = 10.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 68
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 68 aponta para questao
...
--    f0f6bae6-df70-450a-b4ee-3d9747962c2e): curso_conteudos=93,
