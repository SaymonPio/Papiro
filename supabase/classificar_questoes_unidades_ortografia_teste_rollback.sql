...
-- questoes de Ortografia (curso_conteudos.id = 20)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/ortografia.unidades.json + config/ortografia.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_ortografia.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_ortografia.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (66, 'dc6d39fb-5600-43d4-bad6-e3de2356236d', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_20_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 53,
    format('materia_id=%s assunto_id=%s (esperado 6/53)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'dc6d39fb-5600-43d4-bad6-e3de2356236d' and curso_conteudo_id = 20 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 20 — depende de curadoria_unidades_ortografia.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 20) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 20 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 20)));
  else
  ...
  insert into _relatorio values (
    'total_candidatas_24',
    v_total_candidatas = 24,
    format('total_candidatas=%s (esperado 24)', v_total_candidatas)
  );
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=53', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_22_distintas',
    v_distintas = 22,
    format('mapa cobre %s questoes distintas (esperado 22)', v_distintas)
  );

  select exists (select 1 from _mapa where questao_id in (688,689)) into v_contem_excluida;
  insert into _relatorio values (
    'mapa_nao_contem_excluidas',
    not coalesce(v_contem_excluida, false),
    'as questoes excluidas intencionalmente (688, 689) nao devem constar no mapa'
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 24 ativas', v_fora_do_candidato)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 20', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 20;
  insert into _relatorio values ('total_vinculos_22', v_total_vinculos = 22, format('total_vinculos=%s (esperado 22)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 20;
  insert into _relatorio values ('questoes_classificadas_22', v_questoes_classificadas = 22, format('questoes_classificadas=%s (esperado 22)', v_questoes_classificadas));
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'dc6d39fb-5600-43d4-bad6-e3de2356236d';
  insert into _relatorio values ('u1_22_questoes', v_qtd_tmp = 22, format('u1_questoes=%s (esperado 22)', v_qtd_tmp));

  select exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 20 and qup.questao_id = 688
  ) into v_excluida_tmp;
  insert into _relatorio values ('688_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 688 (PROBLEMA_DE_DADO_LACUNAS_AUSENTES — o enunciado armazenado perdeu os marcadores de lacuna originais (a mesma passagem, preservada em Q683, mostra marcadores de posicao I)/II)/III) apos as palavras ja resolvidas concepcao/dimensao/caos, que coincidem com o gabarito de Q688, mas a estrutura exata de onde cortar cada palavra para reconstituir as lacunas nao foi recuperada de forma inequivoca). Nao reconstruido artificialmente. Forte evidencia disponivel para saneamento futuro.) nao deve ter sido classificada por este arquivo');

  select exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 20 and qup.questao_id = 689
  ) into v_excluida_tmp;
  insert into _relatorio values ('689_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 689 (DUPLICATA_EXATA_DE_Q321 — mesmo enunciado, mesmas 5 alternativas na mesma ordem, mesmo gabarito (s-x-s-z). Q321 mantida como classificacao canonica.) nao deve ter sido classificada por este arquivo');
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_22',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 22,
    'total de vinculos deve crescer exatamente 22 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_ortografia) ===';
