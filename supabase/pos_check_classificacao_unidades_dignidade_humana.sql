-- Pos-check SOMENTE LEITURA da classificacao de questoes de Dignidade
-- humana (curso_conteudos.id = 98) — gerado seguindo o mesmo template
...
-- classificar_questoes_unidades_dignidade_humana.sql (a versao que
...
--
-- NOTA: artigos_esperados = ['art. 1º, III'] (CF/88), unico item, por
-- decisao aprovada sobre colisao de parser com a DUDH art. 1
-- (fundamento pleno de Q13, documentado apenas em escopo/mapa/_nota,
-- NAO ausente da futura aula). Sobreposicao tematica (nao duplicata),
-- cross-materia, com o conteudo 57 (Constituicao Federal de 1988, Q794
-- e Q651).

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 98, com
--    titulo/escopo/artigos aplicados pela curadoria. Esperado: 1 linha(s),
--    ordem 1, artigos_esperados com 1 item (art. 1º, III), ativa.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 98
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 98 aponta para questao
...
--    70bd2ed8-e947-4050-b1aa-e1ceb2c0be1f): curso_conteudos=93,
...
-- 8) Conteudo 57 (Constituicao Federal de 1988, materia Legislacao
--    Especifica, ja concluido) permanece intocado por esta operacao —
--    sobreposicao apenas tematica (cross-materia), nao duplicata.
--    Esperado (baseline ao vivo em 2026-08-22, antes desta operacao):
--    unidades_57=1, vinculos_57=14.
select
...
-- 9) Confirma exatamente o artigo_esperado aprovado (art. 1º, III), e
--    que NAO ha "art. 1" isolado (DUDH) nem outros incisos do art. 1º
--    da CF no array.
--    Esperado: contem_art1_iii=true, contem_art1=false,
--    contem_art1_i=false, contem_art1_ii=false, contem_art1_iv=false,
--    contem_art1_v=false, total=1.
select
  (artigos_esperados @> array['art. 1º, III']::text[]) as contem_art1_iii,
  (artigos_esperados @> array['art. 1']::text[]) as contem_art1,
  (artigos_esperados @> array['art. 1º, I']::text[]) as contem_art1_i,
  (artigos_esperados @> array['art. 1º, II']::text[]) as contem_art1_ii,
  (artigos_esperados @> array['art. 1º, IV']::text[]) as contem_art1_iv,
  (artigos_esperados @> array['art. 1º, V']::text[]) as contem_art1_v,
