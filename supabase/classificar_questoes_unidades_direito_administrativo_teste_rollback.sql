...
-- questoes de Direito Administrativo (curso_conteudos.id = 58)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/direito_administrativo.unidades.json + config/direito_administrativo.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_direito_administrativo.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_direito_administrativo.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (195, '1172a885-1419-4ad8-b728-1a0c7492c133', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_58_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 65,
    format('materia_id=%s assunto_id=%s (esperado 10/65)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '1172a885-1419-4ad8-b728-1a0c7492c133' and curso_conteudo_id = 58 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 58 — depende de curadoria_unidades_direito_administrativo.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 58) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 58 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 58)));
  else
  ...
  insert into _relatorio values (
    'total_candidatas_8',
    v_total_candidatas = 8,
    format('total_candidatas=%s (esperado 8)', v_total_candidatas)
  );
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=65', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_8_distintas',
    v_distintas = 8,
    format('mapa cobre %s questoes distintas (esperado 8)', v_distintas)
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 8 ativas', v_fora_do_candidato)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 58', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 58;
  insert into _relatorio values ('total_vinculos_8', v_total_vinculos = 8, format('total_vinculos=%s (esperado 8)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 58;
  insert into _relatorio values ('questoes_classificadas_8', v_questoes_classificadas = 8, format('questoes_classificadas=%s (esperado 8)', v_questoes_classificadas));
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '1172a885-1419-4ad8-b728-1a0c7492c133';
  insert into _relatorio values ('u1_8_questoes', v_qtd_tmp = 8, format('u1_questoes=%s (esperado 8)', v_qtd_tmp));
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_8',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 8,
    'total de vinculos deve crescer exatamente 8 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_direito_administrativo) ===';
