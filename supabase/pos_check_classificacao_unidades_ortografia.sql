-- Pos-check SOMENTE LEITURA da classificacao de questoes de Ortografia
-- (curso_conteudos.id = 20) — gerado seguindo o mesmo template de
-- scripts/curadoria-pedagogica/gerar-pos-check.mjs, adaptado para o
-- primeiro conteudo de Lingua Portuguesa da fila (piloto da
-- metodologia nao juridica). Os totais da consulta 7 foram obtidos ao
-- vivo (leitura direta, somente SELECT) porque este ambiente local nao
-- tinha SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY configurados para o
-- proprio script rodar essa parte sozinho (ver .env.curadoria /
-- env.curadoria.example). A rodar depois de
-- classificar_questoes_unidades_ortografia.sql (a versao que termina
...
--
-- NOTA: artigos_esperados = NULL para toda a unidade (decisao
-- metodologica: parser so reconhece "art.", nao representa as Bases do
-- Acordo Ortografico). Bases XV/XVI documentadas apenas no mapa/_nota.
-- Q688 (PROBLEMA_DE_DADO_LACUNAS_AUSENTES) e Q689
-- (DUPLICATA_EXATA_DE_Q321) excluidas intencionalmente — permanecem
-- ativas, intactas e sem vinculo pedagogico.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 20, com
...
--    Esperado: total_vinculos = 22, questoes_distintas = 22.
...
--    E NAO estao na lista de exclusoes intencionais (688, 689).
--    Esperado: 0 linhas.
...
where q.ativa = true
  and q.materia_id = 6
  and q.assunto_id = 53
  and q.id not in (688, 689)
  and not exists (
    select 1
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 20
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 20 aponta para questao
...
--    dc6d39fb-5600-43d4-bad6-e3de2356236d): curso_conteudos=93,
...
-- 8) Q688 e Q689 permanecem ATIVAS, INTACTAS (enunciado/alternativas
--    inalterados) e SEM vinculo pedagogico. Esperado: ambas ativa=true,
--    vinculos=0.
select
  q.id,
  q.ativa,
...
from public.questoes q
where q.id in (688, 689)
order by q.id;

-- 9) Confirma que artigos_esperados permanece NULL (nao ha "art."
--    nem "Base" no array).
