...
-- questoes de Diagramas lógicos (curso_conteudos.id = 11)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/diagramas_logicos.unidades.json + config/diagramas_logicos.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_diagramas_logicos.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_diagramas_logicos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (246, '5544e77a-f186-4b1e-9a6d-5ebfbfd12ca9', 1, 'alta'),
  (247, '5544e77a-f186-4b1e-9a6d-5ebfbfd12ca9', 1, 'alta'),
  (248, '5544e77a-f186-4b1e-9a6d-5ebfbfd12ca9', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_11_materia_assunto',
    v_materia_id is not distinct from 18 and v_assunto_id is not distinct from 31,
    format('materia_id=%s assunto_id=%s (esperado 18/31)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '5544e77a-f186-4b1e-9a6d-5ebfbfd12ca9' and curso_conteudo_id = 11 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 11 — depende de curadoria_unidades_diagramas_logicos.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 11) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 11 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 11)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=18/assunto_id=31', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 11', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '5544e77a-f186-4b1e-9a6d-5ebfbfd12ca9';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_diagramas_logicos) ===';
