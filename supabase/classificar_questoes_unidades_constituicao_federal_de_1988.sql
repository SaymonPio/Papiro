...
-- supabase/mapa_classificacao_constituicao_federal_de_1988.sql, validado pelo harness
-- classificar_questoes_unidades_constituicao_federal_de_1988_teste_rollback.sql (tudo_ok =
...
-- mesmo padrão de supabase/classificar_questoes_unidades_lei_drogas.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_constituicao_federal_de_1988.sql precisa já
-- ter sido aplicado.
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (48, '682804b0-2762-4aff-87f9-d7a5a81757c2'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 57 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 70 then
    raise exception 'Precondicao falhou: conteudo 57 materia_id=% assunto_id=% (esperado 10/70)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 57) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 57 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '682804b0-2762-4aff-87f9-d7a5a81757c2' and curso_conteudo_id = 57 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_constituicao_federal_de_1988.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=70', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 57', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 57) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 57 != 1';
  end if;
