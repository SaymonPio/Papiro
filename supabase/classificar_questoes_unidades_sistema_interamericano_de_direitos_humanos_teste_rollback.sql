...
-- questoes de Sistema Interamericano de Direitos Humanos (curso_conteudos.id = 85)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/sistema_interamericano_de_direitos_humanos.unidades.json + config/sistema_interamericano_de_direitos_humanos.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_sistema_interamericano_de_direitos_humanos.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_sistema_interamericano_de_direitos_humanos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (183, 'd815fc1f-82d3-4411-9dc5-63dae5373d2b', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_85_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 86,
    format('materia_id=%s assunto_id=%s (esperado 11/86)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'd815fc1f-82d3-4411-9dc5-63dae5373d2b' and curso_conteudo_id = 85 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 85 — depende de curadoria_unidades_sistema_interamericano_de_direitos_humanos.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 85) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 85 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 85)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=86', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 85', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'd815fc1f-82d3-4411-9dc5-63dae5373d2b';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_sistema_interamericano_de_direitos_humanos) ===';
