...
-- questoes de Tratado de Marraqueche (curso_conteudos.id = 94)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/tratado_de_marraqueche.unidades.json + config/tratado_de_marraqueche.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_tratado_de_marraqueche.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_tratado_de_marraqueche.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (189, '389ff0e7-38f5-4fe5-b66d-4ebbbb5ee9bf', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_94_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 105,
    format('materia_id=%s assunto_id=%s (esperado 11/105)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '389ff0e7-38f5-4fe5-b66d-4ebbbb5ee9bf' and curso_conteudo_id = 94 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 94 — depende de curadoria_unidades_tratado_de_marraqueche.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 94) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 94 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 94)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=105', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 94', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '389ff0e7-38f5-4fe5-b66d-4ebbbb5ee9bf';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_tratado_de_marraqueche) ===';
