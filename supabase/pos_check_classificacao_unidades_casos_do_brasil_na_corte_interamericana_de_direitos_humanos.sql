-- Pos-check SOMENTE LEITURA da classificacao de questoes de Casos do
-- Brasil na Corte Interamericana de Direitos Humanos
-- (curso_conteudos.id = 90) — gerado seguindo o mesmo template de
...
--
-- NOTA: conteudo genuinamente jurisprudencial — artigos_esperados
-- aprovado como lista vazia (NULL no banco). Os tres casos (Ximenes
-- Lopes, Gomes Lund/Guerrilha do Araguaia, Herzog) sao o nucleo
-- pedagogico da unidade, documentados em escopo/mapa/_nota, sem
-- converter jurisprudencia em artigo artificial.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 90, com
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 90
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 90 aponta para questao
...
--    420c8c2f-6f80-422a-9e63-d64aace51465): curso_conteudos=93,
...
-- 8) Conteudo 88 (Corte Interamericana de Direitos Humanos — estrutura
--    institucional), ja concluido, permanece intocado por esta operacao
--    — sobreposicao apenas tematica, nao duplicata. Esperado (baseline
--    ao vivo em 2026-08-22, antes desta operacao): unidades_88=1,
--    vinculos_88=4.
select
...
-- 9) Confirma que artigos_esperados permanece NULL (lista vazia
--    aprovada — conteudo genuinamente jurisprudencial).
