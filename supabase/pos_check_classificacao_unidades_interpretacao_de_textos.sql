...
-- Interpretacao de textos (curso_conteudos.id = 12).
--
...
-- 1) A unidade pedagogica do conteudo 12. Esperado: 1 linha, ordem 1,
...
-- 4) QIDs vinculados exatos. Esperado: 17, 65, 306.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 12
  );
...
-- 8) Saneamento de Q65 preservado. Esperado: todas as flags true.
select
  (select position('[01]' in enunciado) > 0 from public.questoes where id = 65) as q65_marcador_01,
  (select position('[32]' in enunciado) > 0 from public.questoes where id = 65) as q65_marcador_32,
  (select count(*) from public.questao_unidades_pedagogicas where questao_id = 65) as q65_vinculos;

-- 9) Confirma artigos_esperados NULL e estado geral do sistema.
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 12) as artigos_esperados_null,
