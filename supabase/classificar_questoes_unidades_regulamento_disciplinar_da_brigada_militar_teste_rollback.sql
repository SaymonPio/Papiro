...
-- questoes de Regulamento Disciplinar da Brigada Militar (curso_conteudos.id = 64)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/regulamento_disciplinar_da_brigada_militar.unidades.json + config/regulamento_disciplinar_da_brigada_militar.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_regulamento_disciplinar_da_brigada_militar.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_regulamento_disciplinar_da_brigada_militar.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (210, '454f8501-7818-4dc4-b22a-337247678c58', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_64_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 73,
    format('materia_id=%s assunto_id=%s (esperado 10/73)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '454f8501-7818-4dc4-b22a-337247678c58' and curso_conteudo_id = 64 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 64 — depende de curadoria_unidades_regulamento_disciplinar_da_brigada_militar.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 64) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 64 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 64)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=73', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 64', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '454f8501-7818-4dc4-b22a-337247678c58';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_regulamento_disciplinar_da_brigada_militar) ===';
