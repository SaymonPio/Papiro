...
-- supabase/mapa_classificacao_jurisprudencia_do_stf_e_stj.sql,
...
-- mesmo padrão de supabase/classificar_questoes_unidades_estatuto_nacional_da_igualdade_racial.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_jurisprudencia_do_stf_e_stj.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (198, '8e1d1204-1f66-431c-a95f-6403e77882dc'::uuid, 1, 'alta'),
  (199, '8e1d1204-1f66-431c-a95f-6403e77882dc', 1, 'alta'),
  (200, '8e1d1204-1f66-431c-a95f-6403e77882dc', 1, 'alta'),
  (670, '8e1d1204-1f66-431c-a95f-6403e77882dc', 1, 'alta'),
  (733, '8e1d1204-1f66-431c-a95f-6403e77882dc', 1, 'alta');

-- Lock determinístico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 69
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 69 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 66 then
    raise exception 'Precondicao falhou: conteudo 69 materia_id=% assunto_id=% (esperado 10/66)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 69) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 69 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '8e1d1204-1f66-431c-a95f-6403e77882dc' and curso_conteudo_id = 69 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_jurisprudencia_do_stf_e_stj.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 69
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 66);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=66', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 69);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 69', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 69) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 69 != 1';
  end if;
