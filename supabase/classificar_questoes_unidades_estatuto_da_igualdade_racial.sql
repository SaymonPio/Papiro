...
-- supabase/mapa_classificacao_estatuto_da_igualdade_racial.sql,
...
-- questoes/alternativas/unidades_pedagogicas/aulas/histórico — Q131/Q816
-- e Q815, todas quase/exatamente duplicadas, permanecem intactas e
-- classificadas; o curso_conteudo_id 67 não é tocado por este arquivo).
-- Segue o mesmo padrão de
-- supabase/classificar_questoes_unidades_pacto_de_san_jose_da_costa_rica.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_estatuto_da_igualdade_racial.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (131, '9cc42871-c31c-440a-88cb-32f0d4f232ff'::uuid, 1, 'alta'),
  (255, '9cc42871-c31c-440a-88cb-32f0d4f232ff', 1, 'alta'),
  (256, '9cc42871-c31c-440a-88cb-32f0d4f232ff', 1, 'alta'),
  (695, '9cc42871-c31c-440a-88cb-32f0d4f232ff', 1, 'alta'),
  (790, '9cc42871-c31c-440a-88cb-32f0d4f232ff', 1, 'alta'),
  (813, '9cc42871-c31c-440a-88cb-32f0d4f232ff', 1, 'alta'),
  (814, '9cc42871-c31c-440a-88cb-32f0d4f232ff', 1, 'alta'),
  (815, '9cc42871-c31c-440a-88cb-32f0d4f232ff', 1, 'alta'),
  (816, '9cc42871-c31c-440a-88cb-32f0d4f232ff', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 97 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 98 then
    raise exception 'Precondicao falhou: conteudo 97 materia_id=% assunto_id=% (esperado 11/98)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 97) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 97 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '9cc42871-c31c-440a-88cb-32f0d4f232ff' and curso_conteudo_id = 97 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_estatuto_da_igualdade_racial.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 97
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 98);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=98', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 97);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 97', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 97) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 97 != 1';
  end if;
