...
-- supabase/mapa_classificacao_protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul.sql,
...
-- conteudo ja concluido 91 (Direitos Humanos no Mercosul) — Q182 deste
-- conteudo testa substancialmente a mesma tese juridica do art. 1 do
-- mesmo Protocolo de Assuncao ja usada para Q162/Q163 do conteudo 91,
-- mas com enunciados diferentes e nenhuma duplicata exata de ID. O
-- conteudo 91 nao e tocado por este arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_direitos_humanos_no_mercosul.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 655700d6-b585-468f-8d98-8143090cbafb — não cria unidade nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (180, '655700d6-b585-468f-8d98-8143090cbafb'::uuid, 1, 'alta'),
  (181, '655700d6-b585-468f-8d98-8143090cbafb', 1, 'alta'),
  (182, '655700d6-b585-468f-8d98-8143090cbafb', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 92 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 93 then
    raise exception 'Precondicao falhou: conteudo 92 materia_id=% assunto_id=% (esperado 11/93)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 92) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 92 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '655700d6-b585-468f-8d98-8143090cbafb' and curso_conteudo_id = 92 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_protocolo_de_assuncao_sobre_direitos_humanos_no_mercosul.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 92
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 91) then
    raise exception 'Precondicao falhou: curso_conteudos 91 (sobreposicao tematica Direitos Humanos no Mercosul) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 93);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=93', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 92);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 92', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 92) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 92 != 1';
  end if;
  ...
  -- Pos-condicao extra: artigos_esperados deve ser exatamente ['art. 1'].
  select artigos_esperados into v_artigos_esperados
  from public.unidades_pedagogicas
  where id = '655700d6-b585-468f-8d98-8143090cbafb';
  if v_artigos_esperados is distinct from array['art. 1']::text[] then
    raise exception 'Pos-condicao falhou: artigos_esperados=% (esperado [art. 1])', v_artigos_esperados;
  end if;

  -- Pos-condicao extra: conteudo 91 (sobreposicao tematica) permanece intocado.
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 91) <> (select unidades_91 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 91 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 91) <> (select vinculos_91 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 91 mudou — nao deveria ser tocado';
  end if;

  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / artigos_esperados = [art. 1] / conteudo 91 intocado.';
end $$;
