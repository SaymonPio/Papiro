...
-- supabase/mapa_classificacao_pacto_de_san_jose_da_costa_rica.sql,
...
-- questoes/alternativas/unidades_pedagogicas/aulas/histórico — Q130 e
-- Q822, quase-duplicatas, permanecem ambas intactas e classificadas).
-- Segue o mesmo padrão de
-- supabase/classificar_questoes_unidades_lei_de_organizacao_basica_da_brigada_militar.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_pacto_de_san_jose_da_costa_rica.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (56, 'b918b069-8364-4412-80a6-07d78b369317'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 72 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 90 then
    raise exception 'Precondicao falhou: conteudo 72 materia_id=% assunto_id=% (esperado 11/90)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 72) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 72 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'b918b069-8364-4412-80a6-07d78b369317' and curso_conteudo_id = 72 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_pacto_de_san_jose_da_costa_rica.sql precisa ter sido aplicado antes';
  end if;
  ...
  if v_total_candidatas <> 11 then
    raise exception 'Precondicao falhou: total de candidatas ativas = % (esperado 11)', v_total_candidatas;
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=90', v_invalidas;
  end if;
  ...
  if v_fora_do_candidato <> 0 then
    raise exception 'Mapa invalido: % linha(s) fora do conjunto candidato de 11 ativas', v_fora_do_candidato;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 72', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 72) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 72 != 1';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 11 questoes / 11 vinculos / 0 multiunidade.';
end $$;
