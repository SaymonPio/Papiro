...
-- questoes de Significação das palavras (curso_conteudos.id = 23)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/significacao_das_palavras.unidades.json + config/significacao_das_palavras.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_significacao_das_palavras.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_significacao_das_palavras.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (122, '290650b5-0f55-49e1-871e-932003447e41', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_23_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 59,
    format('materia_id=%s assunto_id=%s (esperado 6/59)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '290650b5-0f55-49e1-871e-932003447e41' and curso_conteudo_id = 23 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 23 — depende de curadoria_unidades_significacao_das_palavras.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 23) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 23 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 23)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=59', v_invalidas)
  );
  ...
  select exists (select 1 from _mapa where questao_id in (68)) into v_contem_excluida;
  insert into _relatorio values (
    'mapa_nao_contem_excluidas',
    not coalesce(v_contem_excluida, false),
    'as questoes excluidas intencionalmente (68) nao devem constar no mapa'
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 23', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '290650b5-0f55-49e1-871e-932003447e41';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_significacao_das_palavras) ===';
