...
-- config/regencia_verbal_e_nominal.mapa.json,
...
--
-- Décimo sexto conteúdo de Língua Portuguesa da fila. artigos_esperados
-- desta unidade é NULL (metodologia não jurídica). 0 exclusões
-- intencionais — todas as 3 questões ativas foram classificadas.
-- Microauditoria global prévia confirmou ausência de candidatas reais
-- adicionais (GAP_DE_BANCO_REGENCIA_VERBAL_E_NOMINAL quantitativo real
-- severo — 1 real/2 autorais). GAP_SUBASSUNTO_REGENCIA_NOMINAL também
-- registrado no config (nenhuma questão deste corpus cobre regência
...
-- mesmo padrão de supabase/classificar_questoes_unidades_pontuacao.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_regencia_verbal_e_nominal.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 735f736a-37c0-477f-a555-dcd73d243d21 — não cria unidade
-- nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (120, '735f736a-37c0-477f-a555-dcd73d243d21'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 17 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 42 then
    raise exception 'Precondicao falhou: conteudo 17 materia_id=% assunto_id=% (esperado 6/42)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 17) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 17 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '735f736a-37c0-477f-a555-dcd73d243d21' and curso_conteudo_id = 17 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_regencia_verbal_e_nominal.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=42', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 17', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 17) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 17 != 1';
  end if;
  ...
  where id = '735f736a-37c0-477f-a555-dcd73d243d21';
