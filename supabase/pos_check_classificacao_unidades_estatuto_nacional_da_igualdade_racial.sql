-- Pos-check SOMENTE LEITURA da classificacao de questoes do Estatuto
-- Nacional da Igualdade Racial (curso_conteudos.id = 67) — gerado seguindo
-- o mesmo template de scripts/curadoria-pedagogica/gerar-pos-check.mjs.
-- Os totais da consulta 7 foram obtidos ao vivo (leitura direta, somente
...
--
-- NOTA: Lei nº 12.288/2010 vigente. O art. 39 recebeu acréscimos
-- posteriores pela Lei 14.553/2023 (§§8º e 9º), sem impacto sobre o §1º
-- efetivamente cobrado nesta unidade. Os demais dispositivos utilizados
-- pelas cinco questões foram verificados no texto oficial vigente.
--
-- NOTA: existe conteúdo separado para o Estatuto Estadual da Igualdade
-- Racial (curso_conteudo_id 68, Lei Estadual RS nº 13.694/2011) — diploma
-- diferente, nenhuma questão duplicada, nenhuma movimentação/exclusão.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 67, com
...
--    ordem 1 (Estatuto Nacional da Igualdade Racial) = 5
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 67
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 67 aponta para questao
...
--    5bf890e1-8e09-4f7a-9410-7f5e5168d9c4): curso_conteudos=93,
