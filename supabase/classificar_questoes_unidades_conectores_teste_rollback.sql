...
-- questoes de Conectores (curso_conteudos.id = 14)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/conectores.unidades.json + config/conectores.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_conectores.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_conectores.sql
...
begin
  ...
  insert into _relatorio values (
    'conteudo_14_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 57,
    format('materia_id=%s assunto_id=%s (esperado 6/57)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'd1e31767-d27d-431b-ba59-7a2008c7473d' and curso_conteudo_id = 14 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 14 — depende de curadoria_unidades_conectores.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 14) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 14 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 14)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=57', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 14', v_unidade_fora)
  );
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_conectores) ===';
