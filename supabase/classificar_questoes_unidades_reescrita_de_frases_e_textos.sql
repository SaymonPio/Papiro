...
-- config/reescrita_de_frases_e_textos.mapa.json,
-- validado pelo harness
-- classificar_questoes_unidades_reescrita_de_frases_e_textos_teste_rollback.sql
-- (tudo_ok = true confirmado antes de rodar este arquivo, contra o
-- estado LIVE apos o saneamento taxonomico da Q279 — commit bed8641 —
-- que ja removeu Q279 do conjunto candidato deste assunto).
--
-- Decimo oitavo conteudo de Lingua Portuguesa da fila. artigos_esperados
-- desta unidade e NULL (metodologia nao juridica). 0 exclusoes
-- intencionais — as 2 questoes ativas restantes (Q72, Q280) foram
-- classificadas, apos auditoria alternativa-por-alternativa confirmar
-- que nenhuma delas e QUESTAO_HIBRIDA_MULTICONTEUDO.
--
...
-- pos-condicao extra confirmando que Q279 (realocada para Conectores)
-- nao aparece como candidata nem recebe vinculo neste conteudo.
--
-- PRE-REQUISITO: supabase/curadoria_unidades_reescrita_de_frases_e_textos.sql
-- precisa ja ter sido aplicado (reaproveita unidade placeholder ja
-- existente 5cc30e49-890f-4b83-b96f-31724024ee24 — nao cria unidade
-- nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (72, '5cc30e49-890f-4b83-b96f-31724024ee24'::uuid, 1, 'alta'),
  (280, '5cc30e49-890f-4b83-b96f-31724024ee24', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 21 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 49 then
    raise exception 'Precondicao falhou: conteudo 21 materia_id=% assunto_id=% (esperado 6/49)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 21) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 21 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '5cc30e49-890f-4b83-b96f-31724024ee24' and curso_conteudo_id = 21 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_reescrita_de_frases_e_textos.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 21
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if v_total_candidatas <> 2 then
    raise exception 'Precondicao falhou: total de candidatas ativas = % (esperado 2 — Q279 ja deve ter sido realocada para Conectores)', v_total_candidatas;
  end if;
  ...
  -- Confirma que Q279 ja foi realocada e nao e mais candidata deste assunto.
  if exists (select 1 from public.questoes where id = 279 and assunto_id = 49) then
    raise exception 'Precondicao falhou: questao 279 ainda esta com assunto_id=49 (esperado: ja realocada para 57/Conectores)';
  end if;
  if exists (
    ...
    raise exception 'Precondicao falhou: questao 279 possui vinculo inesperado no conteudo 21';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 49);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=49', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 21);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 21', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  -- Q279 continua fora deste conteudo (realocada para Conectores).
  if exists (select 1 from public.questoes where id = 279 and assunto_id = 49) then
    raise exception 'Pos-condicao falhou: questao 279 voltou a ter assunto_id=49 indevidamente';
  end if;
  if exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    ...
    raise exception 'Pos-condicao falhou: questao 279 recebeu vinculo indevido no conteudo 21';
  end if;
  if not exists (select 1 from public.questoes where id = 279 and assunto_id = 57) then
    raise exception 'Pos-condicao falhou: questao 279 nao esta mais com assunto_id=57 (Conectores) — saneamento taxonomico anterior foi desfeito';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 21) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 21 != 1';
  end if;
  ...
  where id = '5cc30e49-890f-4b83-b96f-31724024ee24';
  ...
  raise notice 'Pos-condicoes OK: 2 questoes classificadas (72, 280) / 2 vinculos / 0 multiunidade / artigos_esperados NULL / questao 279 permanece fora deste conteudo, corretamente vinculada a Conectores (assunto_id=57).';
end $$;
