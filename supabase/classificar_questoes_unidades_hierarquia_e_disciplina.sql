...
-- supabase/mapa_classificacao_hierarquia_e_disciplina.sql,
...
-- mesmo padrão de supabase/classificar_questoes_unidades_plano_de_carreira_dos_servidores_militares.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_hierarquia_e_disciplina.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (12, 'cbde0aeb-3df8-4c41-a82f-91b83b529668'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 52 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 11 then
    raise exception 'Precondicao falhou: conteudo 52 materia_id=% assunto_id=% (esperado 10/11)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 52) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 52 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'cbde0aeb-3df8-4c41-a82f-91b83b529668' and curso_conteudo_id = 52 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_hierarquia_e_disciplina.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=11', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 52', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 52) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 52 != 1';
  end if;
