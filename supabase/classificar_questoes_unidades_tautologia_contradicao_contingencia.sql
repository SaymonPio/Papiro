...
-- config/tautologia_contradicao_contingencia.mapa.json, validado pelo
-- harness
...
--
-- Sétimo conteúdo do bloco final de Raciocínio Lógico/Matemática, após
-- Quantificadores (ordem 89). artigos_esperados desta unidade é NULL
-- (metodologia não jurídica). 0 exclusões — as 3 questões ativas foram
-- classificadas (2 rows REAL Fundatec + 1 AUTORAL Q315). Nenhum
-- saneamento prévio foi necessário nesta ordem — auditoria taxonômica
-- confirmou que as 3 candidatas pertencem genuinamente a este
-- conteúdo, sem sobreposição com Tabela-verdade, Equivalências
-- lógicas, Proposições e conectivos, Condicional e contrapositiva ou
-- Argumentação lógica.
--
-- MICROAUDITORIA EXTERNA (executada antes deste apply): encontrou Q75
-- (assunto 38, Tabela-verdade, já concluída na ordem 85) mencionando
-- "tautologia" apenas de passagem em sua explicação, sem que essa seja
-- a habilidade nuclear daquela questão. Q75 permanece corretamente em
-- Tabela-verdade; nenhuma ação foi tomada sobre ela.
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_tautologia_contradicao_contingencia.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- ae60f2db-49d0-4326-980c-df1617a0bc35 — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (76, 'ae60f2db-49d0-4326-980c-df1617a0bc35'::uuid, 1, 'alta'),
  (85, 'ae60f2db-49d0-4326-980c-df1617a0bc35'::uuid, 1, 'alta'),
  (315, 'ae60f2db-49d0-4326-980c-df1617a0bc35'::uuid, 1, 'alta');

-- Lock deterministico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 8
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 8 nao existe';
  end if;
  if v_materia_id is distinct from 18 or v_assunto_id is distinct from 37 then
    raise exception 'Precondicao falhou: conteudo 8 materia_id=% assunto_id=% (esperado 18/37)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'ae60f2db-49d0-4326-980c-df1617a0bc35' and curso_conteudo_id = 8 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_tautologia_contradicao_contingencia.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 8) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 8 — decisao aprovada foi manter unidade unica';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 8
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  -- Q75 (candidata externa descartada pela microauditoria) nao deve
  -- ter sido tocada: continua sem vinculo neste conteudo e mantem seu
  -- assunto_id original (38, Tabela-verdade).
  select assunto_id into v_assunto_75 from public.questoes where id = 75;
  if v_assunto_75 is distinct from 38 then
    raise exception 'Precondicao falhou: assunto_id da questao 75 diverge do esperado (38) — atual: %', v_assunto_75;
  end if;

  select count(*) into v_vinculos_75_aqui
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 8 and qup.questao_id = 75;
  if v_vinculos_75_aqui <> 0 then
    raise exception 'Precondicao falhou: questao 75 ja possui vinculo neste conteudo, nao deveria';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 18 and q.assunto_id = 37);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=18/assunto_id=37', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 8);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 8', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  -- Pos-condicao extra: Q75 continua fora deste conteudo, com
  -- assunto_id=38 intacto — este arquivo nao a toca.
  select assunto_id into v_assunto_75 from public.questoes where id = 75;
  if v_assunto_75 is distinct from 38 then
    raise exception 'Pos-condicao falhou: assunto_id da questao 75 foi alterado indevidamente (%), esperado 38', v_assunto_75;
  end if;

  select count(*) into v_vinculos_75
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 8 and qup.questao_id = 75;
  if v_vinculos_75 <> 0 then
    raise exception 'Pos-condicao falhou: questao 75 ganhou vinculo neste conteudo, nao deveria';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 8) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 8 != 1';
  end if;
  ...
  where id = 'ae60f2db-49d0-4326-980c-df1617a0bc35';
  ...
  raise notice 'Pos-condicoes OK: 3 questoes classificadas (Q76/Q85/Q315) / 3 vinculos / 0 multiunidade / artigos_esperados NULL / questao 75 intacta e sem vinculo neste conteudo.';
end $$;
