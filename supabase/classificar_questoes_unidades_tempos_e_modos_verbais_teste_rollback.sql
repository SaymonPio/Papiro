...
-- questoes de Tempos e modos verbais (curso_conteudos.id = 31)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/tempos_e_modos_verbais.unidades.json + config/tempos_e_modos_verbais.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_tempos_e_modos_verbais.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_tempos_e_modos_verbais.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (240, 'd54fb675-78a8-4efb-a99a-44359555d28f', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_31_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 54,
    format('materia_id=%s assunto_id=%s (esperado 6/54)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'd54fb675-78a8-4efb-a99a-44359555d28f' and curso_conteudo_id = 31 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 31 — depende de curadoria_unidades_tempos_e_modos_verbais.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 31) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 31 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 31)));
  else
  ...
  insert into _relatorio values (
    'total_candidatas_13',
    v_total_candidatas = 13,
    format('total_candidatas=%s (esperado 13)', v_total_candidatas)
  );
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=54', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_11_distintas',
    v_distintas = 11,
    format('mapa cobre %s questoes distintas (esperado 11)', v_distintas)
  );
  ...
  insert into _relatorio values (
    ...
    'as questoes excluidas intencionalmente (693, 893) nao devem constar no mapa'
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 13 ativas', v_fora_do_candidato)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 31', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 31;
  insert into _relatorio values ('total_vinculos_11', v_total_vinculos = 11, format('total_vinculos=%s (esperado 11)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 31;
  insert into _relatorio values ('questoes_classificadas_11', v_questoes_classificadas = 11, format('questoes_classificadas=%s (esperado 11)', v_questoes_classificadas));
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'd54fb675-78a8-4efb-a99a-44359555d28f';
  insert into _relatorio values ('u1_11_questoes', v_qtd_tmp = 11, format('u1_questoes=%s (esperado 11)', v_qtd_tmp));
  ...
  insert into _relatorio values ('693_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 693 (FORA_DE_ESCOPO_MORFOLOGIA_LOCUCAO_VERBAL — cobra identificacao da categoria morfologica locucao verbal (estavam relacionados) em contraste com locucao adverbial, locucao adjetiva, substantivo composto e adjetivo composto; nenhuma alternativa envolve tempo ou modo verbal, o tempo do auxiliar estavam (preterito imperfeito) e incidental, nao e o que se pergunta. Provavel destino futuro: Classes de palavras (conteudo 22, ja concluido) ou conteudo de estrutura/formacao de palavras da fila, a confirmar. Nao realocada nesta etapa.) nao deve ter sido classificada por este arquivo');
  ...
  insert into _relatorio values ('893_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 893 (QUESTAO_HIBRIDA_MULTICONTEUDO — cobra 4 assertivas de habilidades distintas sobre a palavra Trabalhando: (I) gerundio nao e preterito [tempos/modos verbais], (II) ausencia de prefixo [formacao de palavras, fora de escopo], (III) presenca de digrafos lh+an [fonemas e digrafos, conteudo 34 ja concluido], (IV) gerundio e forma nominal [tempos/modos verbais]. Para resolver integralmente a questao o aluno nao pode depender apenas da aula de tempos e modos verbais — por isso NAO e vinculada como pratica especifica desta unidade. O fenomeno gerundio como forma nominal cobrado pelos itens I e IV permanece registrado no escopo pedagogico e no raio-x de incidencia como evidencia real de que a Fundatec cobrou esse conceito (Profis Pref Viamao/2022), distinguindo evidencia de que caiu de questao adequada para pratica especifica da unidade. Pode ser reaproveitada futuramente em revisao integrada, simulado ou missao final. Nao realocada nesta etapa.) nao deve ter sido classificada por este arquivo');
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_11',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 11,
    'total de vinculos deve crescer exatamente 11 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_tempos_e_modos_verbais) ===';
