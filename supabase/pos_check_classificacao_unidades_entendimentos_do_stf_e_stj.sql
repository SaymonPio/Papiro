...
-- Entendimentos do STF e STJ (curso_conteudos.id = 80) — gerado
...
-- classificar_questoes_unidades_entendimentos_do_stf_e_stj.sql (a
...
--
-- NOTA: Conteudo integralmente jurisprudencial (SV 11 e Tema 280/RE
-- 603.616, STF). artigos_esperados contem apenas "art. 5º, XI" (unico
-- dispositivo normativo efetivamente interpretado por Q253/Q254); SV
-- 11 (Q146) fica somente em documentacao, sem artigo artificial.
-- LV/LVI nao incluidos (microchecagem confirmou ausencia de cobranca
-- autonoma).

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 80, com
--    titulo/escopo/artigos aplicados pela curadoria. Esperado: 1 linha(s),
--    ordem 1, artigos_esperados = ["art. 5º, XI"], ativa.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 80
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 80 aponta para questao
...
--    572c64ec-c7bb-412e-ab3d-94435ed7df12): curso_conteudos=93,
...
-- 8) Conteudos 69 (Jurisprudencia do STF e STJ, ja concluido) e 76
--    (Direitos fundamentais, ja concluido) permanecem intocados por esta
--    operacao — sobreposicao normativa/tematica documentada, nao
--    duplicata. Esperado (baseline ao vivo em 2026-08-22, antes desta
--    operacao): unidades_conteudo_69=1, vinculos_conteudo_69=5,
--    unidades_conteudo_76=1, vinculos_conteudo_76=5.
select
...
-- 9) Confirma exatamente 1 artigo_esperado ("art. 5º, XI"), sem LV/LVI.
--    Esperado: contem_art5_XI=true, contem_LV=false, contem_LVI=false,
--    total=1.
select
  (artigos_esperados @> array['art. 5º, XI']::text[]) as contem_art5_XI,
  exists (select 1 from unnest(artigos_esperados) as a(v) where a.v ilike '%LV%' and a.v not ilike '%XI%') as contem_LV_ou_LVI,
