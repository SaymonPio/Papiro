-- Pos-check SOMENTE LEITURA da classificacao de questoes de Regulamento
-- Disciplinar da Brigada Militar (curso_conteudos.id = 64) — gerado seguindo
-- o mesmo template de scripts/curadoria-pedagogica/gerar-pos-check.mjs. Os
...
-- SUPABASE_SERVICE_ROLE_KEY configurados para o próprio script rodar essa
-- parte sozinho (ver .env.curadoria / env.curadoria.example). A rodar
-- depois de classificar_questoes_unidades_regulamento_disciplinar_da_brigada_militar.sql
...
-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 64, com
...
--    ordem 1 (Regulamento Disciplinar da Brigada Militar) = 6
...
--    Esperado: total_vinculos = 6, questoes_distintas = 6.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 64
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 64 aponta para questao
...
--    454f8501-7818-4dc4-b22a-337247678c58): curso_conteudos=93,
