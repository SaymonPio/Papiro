...
-- questoes de Constituição Federal de 1988 (curso_conteudos.id = 57)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/constituicao_federal_de_1988.unidades.json + config/constituicao_federal_de_1988.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_constituicao_federal_de_1988.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_constituicao_federal_de_1988.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (48, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (111, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (296, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (327, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (647, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (649, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (650, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (651, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (652, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (653, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'media'),
  (716, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (717, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'media'),
  (773, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta'),
  (794, '682804b0-2762-4aff-87f9-d7a5a81757c2', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_57_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 70,
    format('materia_id=%s assunto_id=%s (esperado 10/70)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '682804b0-2762-4aff-87f9-d7a5a81757c2' and curso_conteudo_id = 57 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 57 — depende de curadoria_unidades_constituicao_federal_de_1988.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 57) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 57 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 57)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 57
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  insert into _relatorio values (
    'total_candidatas_14',
    v_total_candidatas = 14,
    format('total_candidatas=%s (esperado 14)', v_total_candidatas)
  );
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 70);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=70', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 14 ativas', v_fora_do_candidato)
  );

  select count(*) into v_unidade_fora
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 57);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 57', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '682804b0-2762-4aff-87f9-d7a5a81757c2';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_constituicao_federal_de_1988) ===';
