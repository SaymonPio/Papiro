-- Pos-check SOMENTE LEITURA da classificacao de questoes de Estatuto dos
-- Militares Estaduais (curso_conteudos.id = 51) — gerado seguindo o mesmo
...
--
-- NOTA: Lei Complementar Estadual nº 10.990/1997, vigente. Os dispositivos
-- efetivamente cobrados pelas questões desta unidade foram verificados
-- individualmente e não foi identificada incompatibilidade com a redação
-- vigente consultada.
--
-- NOTA: redundância temática conhecida e mantida por decisão do usuário:
-- Q20 e Q300 são quase-duplicatas temáticas (mesmo dispositivo, art. 12,
-- caput), e ambas testam a mesma ideia jurídica também coberta pela Q212
-- do curso_conteudo_id 64 (Regulamento Disciplinar da Brigada Militar,
-- Decreto 43.245/2004, art. 3º) — diplomas distintos, nenhuma duplicata
-- de banco.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 51, com
...
--    ordem 1 (Estatuto dos Militares Estaduais) = 5
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 51
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 51 aponta para questao
...
--    bf13f365-3dd9-4d22-9ad7-f369a298eb19): curso_conteudos=93,
