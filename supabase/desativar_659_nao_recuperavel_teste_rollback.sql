...
-- ============================================================================
--
-- Contexto: a questao 659 (fonte declarada "TEC Concursos — questão 3792246
-- — FUNDATEC — posição 254 no conjunto de 1.000 questões") tem registro
-- real de banca/concurso "FUNDATEC — GM (Pref Gravataí)/Pref Gravataí/2026"
-- -- confirmado no proprio banco e preservado sem alteracao por este
-- harness. No momento da decisao de desativar (2026-08-18), o enunciado
-- estava truncado/corrompido (termina em "...que é garantido(" com
-- parenteses nao fechado) e a alternativa A tinha fragmento de numeracao
-- duplicado. Uma investigacao externa anterior, feita por engano contra o
-- concurso errado ("GM Araquari/2026", nao o real "GM Gravataí/2026"), NAO
-- fundamenta esta decisao -- foi descartada por nao ter checado a prova
-- certa. Classificacao operacional definida pelo usuario, 2026-08-18:
-- PROBLEMATICA — NAO_RECUPERAVEL / FONTE_NAO_CONFIRMADA.
--
-- Decisao de produto: NAO reconstruir o enunciado, NAO alterar
...
-- integralmente. A tabela public.questoes nao tem coluna propria de
-- classificacao/motivo (mesma verificacao ja feita para 1337/1340) -- essa
-- classificacao vive apenas nesta documentacao.
--
...
--
-- Escopo estrito: altera SOMENTE `ativa` (+ `atualizado_em` -- sem trigger
-- automatico nesta tabela, setado explicitamente) para exatamente o id
-- 659. Nenhuma outra coluna, nenhuma outra linha, nenhuma tabela
-- relacionada e tocada.
--
-- Sem objetos permanentes: apenas CREATE TEMPORARY TABLE ... ON COMMIT
-- DROP e blocos DO $$ ... $$ inline, mesmo padrao da Fase 3C e da
-- desativacao 1337/1340.
--
-- ESTE ARQUIVO TERMINA SEMPRE EM ROLLBACK.
-- ============================================================================
...
-- Nada commitado: snapshots, staging de asserts e o UPDATE em si -- tudo
-- desfeito abaixo.
ROLLBACK;
