...
-- config/estrutura_e_formacao_de_palavras.mapa.json,
...
--
-- Sexto conteudo de Lingua Portuguesa da fila. artigos_esperados desta
-- unidade e NULL (metodologia nao juridica). 0 exclusoes intencionais
-- — todas as 13 questoes ativas foram classificadas.
--
-- ATENCAO: sobreposicao tematica (nao duplicata) com o conteudo ja
-- concluido 20 (Ortografia)/26 (Acentuação gráfica)/34 (Fonemas e
-- dígrafos)/31 (Tempos e modos verbais) — Q883 reaproveita o artigo
-- "RS Seguro" ja usado la. Os conteudos 20, 22, 26, 31 e 34 nao sao
-- tocados por este arquivo. Q336 permanece com contaminacao de OCR
-- na alternativa E intacta (pendencia OCR_CONTAMINACAO_ALTERNATIVA_Q336,
-- nao saneada neste apply).
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_tempos_e_modos_verbais.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_estrutura_e_formacao_de_palavras.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente 1a6bb75c-23d7-4110-be96-8803cbd85331 — não cria unidade
-- nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (231, '1a6bb75c-23d7-4110-be96-8803cbd85331'::uuid, 1, 'alta'),
  (232, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (233, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (336, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (685, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (753, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (754, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (755, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (807, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (880, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (881, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (882, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
  (883, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 33 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 45 then
    raise exception 'Precondicao falhou: conteudo 33 materia_id=% assunto_id=% (esperado 6/45)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 33) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 33 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '1a6bb75c-23d7-4110-be96-8803cbd85331' and curso_conteudo_id = 33 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_estrutura_e_formacao_de_palavras.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 33
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 31) then
    raise exception 'Precondicao falhou: curso_conteudos 31 (Tempos e modos verbais — origem de Q693/Q893) nao existe mais — verificar integridade';
  end if;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 45);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=45', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 33);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 33', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 33) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 33 != 1';
  end if;
  ...
  where id = '1a6bb75c-23d7-4110-be96-8803cbd85331';
  ...
  -- Pos-condicao extra: Q336 permanece intacta, alternativa E com a
  -- contaminacao de OCR NAO saneada neste apply (pendencia registrada,
  -- nao corrigida agora).
  select a.texto into v_alt_e_336
  from public.alternativas a
  where a.questao_id = 336 and a.ordem = 5;
  if v_alt_e_336 is null or v_alt_e_336 not like 'Inativo.%MATEMÁTICA E RACIOCÍNIO LÓGICO%' then
    raise exception 'Pos-condicao falhou: alternativa E da questao 336 foi alterada — este apply nao deveria tocar em questoes/alternativas, apenas classificar. Texto atual: %', v_alt_e_336;
  end if;

  -- Pos-condicao extra: conteudos 20, 22, 26, 31 e 34 (sobreposicao
  ...
  raise notice 'Pos-condicoes OK: 13 questoes classificadas / 13 vinculos / 0 multiunidade / artigos_esperados NULL / Q336 intacta (OCR nao saneado) / conteudos 20, 22, 26, 31 e 34 intocados.';
end $$;
