...
-- questoes de Coesão textual (curso_conteudos.id = 13)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/coesao_textual.unidades.json + config/coesao_textual.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_coesao_textual.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_coesao_textual.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (69, '29a4bec1-2c3a-40f3-a86f-fa6bda25d04f', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_13_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 55,
    format('materia_id=%s assunto_id=%s (esperado 6/55)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '29a4bec1-2c3a-40f3-a86f-fa6bda25d04f' and curso_conteudo_id = 13 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 13 — depende de curadoria_unidades_coesao_textual.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 13) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 13 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 13)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=55', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_7_distintas',
    v_distintas = 7,
    format('mapa cobre %s questoes distintas (esperado 7)', v_distintas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 13', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 13;
  insert into _relatorio values ('total_vinculos_7', v_total_vinculos = 7, format('total_vinculos=%s (esperado 7)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 13;
  insert into _relatorio values ('questoes_classificadas_7', v_questoes_classificadas = 7, format('questoes_classificadas=%s (esperado 7)', v_questoes_classificadas));
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '29a4bec1-2c3a-40f3-a86f-fa6bda25d04f';
  insert into _relatorio values ('u1_7_questoes', v_qtd_tmp = 7, format('u1_questoes=%s (esperado 7)', v_qtd_tmp));
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_7',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 7,
    'total de vinculos deve crescer exatamente 7 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_coesao_textual) ===';
