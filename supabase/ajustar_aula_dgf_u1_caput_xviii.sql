-- AJUSTE CIRURGICO DO RASCUNHO — unidade "Direitos Individuais e Coletivos
-- Fundamentais" (BM RS), primeira geração assíncrona real (Fase 5-6).
--
...
--      a explicação listava só "vida, liberdade e igualdade" como
--      invioláveis; o caput real do art. 5º enumera CINCO direitos
--      (vida, liberdade, igualdade, segurança e propriedade). Corrigido
--      para a enumeração completa, removendo o "entre outros" (que deixa
--      de ser necessário/preciso uma vez completa a lista).
--
--   2. componente "Liberdade de associação" (XVII/XVIII/XXI): a
--      explicação do inciso XVIII omitia a cláusula "e, na forma da lei,
--      a de cooperativas" (o texto constitucional real dispensa de
--      autorização tanto a criação de associações quanto, na forma da
--      lei, a de cooperativas). Acrescentada essa cláusula, preservando
--      integralmente o resto do parágrafo (vedação de interferência
--      estatal) e do componente (representação de filiados, XXI).
--
-- NAO existe RPC/admin-flow canonico para editar o conteudo de uma
-- aula_versoes ja existente (mesmo achado já documentado em
-- supabase/ajustar_aula_rascunho_lei_tortura.sql: só existem
-- carregar_aula_rascunho_admin [leitura] e publicar_aula_versao_admin
-- [status] — nenhuma função de edição de conteúdo). Por isso este ajuste
-- usa UPDATE direto, escopado a UMA ÚNICA linha por aula_versao_id exato,
-- seguindo o MESMO padrão já estabelecido e aplicado com sucesso no
-- projeto (Lote A1, Lei de Tortura).
--
-- Os outros 11 componentes, a fonte vinculada, artigos_abordados,
-- numero_versao e status permanecem byte-a-byte idênticos — só as 2
-- strings de "explicacao" indicadas acima mudam, dentro do mesmo objeto
-- JSON, preservando todos os ids/ordem/demais campos.
--
-- NAO altera status (permanece 'rascunho'), NAO publica, NAO toca
-- aula_versao_fontes/aulas/unidades_pedagogicas/curso_conteudos ou
-- qualquer outra aula/aula_versao.
--
-- Alvo exato: aula_versao_id = '52756262-8be4-4fb2-a9f3-2f5a158ea1d4'
-- (unidade 0c5d1d64-0cae-406e-be19-b03d387bee8a, "Direitos Individuais e
-- Coletivos Fundamentais", rascunho v1, geração e1b5f69e-bd01-4c97-ad56-
-- 93fee93a6348).
--
-- ROLLBACK-TESTADO em ajustar_aula_dgf_u1_caput_xviii_teste_rollback.sql
...
begin
  select status, jsonb_array_length(estrutura->'componentes'),
         ...
         estrutura::text
  into v_status, v_qtd_componentes, v_tipos, v_texto_completo
  ...
  if position('vida, liberdade, igualdade, segurança e propriedade' in v_texto_caput) = 0 then
    raise exception 'POSCOND: enumeracao completa do caput nao encontrada no componente esperado';
  end if;
  ...
  if position('e, na forma da lei, a de cooperativas' in v_texto_xviii) = 0 then
    raise exception 'POSCOND: clausula de cooperativas nao encontrada no componente XVIII esperado';
  end if;
  ...
  if jsonb_array_length((select estrutura->'artigos_abordados' from public.aula_versoes where id = '52756262-8be4-4fb2-a9f3-2f5a158ea1d4'::uuid)) <> 14 then
    raise exception 'POSCOND: artigos_abordados deveria continuar com 14 itens (inalterado por este patch, so o texto de 2 componentes mudou)';
  end if;
  ...
  raise notice 'POSCOND OK: status=rascunho preservado, 13 componentes, 5 tipos obrigatorios presentes, caput e XVIII corrigidos, artigos_abordados inalterado (14), nenhuma outra aula_versao tocada';
end $$;
