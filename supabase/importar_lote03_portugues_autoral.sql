-- IMPORTACAO LOTE03_PORTUGUES_AUTORAL — 28 questoes autorais para 4 unidades de
-- Lingua Portuguesa (curso Brigada Militar RS): Coordenacao e subordinacao (+7),
-- Pontuacao (+7), Denotacao e conotacao (+7), Colocacao pronominal (+7).
--
-- Human sign-off explicito, por unidade (Etapas 1.1/2.1/3.1/4.1desta fase).
-- Reservas NAO promovidas (64f5f0a5...q02, 73e0b76f...q02, e7e762c0...q02,
-- 7c5a7a70...q02) NAO importadas nesta fase — permanecem fora do banco.
--
-- Padrao identico ao lote precedente (LOTE02_PORTUGUES_AUTORAL): staging de
-- questoes/explicacoes/alternativas, insercao por INSERT direto em
...
--
-- Texto final (enunciado/alternativas/explicacao/fundamento) e o texto
-- pos-edicao humana persistido em
-- outputs/curadoria-autoral/review/LOTE03-PORTUGUES-HUMAN-REVIEW.md,
-- congelado em outputs/curadoria-autoral/review/LOTE03-FROZEN-28.json
-- (corpus_hash 0593358b8021009acce03c016330e41850bb6d498c1257da167309555e12091a).
--
-- ROLLBACK-TESTADO em importar_lote03_portugues_autoral_teste_rollback.sql
-- REVERSAO REAL disponivel em reverter_lote03_portugues_autoral.sql
...
begin
  ...
  if v_questoes - v_snap.total_questoes <> 28 then raise exception 'POSCOND: questoes criadas=% esperado 28', v_questoes - v_snap.total_questoes; end if;
  if v_alternativas - v_snap.total_alternativas <> 140 then raise exception 'POSCOND: alternativas criadas=% esperado 140', v_alternativas - v_snap.total_alternativas; end if;
  if v_vinculos - v_snap.total_vinculos <> 28 then raise exception 'POSCOND: vinculos criados=% esperado 28', v_vinculos - v_snap.total_vinculos; end if;
  if v_curso_questoes - v_snap.total_curso_questoes <> 28 then raise exception 'POSCOND: curso_questoes criadas=% esperado 28', v_curso_questoes - v_snap.total_curso_questoes; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='bfeaf283-09fc-4f14-9d0c-41ff0db4eab7' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Coordenação e subordinação=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='bfeaf283-09fc-4f14-9d0c-41ff0db4eab7' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 0 then raise exception 'POSCOND: REAL unidade Coordenação e subordinação=% esperado 0', v_real; end if;
  if v_autoral <> 10 then raise exception 'POSCOND: AUTORAL unidade Coordenação e subordinação=% esperado 10', v_autoral; end if;
  ...
  if v_gabaritos <> 'ACEEBDB' then raise exception 'POSCOND: gabaritos unidade Coordenação e subordinação=% esperado ACEEBDB', v_gabaritos; end if;
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Pontuação=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='dca4fe2e-50e9-41db-abeb-0ef6b388c5af' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 0 then raise exception 'POSCOND: REAL unidade Pontuação=% esperado 0', v_real; end if;
  if v_autoral <> 10 then raise exception 'POSCOND: AUTORAL unidade Pontuação=% esperado 10', v_autoral; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = 'dca4fe2e-50e9-41db-abeb-0ef6b388c5af';
  if v_gabaritos <> 'ACCBBDD' then raise exception 'POSCOND: gabaritos unidade Pontuação=% esperado ACCBBDD', v_gabaritos; end if;
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Denotação e conotação=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='f1377f8b-348e-4da6-9713-e901ce5ea516' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 0 then raise exception 'POSCOND: REAL unidade Denotação e conotação=% esperado 0', v_real; end if;
  if v_autoral <> 10 then raise exception 'POSCOND: AUTORAL unidade Denotação e conotação=% esperado 10', v_autoral; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = 'f1377f8b-348e-4da6-9713-e901ce5ea516';
  if v_gabaritos <> 'AACBBEB' then raise exception 'POSCOND: gabaritos unidade Denotação e conotação=% esperado AACBBEB', v_gabaritos; end if;
  ...
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Colocação pronominal=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='c03b4993-601a-4c1e-b24d-c9e09d01db84' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_real <> 0 then raise exception 'POSCOND: REAL unidade Colocação pronominal=% esperado 0', v_real; end if;
  if v_autoral <> 10 then raise exception 'POSCOND: AUTORAL unidade Colocação pronominal=% esperado 10', v_autoral; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = 'c03b4993-601a-4c1e-b24d-c9e09d01db84';
  if v_gabaritos <> 'AAABBAA' then raise exception 'POSCOND: gabaritos unidade Colocação pronominal=% esperado AAABBAA', v_gabaritos; end if;
  ...
  if v_vinc_ok <> 28 then raise exception 'POSCOND: %/28 vinculos corretos (unidade unica) confirmados', v_vinc_ok; end if;
  ...
  if v_cq_ok <> 28 then raise exception 'POSCOND: %/28 curso_questoes novas confirmadas', v_cq_ok; end if;

  raise notice 'POSCONDICOES OK: +28 questoes, +140 alternativas, +28 vinculos, +28 curso_questoes; todas as 4 unidades com gabaritos e contagens confirmadas';
end $$;
