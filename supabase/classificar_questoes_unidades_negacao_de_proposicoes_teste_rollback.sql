...
-- questoes de Negação de proposições (curso_conteudos.id = 3)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/negacao_de_proposicoes.unidades.json + config/negacao_de_proposicoes.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_negacao_de_proposicoes.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_negacao_de_proposicoes.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (81, 'c6ccefae-14df-4760-8c1d-2822090a2a93', 1, 'alta'),
  (86, 'c6ccefae-14df-4760-8c1d-2822090a2a93', 1, 'alta'),
  (312, 'c6ccefae-14df-4760-8c1d-2822090a2a93', 1, 'alta'),
  (337, 'c6ccefae-14df-4760-8c1d-2822090a2a93', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_3_materia_assunto',
    v_materia_id is not distinct from 18 and v_assunto_id is not distinct from 35,
    format('materia_id=%s assunto_id=%s (esperado 18/35)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'c6ccefae-14df-4760-8c1d-2822090a2a93' and curso_conteudo_id = 3 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 3 — depende de curadoria_unidades_negacao_de_proposicoes.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 3) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 3 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 3)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=18/assunto_id=35', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 3', v_unidade_fora)
  );
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_negacao_de_proposicoes) ===';
