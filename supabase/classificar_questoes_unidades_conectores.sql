...
--
-- Oitavo conteudo de Lingua Portuguesa da fila. artigos_esperados desta
...
-- concluido 13 (Coesão textual) — Q67 e Q316 reaproveitam textos-base ja
-- usados por Q69 e Q319 respectivamente, testando aqui valor semantico
-- de conectores, nao referenciacao. O conteudo 13 nao e tocado por este
-- arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_coesao_textual.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_conectores.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente d1e31767-d27d-431b-ba59-7a2008c7473d — não cria unidade
-- nova).
begin;
...
select
  (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 13) as unidades_13,
  (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 13) as vinculos_13;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (67, 'd1e31767-d27d-431b-ba59-7a2008c7473d'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 14 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 57 then
    raise exception 'Precondicao falhou: conteudo 14 materia_id=% assunto_id=% (esperado 6/57)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 14) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 14 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'd1e31767-d27d-431b-ba59-7a2008c7473d' and curso_conteudo_id = 14 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_conectores.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 14
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 13) then
    raise exception 'Precondicao falhou: curso_conteudos 13 (Coesão textual, sobreposicao tematica) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 57);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=57', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 14);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 14', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 14) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 14 != 1';
  end if;
  ...
  where id = 'd1e31767-d27d-431b-ba59-7a2008c7473d';
  ...
  -- Pos-condicao extra: conteudo 13 (sobreposicao tematica) permanece intocado.
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 13) <> (select unidades_13 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 13 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 13) <> (select vinculos_13 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 13 mudou — nao deveria ser tocado';
  end if;

  raise notice 'Pos-condicoes OK: 5 questoes classificadas / 5 vinculos / 0 multiunidade / artigos_esperados NULL / conteudo 13 intocado.';
end $$;
