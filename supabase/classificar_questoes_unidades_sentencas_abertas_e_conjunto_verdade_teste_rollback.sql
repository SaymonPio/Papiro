...
-- questoes de Sentenças abertas e conjunto-verdade (curso_conteudos.id = 9)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/sentencas_abertas_e_conjunto_verdade.unidades.json + config/sentencas_abertas_e_conjunto_verdade.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_sentencas_abertas_e_conjunto_verdade.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_sentencas_abertas_e_conjunto_verdade.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (78, '4ed265ff-578a-4462-bce6-d756b8ad5838', 1, 'alta'),
  (289, '4ed265ff-578a-4462-bce6-d756b8ad5838', 1, 'alta'),
  (290, '4ed265ff-578a-4462-bce6-d756b8ad5838', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_9_materia_assunto',
    v_materia_id is not distinct from 18 and v_assunto_id is not distinct from 32,
    format('materia_id=%s assunto_id=%s (esperado 18/32)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '4ed265ff-578a-4462-bce6-d756b8ad5838' and curso_conteudo_id = 9 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 9 — depende de curadoria_unidades_sentencas_abertas_e_conjunto_verdade.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 9) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 9 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 9)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 9
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 18 and q.assunto_id = 32);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=18/assunto_id=32', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 9);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 9', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '4ed265ff-578a-4462-bce6-d756b8ad5838';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_sentencas_abertas_e_conjunto_verdade) ===';
