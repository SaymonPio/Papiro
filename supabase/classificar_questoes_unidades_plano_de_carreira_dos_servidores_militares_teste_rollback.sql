...
-- questoes de Plano de Carreira dos Servidores Militares (curso_conteudos.id = 65)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/plano_de_carreira_dos_servidores_militares.unidades.json + config/plano_de_carreira_dos_servidores_militares.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_plano_de_carreira_dos_servidores_militares.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_plano_de_carreira_dos_servidores_militares.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (201, '9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200', 1, 'alta'),
  (202, '9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200', 1, 'alta'),
  (203, '9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200', 1, 'alta'),
  (364, '9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_65_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 77,
    format('materia_id=%s assunto_id=%s (esperado 10/77)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200' and curso_conteudo_id = 65 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 65 — depende de curadoria_unidades_plano_de_carreira_dos_servidores_militares.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 65) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 65 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 65)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 65
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 77);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=77', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 65);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 65', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200';
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_4',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 4,
    'total de vinculos deve crescer exatamente 4 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_plano_de_carreira_dos_servidores_militares) ===';
