...
-- questoes de Estatuto dos Militares Estaduais (curso_conteudos.id = 51)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/estatuto_dos_militares_estaduais.unidades.json + config/estatuto_dos_militares_estaduais.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_estatuto_dos_militares_estaduais.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_estatuto_dos_militares_estaduais.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (20, 'bf13f365-3dd9-4d22-9ad7-f369a298eb19', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_51_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 18,
    format('materia_id=%s assunto_id=%s (esperado 10/18)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'bf13f365-3dd9-4d22-9ad7-f369a298eb19' and curso_conteudo_id = 51 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 51 — depende de curadoria_unidades_estatuto_dos_militares_estaduais.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 51) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 51 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 51)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=18', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 51', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'bf13f365-3dd9-4d22-9ad7-f369a298eb19';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_estatuto_dos_militares_estaduais) ===';
