...
-- questoes de Abuso de Autoridade (curso_conteudos.id = 62)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/abuso_de_autoridade.unidades.json + config/abuso_de_autoridade.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_abuso_de_autoridade.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_abuso_de_autoridade.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (192, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (193, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (194, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (712, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (713, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (771, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (772, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (793, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (837, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (838, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta'),
  (839, '7f7298d6-c0b8-4907-8f72-1072a0796a5f', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_62_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 79,
    format('materia_id=%s assunto_id=%s (esperado 10/79)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '7f7298d6-c0b8-4907-8f72-1072a0796a5f' and curso_conteudo_id = 62 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 62 — depende de curadoria_unidades_abuso_de_autoridade.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 62) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 62 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 62)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 62
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 10 and q.assunto_id = 79);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=79', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 62);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 62', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '7f7298d6-c0b8-4907-8f72-1072a0796a5f';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_abuso_de_autoridade) ===';
