-- TESTE DE ROLLBACK do Lote DH-AUT-06 de Direitos Humanos e Cidadania — 22
-- questoes AUTORAL_PAPIRO + 110 alternativas + 22 vinculos (cc92,93,94,95).
--
-- Mirror exato de supabase/importar_dh_aut_06.sql, substituindo os
...
-- todos os checks passem.
--
-- Gerado programaticamente a partir do arquivo de aplicacao real (staging
-- de _lote_questoes/_lote_explicacoes/_lote_alternativas extraido verbatim
-- por slice de string), eliminando risco de divergencia por retranscricao
-- manual do corpus canonico.
...
-- Precondicoes.
insert into _relatorio select 'staging_total_22', (select count(*) from _lote_questoes) = 22, (select count(*) from _lote_questoes)::text;
...
insert into _relatorio select 'staging_cc92_6', (select count(*) from _lote_questoes where cc_id=92) = 6, (select count(*) from _lote_questoes where cc_id=92)::text;
insert into _relatorio select 'staging_cc93_5', (select count(*) from _lote_questoes where cc_id=93) = 5, (select count(*) from _lote_questoes where cc_id=93)::text;
insert into _relatorio select 'staging_cc94_6', (select count(*) from _lote_questoes where cc_id=94) = 6, (select count(*) from _lote_questoes where cc_id=94)::text;
insert into _relatorio select 'staging_cc95_5', (select count(*) from _lote_questoes where cc_id=95) = 5, (select count(*) from _lote_questoes where cc_id=95)::text;

insert into _relatorio select 'snapshot_cc92_uteis_4', (select cc92_uteis from _snapshot_antes) = 4, (select cc92_uteis from _snapshot_antes)::text;
insert into _relatorio select 'snapshot_cc93_uteis_5', (select cc93_uteis from _snapshot_antes) = 5, (select cc93_uteis from _snapshot_antes)::text;
insert into _relatorio select 'snapshot_cc94_uteis_4', (select cc94_uteis from _snapshot_antes) = 4, (select cc94_uteis from _snapshot_antes)::text;
insert into _relatorio select 'snapshot_cc95_uteis_5', (select cc95_uteis from _snapshot_antes) = 5, (select cc95_uteis from _snapshot_antes)::text;

insert into _relatorio select 'snapshot_q180_hash_ok', (select q180_hash from _snapshot_antes) = 'c189e7cb437586cc94cae1de61903cae', (select q180_hash from _snapshot_antes);
insert into _relatorio select 'snapshot_q181_hash_ok', (select q181_hash from _snapshot_antes) = '950ddb18cfb2857907c09b7297751cf8', (select q181_hash from _snapshot_antes);
insert into _relatorio select 'snapshot_q182_hash_ok', (select q182_hash from _snapshot_antes) = 'a6033d457b0f9e24528ba2245d3fdb38', (select q182_hash from _snapshot_antes);
insert into _relatorio select 'snapshot_q189_hash_ok', (select q189_hash from _snapshot_antes) = '214e0ec3a8ee94a22e39e16abd5398c5', (select q189_hash from _snapshot_antes);
insert into _relatorio select 'snapshot_q190_hash_ok', (select q190_hash from _snapshot_antes) = '5f0a106956022042a6a212904b4ff65b', (select q190_hash from _snapshot_antes);
insert into _relatorio select 'snapshot_q191_hash_ok', (select q191_hash from _snapshot_antes) = '06d4e174f2db3310fec46b97c14e69d6', (select q191_hash from _snapshot_antes);
...
insert into _relatorio select 'pos_questoes_novas_22',
  (select count(*) from public.questoes where id in (select questao_id from _mapa_ids)) = 22,
...
insert into _relatorio select 'pos_alternativas_110',
  (select count(*) from public.alternativas where questao_id in (select questao_id from _mapa_ids)) = 110,
...
insert into _relatorio select 'pos_dificuldade_5_13_4',
  (select count(*) from public.questoes where id in (select questao_id from _mapa_ids) and dificuldade='facil') = 5
  and (select count(*) from public.questoes where id in (select questao_id from _mapa_ids) and dificuldade='media') = 13
...
insert into _relatorio select 'pos_gabarito_A5_B5_C4_D4_E4',
  (select count(*) from public.alternativas a join _mapa_ids m on a.questao_id=m.questao_id where a.correta and a.ordem=1) = 5
  and (select count(*) from public.alternativas a join _mapa_ids m on a.questao_id=m.questao_id where a.correta and a.ordem=2) = 5
...
insert into _relatorio select 'pos_vinculos_novos_22',
  (select count(*) from public.questao_unidades_pedagogicas where questao_id in (select questao_id from _mapa_ids)) = 22,
...
insert into _relatorio select 'pos_cc92_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=92) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=92)::text;
insert into _relatorio select 'pos_cc93_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=93) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=93)::text;
insert into _relatorio select 'pos_cc94_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=94) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=94)::text;
insert into _relatorio select 'pos_cc95_uteis_10',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=95) = 10,
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id where up.curso_conteudo_id=95)::text;

insert into _relatorio select 'pos_real_inalterado',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=92 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc92_real from _snapshot_antes)
  and (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=93 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc93_real from _snapshot_antes)
  and (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=94 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc94_real from _snapshot_antes)
  and (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id=qup.unidade_pedagogica_id join public.questoes q on q.id=qup.questao_id where up.curso_conteudo_id=95 and coalesce(lower(q.banca),'') not like '%papiro%') = (select cc95_real from _snapshot_antes),
  'ok';

insert into _relatorio select 'pos_dh_uteis_delta_22',
  (select count(distinct qup.questao_id) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas up on up.id = qup.unidade_pedagogica_id join public.curso_conteudos cc on cc.id = up.curso_conteudo_id join public.assuntos a on a.id = cc.assunto_id where a.materia_id = 11) = (select dh_uteis from _snapshot_antes) + 22,
...
insert into _relatorio select 'pos_q180_hash_inalterado', (select md5(coalesce(explicacao,'')) from public.questoes where id = 180) = (select q180_hash from _snapshot_antes), (select md5(coalesce(explicacao,'')) from public.questoes where id = 180);
insert into _relatorio select 'pos_q181_hash_inalterado', (select md5(coalesce(explicacao,'')) from public.questoes where id = 181) = (select q181_hash from _snapshot_antes), (select md5(coalesce(explicacao,'')) from public.questoes where id = 181);
insert into _relatorio select 'pos_q182_hash_inalterado', (select md5(coalesce(explicacao,'')) from public.questoes where id = 182) = (select q182_hash from _snapshot_antes), (select md5(coalesce(explicacao,'')) from public.questoes where id = 182);
insert into _relatorio select 'pos_q189_hash_inalterado', (select md5(coalesce(explicacao,'')) from public.questoes where id = 189) = (select q189_hash from _snapshot_antes), (select md5(coalesce(explicacao,'')) from public.questoes where id = 189);
insert into _relatorio select 'pos_q190_hash_inalterado', (select md5(coalesce(explicacao,'')) from public.questoes where id = 190) = (select q190_hash from _snapshot_antes), (select md5(coalesce(explicacao,'')) from public.questoes where id = 190);
insert into _relatorio select 'pos_q191_hash_inalterado', (select md5(coalesce(explicacao,'')) from public.questoes where id = 191) = (select q191_hash from _snapshot_antes), (select md5(coalesce(explicacao,'')) from public.questoes where id = 191);

insert into _relatorio select 'pos_total_questoes_delta_22', (select count(*) from public.questoes) = (select total_questoes from _snapshot_antes) + 22, (select count(*) from public.questoes)::text;
insert into _relatorio select 'pos_total_alternativas_delta_110', (select count(*) from public.alternativas) = (select total_alternativas from _snapshot_antes) + 110, (select count(*) from public.alternativas)::text;
insert into _relatorio select 'pos_total_vinculos_delta_22', (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 22, (select count(*) from public.questao_unidades_pedagogicas)::text;
