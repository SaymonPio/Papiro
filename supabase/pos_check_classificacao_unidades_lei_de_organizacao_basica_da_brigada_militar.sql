...
-- Organizacao Basica da Brigada Militar (curso_conteudos.id = 63) —
...
-- (a versao que termina em COMMIT, escrita/revisada a parte — ver README
...
--
-- NOTA DE SUCESSAO NORMATIVA: a Lei no 10.991/1997 (citada no enunciado
-- original da Q43) foi revogada pelo art. 47 da Lei Complementar no
-- 16.450/2025 (vigente desde 26/12/2025). O conteudo juridico da Q43
-- (competencia do Chefe do Estado-Maior de assessorar o Comandante-Geral)
-- foi preservado no art. 10 da lei sucessora, unico item em
-- artigos_esperados. A referencia normativa do enunciado da Q43 permanece
-- sinalizada para saneamento textual futuro, fora desta curadoria — a
-- questao e o gabarito NAO foram alterados nesta operacao (consulta 8
-- confirma o enunciado/gabarito intactos).

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 63, com
...
--    ordem 1 (Lei de Organização Básica da Brigada Militar) = 3
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 63
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 63 aponta para questao
...
--    3c033d9a-5543-422a-a935-c55095bdfc86): curso_conteudos=93,
...
-- 8) Confirma que a Q43 permaneceu textualmente intacta (enunciado ainda
--    cita a Lei no 10.991/1997 — sinalizado para saneamento textual
--    futuro fora desta curadoria) e que o gabarito nao foi alterado.
--    Esperado: 1 linha, alt_correta_texto = 'Assessorar o Comandante-Geral.'
select q.id, q.enunciado, a.texto as alt_correta_texto
from public.questoes q
join public.alternativas a on a.questao_id = q.id and a.correta = true
where q.id = 43;
