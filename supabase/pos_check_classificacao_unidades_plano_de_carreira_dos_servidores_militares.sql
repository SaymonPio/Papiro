-- Pos-check SOMENTE LEITURA da classificacao de questoes de Plano de
-- Carreira dos Servidores Militares (curso_conteudos.id = 65) — gerado
...
--
-- NOTA: Lei Complementar Estadual nº 10.992/1997, vigente. Q201/Q202/Q203
-- sao afirmacoes genericas sobre sujeicao a legislacao especifica, sem
-- dispositivo numerado atribuido. Q364 mapeada aos arts. 2º §2º, 3º §1º,
-- 3º §2º, 5º §1º e 5º §2º, confirmados em versao oficial consolidada
-- (atualizada ate a LC 15.454/2020) com texto identico ao original de
-- 1997 — a LC 15.454/2020 so alterou os arts. 13/14/25-A (Pracas), sem
-- impacto sobre os dispositivos aqui testados (estrutura QOEM/QOES).

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 65, com
...
--    ordem 1 (Plano de Carreira dos Servidores Militares) = 4
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 65
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 65 aponta para questao
...
--    9f8a76ec-2c8c-4bd8-9d72-9ecb7218a200): curso_conteudos=93,
