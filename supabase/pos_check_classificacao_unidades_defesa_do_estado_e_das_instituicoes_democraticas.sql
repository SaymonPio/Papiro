-- Pos-check SOMENTE LEITURA da classificacao de questoes de Defesa do
-- Estado e das Instituicoes Democraticas (curso_conteudos.id = 48) —
...
--
-- NOTA: sobreposicao tematica conhecida e mantida por decisao do usuario:
-- Q297 (art. 144, caput) tambem testada no curso_conteudo_id 49
-- (Seguranca publica, questao 19). Nenhuma questao duplicada entre
-- conteudos.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 48, com
...
--    ordem 1 (Defesa do Estado e das Instituições Democráticas) = 6
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 48
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 48 aponta para questao
...
--    ecdb1d62-ef44-4407-b899-85911402bc90): curso_conteudos=93,
