...
-- supabase/mapa_classificacao_lei_drogas.sql, validado pelo harness
-- classificar_questoes_unidades_lei_drogas_teste_rollback.sql (tudo_ok =
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (143, 'ba2341c7-0598-48f1-99dd-484692c1dfdb'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 66 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 78 then
    raise exception 'Precondicao falhou: conteudo 66 materia_id=% assunto_id=% (esperado 10/78)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 66) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 66 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'ba2341c7-0598-48f1-99dd-484692c1dfdb' and curso_conteudo_id = 66 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_lei_drogas.sql precisa ter sido aplicado antes';
  end if;
  ...
  if v_total_candidatas <> 16 then
    raise exception 'Precondicao falhou: total de candidatas ativas = % (esperado 16, incluindo a 674 intencionalmente excluida do mapa)', v_total_candidatas;
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=78', v_invalidas;
  end if;
  ...
  if exists (select 1 from _mapa where questao_id = 674) then
    raise exception 'Mapa invalido: a questao 674 (fora de escopo) nao deveria constar no mapa';
  end if;
  ...
  if v_fora_do_candidato <> 0 then
    raise exception 'Mapa invalido: % linha(s) fora do conjunto candidato de 16 ativas', v_fora_do_candidato;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 66', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if coalesce(v_674_classificada, false) then
    raise exception 'Pos-condicao falhou: a questao 674 foi classificada indevidamente';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 66) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 66 != 1';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 15 questoes / 15 vinculos / 0 multiunidade / 674 permanece nao classificada.';
end $$;
