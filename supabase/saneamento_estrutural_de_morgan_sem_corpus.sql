...
-- em operacao separada — NAO altera ordem-curadoria.json).
--
...
--
-- Reativacao futura (registrado para referencia, nenhuma acao aqui): se
-- surgir corpus genuino de De Morgan, o fluxo esperado e (1) curar/
-- classificar questoes proprias; (2) vincular a unidade apropriada; (3)
-- revisar escopo; (4) unidade ativa=false -> true; (5) conteudo
-- relevante=false -> true; (6) gerar/validar/publicar aula; (7) retornar
-- ao cronograma. Nenhuma reconstrucao destrutiva e necessaria — id,
-- titulo e escopo da unidade permanecem intactos por este saneamento.
--
...
-- vinculos — Negacao de proposicoes permanece com exatamente 7
-- vinculos (Q77, Q81, Q86, Q88, Q311, Q312, Q337).
--
-- Diferenca deste arquivo para o harness de teste
-- (saneamento_estrutural_de_morgan_sem_corpus_teste_rollback.sql, ja
-- executado com tudo_ok = true): termina em COMMIT, e cada precondicao/
...
begin
  ...
  -- passa mais nesse filtro, sem criar nenhuma missao real.
  select exists (
  ...
  raise notice 'Pos-condicoes OK: curso_conteudo 4 (Leis de De Morgan) com relevante_para_preparacao=false, unidade 5ad41c42-25b4-4853-b328-b562a9fc8076 com ativa=false, 0 vinculos, taxonomia/titulo/escopo/artigos_esperados inalterados, assunto 34 com 0 candidatas, Negacao de proposicoes com 7 vinculos intactos, conteudo 4 nao passa mais no filtro do cronograma.';
end $$;
