...
-- questoes de Pacto Internacional sobre Direitos Econômicos, Sociais e Culturais (curso_conteudos.id = 77)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/pacto_internacional_sobre_direitos_economicos_sociais_e_culturais.unidades.json + config/pacto_internacional_sobre_direitos_economicos_sociais_e_culturais.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_pacto_internacional_sobre_direitos_economicos_sociais_e_culturais.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_pacto_internacional_sobre_direitos_economicos_sociais_e_culturais.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (125, 'f33221cc-f53c-4f91-88e4-a6d8440beca0', 1, 'alta'),
  (261, 'f33221cc-f53c-4f91-88e4-a6d8440beca0', 1, 'alta'),
  (262, 'f33221cc-f53c-4f91-88e4-a6d8440beca0', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_77_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 84,
    format('materia_id=%s assunto_id=%s (esperado 11/84)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'f33221cc-f53c-4f91-88e4-a6d8440beca0' and curso_conteudo_id = 77 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 77 — depende de curadoria_unidades_pacto_internacional_sobre_direitos_economicos_sociais_e_culturais.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 77) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 77 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 77)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 77
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 84);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=84', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 77);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 77', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'f33221cc-f53c-4f91-88e4-a6d8440beca0';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_pacto_internacional_sobre_direitos_economicos_sociais_e_culturais) ===';
