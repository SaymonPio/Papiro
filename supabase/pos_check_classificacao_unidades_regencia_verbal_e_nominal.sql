...
-- Regencia verbal e nominal (curso_conteudos.id = 17).
--
...
-- 1) A unidade pedagogica do conteudo 17. Esperado: 1 linha, ordem 1,
...
-- 4) QIDs vinculados exatos. Esperado: 120, 281, 282.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 17
  );
...
-- 8) Confirma REAL x AUTORAL. Esperado: Q120 banca Fundatec; Q281/Q282 banca Papiro.
select q.id, q.banca
from public.questoes q
...
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 17) as artigos_esperados_null,
