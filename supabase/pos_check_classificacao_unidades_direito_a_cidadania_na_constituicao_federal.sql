-- Pos-check SOMENTE LEITURA da classificacao de questoes de Direito à
-- cidadania na Constituição Federal (curso_conteudos.id = 95) — gerado
...
--
-- NOTA: Constituicao Federal de 1988, arts. 1o e 14. Q161 e questao de
-- natureza conceitual (direito de votar), mapeada com confianca media
-- para art. 14, caput. Q694: idade minima constitucional para Prefeito
-- (21 anos, art. 14, SS3o, VI, "c") inalterada pela Lei no 15.230/2025,
-- que so mudou o momento de afericao na Lei no 9.504/1997 (nao coberto
-- por artigos_esperados, pois nao testado pela questao).

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 95, com
...
--    ordem 1 (Direito à cidadania na Constituição Federal) = 5
...
where q.ativa = true
  and q.materia_id = 11
  and q.assunto_id = 100
  and not exists (
    select 1
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 95
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 95 aponta para questao
...
--    c80fe9be-da9c-4e28-b2a1-27a04be17bf7): curso_conteudos=93,
