...
-- supabase/mapa_classificacao_lei_de_organizacao_basica_da_brigada_militar.sql,
...
-- questoes/alternativas/unidades_pedagogicas/aulas/histórico — a Q43
-- permanece textualmente intacta, apenas classificada). Segue o mesmo
-- padrão de supabase/classificar_questoes_unidades_responsabilidade_civil_do_estado.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_lei_de_organizacao_basica_da_brigada_militar.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (43, '3c033d9a-5543-422a-a935-c55095bdfc86'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 63 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 72 then
    raise exception 'Precondicao falhou: conteudo 63 materia_id=% assunto_id=% (esperado 10/72)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 63) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 63 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '3c033d9a-5543-422a-a935-c55095bdfc86' and curso_conteudo_id = 63 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_lei_de_organizacao_basica_da_brigada_militar.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=72', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 63', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 63) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 63 != 1';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade.';
end $$;
