-- IMPORTACAO LOTE04_PORTUGUES_AUTORAL — 25 questoes autorais para 4 unidades de
-- Lingua Portuguesa (curso Brigada Militar RS): Concordancia nominal (+7),
-- Crase (+6), Implicitos e subentendidos (+6), Significacao das palavras (+6).
--
-- Human sign-off explicito, revisao final unica (mandato "PAPIRO — LOTE 04 —
-- HUMAN REVIEW FINAL + IMPORTACAO AUTOMATIZADA"). Nao selecionadas/rejeitadas
-- (7): CONC-06 (reserva), CRASE-01 (reserva), CRASE-06 (REJECT — erro material),
-- IMPL-07, IMPL-08, SIG-06, SIG-07 (nao selecionadas) — NENHUMA importada nesta
-- fase.
--
-- CRASE-08 recebe edicao humana autorizada no comando (ver LOTE04-FROZEN-25.json).
--
-- Padrao identico ao lote precedente (LOTE03_PORTUGUES_AUTORAL): staging de
...
-- outputs/curadoria-autoral/review/LOTE04-PORTUGUES-HUMAN-REVIEW.md,
-- congelado em outputs/curadoria-autoral/review/LOTE04-FROZEN-25.json
-- (corpus_hash 3d372e5836b08c38a50988c4dfc76b70f039748840680137b69e313eb0718388).
--
-- ROLLBACK-TESTADO em importar_lote04_portugues_autoral_teste_rollback.sql
-- REVERSAO REAL disponivel em reverter_lote04_portugues_autoral.sql
...
begin
  ...
  if v_questoes - v_snap.total_questoes <> 25 then raise exception 'POSCOND: questoes criadas=% esperado 25', v_questoes - v_snap.total_questoes; end if;
  if v_alternativas - v_snap.total_alternativas <> 125 then raise exception 'POSCOND: alternativas criadas=% esperado 125', v_alternativas - v_snap.total_alternativas; end if;
  if v_vinculos - v_snap.total_vinculos <> 25 then raise exception 'POSCOND: vinculos criados=% esperado 25', v_vinculos - v_snap.total_vinculos; end if;
  if v_curso_questoes - v_snap.total_curso_questoes <> 25 then raise exception 'POSCOND: curso_questoes criadas=% esperado 25', v_curso_questoes - v_snap.total_curso_questoes; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='9a4936e1-a9a6-452c-9385-d5a5899ae5c5' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Concordância nominal=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='9a4936e1-a9a6-452c-9385-d5a5899ae5c5' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_autoral + v_real <> 10 then raise exception 'POSCOND: uteis(real+autoral) unidade Concordância nominal=% esperado 10', v_autoral + v_real; end if;
  ...
  if v_gabaritos <> 'BDBBAAB' then raise exception 'POSCOND: gabaritos unidade Concordância nominal=% esperado BDBBAAB', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='57bf73b4-ca89-4809-bc8d-7bf1ae7fa4c2' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Crase=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='57bf73b4-ca89-4809-bc8d-7bf1ae7fa4c2' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_autoral + v_real <> 10 then raise exception 'POSCOND: uteis(real+autoral) unidade Crase=% esperado 10', v_autoral + v_real; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '57bf73b4-ca89-4809-bc8d-7bf1ae7fa4c2';
  if v_gabaritos <> 'CBBBBB' then raise exception 'POSCOND: gabaritos unidade Crase=% esperado CBBBBB', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='1a2158e8-f690-43ab-8ca5-051ba1c0fa3e' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Implícitos e subentendidos=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='1a2158e8-f690-43ab-8ca5-051ba1c0fa3e' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_autoral + v_real <> 10 then raise exception 'POSCOND: uteis(real+autoral) unidade Implícitos e subentendidos=% esperado 10', v_autoral + v_real; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '1a2158e8-f690-43ab-8ca5-051ba1c0fa3e';
  if v_gabaritos <> 'BCAAAA' then raise exception 'POSCOND: gabaritos unidade Implícitos e subentendidos=% esperado BCAAAA', v_gabaritos; end if;

  select count(distinct q.id) into v_uteis
  from public.questoes q join public.questao_unidades_pedagogicas qup on qup.questao_id=q.id
  where qup.unidade_pedagogica_id='290650b5-0f55-49e1-871e-932003447e41' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_uteis <> 10 then raise exception 'POSCOND: uteis unidade Significação das palavras=% esperado 10', v_uteis; end if;
  ...
  where qup.unidade_pedagogica_id='290650b5-0f55-49e1-871e-932003447e41' and q.ativa=true
  and exists(select 1 from public.curso_questoes cq where cq.curso_id='7543be16-4c5b-4cb6-8724-8fbdfb96f2d4' and cq.questao_id=q.id);
  if v_autoral + v_real <> 10 then raise exception 'POSCOND: uteis(real+autoral) unidade Significação das palavras=% esperado 10', v_autoral + v_real; end if;

  select string_agg((select chr(64+ordem) from public.alternativas where questao_id=m.questao_id and correta=true), '' order by m.ordem)
    into v_gabaritos from _mapa_ids m where m.unidade_pedagogica_id = '290650b5-0f55-49e1-871e-932003447e41';
  if v_gabaritos <> 'CAABAB' then raise exception 'POSCOND: gabaritos unidade Significação das palavras=% esperado CAABAB', v_gabaritos; end if;
  ...
  if v_vinc_ok <> 25 then raise exception 'POSCOND: %/25 vinculos corretos (unidade unica) confirmados', v_vinc_ok; end if;
  ...
  if v_cq_ok <> 25 then raise exception 'POSCOND: %/25 curso_questoes novas confirmadas', v_cq_ok; end if;

  raise notice 'POSCONDICOES OK: +25 questoes, +125 alternativas, +25 vinculos, +25 curso_questoes; todas as 4 unidades com gabaritos e contagens confirmadas';
end $$;
