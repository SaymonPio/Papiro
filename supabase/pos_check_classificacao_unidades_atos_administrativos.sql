-- Pos-check SOMENTE LEITURA da classificacao de questoes de Atos administrativos
-- (curso_conteudos.id = 54) — gerado seguindo o mesmo template de
...
-- classificar_questoes_unidades_atos_administrativos.sql (a versao
...
-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 54, com
...
--    ordem 1 (Atos administrativos) = 8
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 54
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 54 aponta para questao
...
--    85323ce5-772b-4bf1-bc32-a79e2316158b): curso_conteudos=93,
