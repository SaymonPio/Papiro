-- Pos-check SOMENTE LEITURA da classificacao de questoes do Tratado
-- de Marraqueche (curso_conteudos.id = 94) — gerado seguindo o mesmo
...
-- classificar_questoes_unidades_tratado_de_marraqueche.sql (a versao
...
--
-- NOTA: artigos_esperados combina dois diplomas — art. 3º pertence ao
-- Tratado de Marraqueche (Q189, beneficiarios); art. 5º, §3º pertence a
-- CF/88 (Q190, rito qualificado de EC). Q191 sem artigo (proposito
-- geral do tratado; art. 4º do proprio Tratado documentado apenas
-- como contexto operacional em escopo/_nota, sem entrar em
-- artigos_esperados). Sobreposicao tematica (nao duplicata) com o
-- conteudo 78 (Q127 la menciona o Tratado de Marraqueche).

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 94, com
...
where q.ativa = true
  and q.materia_id = 11
  and q.assunto_id = 105
  and not exists (
    select 1
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 94
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 94 aponta para questao
...
--    389ff0e7-38f5-4fe5-b66d-4ebbbb5ee9bf): curso_conteudos=93,
...
-- 9) Confirma exatamente os 2 artigos_esperados aprovados.
--    Esperado: contem_art3=true, contem_art5_par3=true, total=2.
select
  (artigos_esperados @> array['art. 3º']::text[]) as contem_art3,
