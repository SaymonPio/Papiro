...
-- questoes de Declaração Universal dos Direitos Humanos (curso_conteudos.id = 83)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/declaracao_universal_dos_direitos_humanos.unidades.json + config/declaracao_universal_dos_direitos_humanos.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_declaracao_universal_dos_direitos_humanos.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_declaracao_universal_dos_direitos_humanos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (145, 'e7bef052-b882-4a2f-b1e6-88ce12740c26', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_83_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 88,
    format('materia_id=%s assunto_id=%s (esperado 11/88)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'e7bef052-b882-4a2f-b1e6-88ce12740c26' and curso_conteudo_id = 83 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 83 — depende de curadoria_unidades_declaracao_universal_dos_direitos_humanos.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 83) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 83 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 83)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=88', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 83', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'e7bef052-b882-4a2f-b1e6-88ce12740c26';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_declaracao_universal_dos_direitos_humanos) ===';
