...
-- questoes de Direito à cidadania na Constituição Federal (curso_conteudos.id = 95)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/direito_a_cidadania_na_constituicao_federal.unidades.json + config/direito_a_cidadania_na_constituicao_federal.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_direito_a_cidadania_na_constituicao_federal.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_direito_a_cidadania_na_constituicao_federal.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (159, 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7', 1, 'alta'),
  (160, 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7', 1, 'alta'),
  (161, 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7', 1, 'média'),
  (694, 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7', 1, 'alta'),
  (789, 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_95_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 100,
    format('materia_id=%s assunto_id=%s (esperado 11/100)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7' and curso_conteudo_id = 95 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 95 — depende de curadoria_unidades_direito_a_cidadania_na_constituicao_federal.sql ja aplicado'
  );

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 95) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 95 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 95)));
  else
  ...
  join public.curso_conteudos cc on cc.id = 95
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 100);

  insert into _relatorio values (
    'mapa_todas_questoes_validas',
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=100', v_invalidas)
  );
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 95);
  insert into _relatorio values (
    'mapa_unidades_pertencem_ao_conteudo',
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 95', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'c80fe9be-da9c-4e28-b2a1-27a04be17bf7';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_direito_a_cidadania_na_constituicao_federal) ===';
