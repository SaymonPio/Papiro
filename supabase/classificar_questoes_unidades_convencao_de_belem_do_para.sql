...
-- supabase/mapa_classificacao_convencao_de_belem_do_para.sql,
...
--
-- ATENCAO: relacao apenas institucional/pedagogica (nao duplicata, nao
-- sobreposicao de questoes) com os conteudos ja concluidos 87 (Comissao
-- Interamericana de Direitos Humanos) e 88 (Corte Interamericana de
-- Direitos Humanos) — a Q155 deste conteudo cita esses orgaos apenas
-- como contexto institucional do Sistema Interamericano, sem repetir
-- nenhuma questao ja classificada la. Nenhum deles e tocado por este
-- arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_comissao_interamericana_de_direitos_humanos.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_convencao_de_belem_do_para.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente a2d8b683-1a53-451e-9072-525a147fed01 — não cria unidade nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (153, 'a2d8b683-1a53-451e-9072-525a147fed01'::uuid, 1, 'alta'),
  (154, 'a2d8b683-1a53-451e-9072-525a147fed01', 1, 'alta'),
  (155, 'a2d8b683-1a53-451e-9072-525a147fed01', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 89 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 107 then
    raise exception 'Precondicao falhou: conteudo 89 materia_id=% assunto_id=% (esperado 11/107)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 89) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 89 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'a2d8b683-1a53-451e-9072-525a147fed01' and curso_conteudo_id = 89 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_convencao_de_belem_do_para.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 89
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  if not exists (select 1 from public.curso_conteudos where id = 87) then
    raise exception 'Precondicao falhou: curso_conteudos 87 (relacao institucional Comissao Interamericana) nao existe mais — verificar integridade';
  end if;
  if not exists (select 1 from public.curso_conteudos where id = 88) then
    raise exception 'Precondicao falhou: curso_conteudos 88 (relacao institucional Corte Interamericana) nao existe mais — verificar integridade';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 107);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=107', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 89);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 89', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 89) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 89 != 1';
  end if;
  ...
  -- Pos-condicao extra: conteudos com relacao institucional (87, 88) permanecem intocados.
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 87) <> (select unidades_87 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 87 mudou — nao deveria ser tocado';
  end if;
  if (select count(*) from public.questao_unidades_pedagogicas qup join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id where u.curso_conteudo_id = 87) <> (select vinculos_87 from _snapshot_sobrepostos_antes) then
    raise exception 'Pos-condicao falhou: quantidade de vinculos do conteudo 87 mudou — nao deveria ser tocado';
  end if;
  ...
  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / conteudos 87 e 88 intocados.';
end $$;
