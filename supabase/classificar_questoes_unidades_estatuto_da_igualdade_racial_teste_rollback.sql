...
-- questoes de Estatuto da Igualdade Racial (curso_conteudos.id = 97)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/estatuto_da_igualdade_racial.unidades.json + config/estatuto_da_igualdade_racial.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_estatuto_da_igualdade_racial.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_estatuto_da_igualdade_racial.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (131, '9cc42871-c31c-440a-88cb-32f0d4f232ff', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_97_materia_assunto',
    v_materia_id is not distinct from 11 and v_assunto_id is not distinct from 98,
    format('materia_id=%s assunto_id=%s (esperado 11/98)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '9cc42871-c31c-440a-88cb-32f0d4f232ff' and curso_conteudo_id = 97 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 97 — depende de curadoria_unidades_estatuto_da_igualdade_racial.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 97) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 97 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 97)));
  else
  ...
  insert into _relatorio values (
    'total_candidatas_9',
    v_total_candidatas = 9,
    format('total_candidatas=%s (esperado 9)', v_total_candidatas)
  );
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=11/assunto_id=98', v_invalidas)
  );

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  insert into _relatorio values (
    'mapa_cobre_9_distintas',
    v_distintas = 9,
    format('mapa cobre %s questoes distintas (esperado 9)', v_distintas)
  );
  ...
  insert into _relatorio values (
    ...
    format('%s linha(s) fora do conjunto candidato de 9 ativas', v_fora_do_candidato)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 97', v_unidade_fora)
  );
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 97;
  insert into _relatorio values ('total_vinculos_9', v_total_vinculos = 9, format('total_vinculos=%s (esperado 9)', v_total_vinculos));

  select count(distinct qup.questao_id) into v_questoes_classificadas
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 97;
  insert into _relatorio values ('questoes_classificadas_9', v_questoes_classificadas = 9, format('questoes_classificadas=%s (esperado 9)', v_questoes_classificadas));
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '9cc42871-c31c-440a-88cb-32f0d4f232ff';
  insert into _relatorio values ('u1_9_questoes', v_qtd_tmp = 9, format('u1_questoes=%s (esperado 9)', v_qtd_tmp));
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_9',
    ...
    'total de vinculos deve crescer exatamente 9 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_estatuto_da_igualdade_racial) ===';
