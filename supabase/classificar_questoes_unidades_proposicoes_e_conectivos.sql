...
-- config/proposicoes_e_conectivos.mapa.json,
...
--
-- Segundo conteúdo do bloco final de Raciocínio Lógico/Matemática, após
-- Negação de proposições (ordem 83). artigos_esperados desta unidade é
-- NULL (metodologia não jurídica). 0 exclusões — as 3 questões ativas
-- foram classificadas (2 rows REAL Fundatec + 1 AUTORAL Q313).
--
-- MICROAUDITORIA TAXONÔMICA EXTERNA (executada e aprovada antes deste
-- apply): confirmou que as 3 candidatas permanecem corretamente
-- classificadas em Proposições e conectivos. Registradas, como achado
-- EXTERNO a esta ordem e SEM NENHUMA AÇÃO sobre elas aqui:
-- NOTA_DE_REVISAO_TAXONOMICA_Q314 (conteúdo futuro Tabela-verdade,
-- curso_conteudo_id 2, ordem 85) e NOTA_DE_REVISAO_TAXONOMICA_Q84
-- (conteúdo futuro Condicional e contrapositiva, curso_conteudo_id 6,
-- ordem 88). Nenhuma dessas 2 questões externas é tocada por este
-- arquivo.
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_proposicoes_e_conectivos.sql
-- ja foi aplicado (reaproveita unidade placeholder ja existente
-- 6683c484-74a7-4b07-9cda-1a72190e6445 — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (74, '6683c484-74a7-4b07-9cda-1a72190e6445'::uuid, 1, 'alta'),
  (87, '6683c484-74a7-4b07-9cda-1a72190e6445'::uuid, 1, 'alta'),
  (313, '6683c484-74a7-4b07-9cda-1a72190e6445'::uuid, 1, 'alta');

-- Lock deterministico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 1
order by id
for update;
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 1 nao existe';
  end if;
  if v_materia_id is distinct from 18 or v_assunto_id is distinct from 36 then
    raise exception 'Precondicao falhou: conteudo 1 materia_id=% assunto_id=% (esperado 18/36)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '6683c484-74a7-4b07-9cda-1a72190e6445' and curso_conteudo_id = 1 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_proposicoes_e_conectivos.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 1) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 1 — decisao aprovada foi manter unidade unica';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 1
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  -- Q314/Q84 (notas de revisao taxonomica externas) nao devem ter sido
  -- tocadas: continuam sem vinculo neste conteudo (nao fazem parte do
  -- mapa) e mantem seu assunto_id original.
  if exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 1 and qup.questao_id in (314, 84)
  ) then
    raise exception 'Precondicao falhou: alguma das questoes externas (314/84) ja possui vinculo neste conteudo — nao deveria';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 18 and q.assunto_id = 36);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=18/assunto_id=36', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 1);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 1', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  -- Pos-condicao extra: as 2 questoes externas continuam sem vinculo
  -- neste conteudo e com seu assunto_id original intacto (Q314 em 38,
  -- Q84 em 39) — nenhum saneamento foi feito por este arquivo.
  select count(*) into v_externas_vinculos
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 1 and qup.questao_id in (314, 84);
  if v_externas_vinculos <> 0 then
    raise exception 'Pos-condicao falhou: questoes externas (314/84) ganharam vinculo neste conteudo (%), nao deveriam', v_externas_vinculos;
  end if;
  ...
  if v_assunto_314 is distinct from 38 or v_assunto_84 is distinct from 39 then
    raise exception 'Pos-condicao falhou: assunto_id de questao externa foi alterado indevidamente (314=%,84=%)', v_assunto_314, v_assunto_84;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 1) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 1 != 1';
  end if;
  ...
  where id = '6683c484-74a7-4b07-9cda-1a72190e6445';
  ...
  raise notice 'Pos-condicoes OK: 3 questoes classificadas / 3 vinculos / 0 multiunidade / artigos_esperados NULL / questoes externas (314/84) intactas e sem vinculo neste conteudo.';
end $$;
