...
-- questoes de Estatuto Estadual da Igualdade Racial (curso_conteudos.id = 68)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/estatuto_estadual_da_igualdade_racial.unidades.json + config/estatuto_estadual_da_igualdade_racial.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_estatuto_estadual_da_igualdade_racial.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_estatuto_estadual_da_igualdade_racial.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (132, 'f0f6bae6-df70-450a-b4ee-3d9747962c2e', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_68_materia_assunto',
    v_materia_id is not distinct from 10 and v_assunto_id is not distinct from 63,
    format('materia_id=%s assunto_id=%s (esperado 10/63)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = 'f0f6bae6-df70-450a-b4ee-3d9747962c2e' and curso_conteudo_id = 68 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 68 — depende de curadoria_unidades_estatuto_estadual_da_igualdade_racial.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 68) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 68 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 68)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=10/assunto_id=63', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 68', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = 'f0f6bae6-df70-450a-b4ee-3d9747962c2e';
  ...
  insert into _relatorio values ('vinculos_cresceram_exatamente_10',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 10,
    'total de vinculos deve crescer exatamente 10 em relacao ao snapshot antes');
end $$;
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_estatuto_estadual_da_igualdade_racial) ===';
