...
-- Coordenacao e subordinacao (curso_conteudos.id = 28).
--
...
-- 1) A unidade pedagogica do conteudo 28. Esperado: 1 linha, ordem 1,
...
-- 4) QIDs vinculados exatos. Esperado: 225, 226, 227.
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 28
  );
...
select
  (select artigos_esperados is null from public.unidades_pedagogicas where curso_conteudo_id = 28) as artigos_esperados_null,
