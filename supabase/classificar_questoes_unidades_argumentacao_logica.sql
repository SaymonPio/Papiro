...
-- config/argumentacao_logica.mapa.json, validado pelo harness
-- classificar_questoes_unidades_argumentacao_logica_teste_rollback.sql
-- (tudo_ok = true confirmado antes de rodar este arquivo).
--
-- Nono conteúdo do bloco final de Raciocínio Lógico/Matemática, após
-- Sentenças abertas e conjunto-verdade (ordem 91). artigos_esperados
-- desta unidade é NULL (metodologia não jurídica). 0 exclusões — as 3
-- questões ativas foram classificadas (1 row REAL Fundatec + 2 AUTORAL
-- Q285/Q286). Nenhum saneamento prévio foi necessário nesta ordem —
-- auditoria taxonômica confirmou que as 3 candidatas pertencem
-- genuinamente a este conteúdo, sem sobreposição com Quantificadores,
-- Diagramas lógicos, Tautologia/contradição/contingência ou
-- Proposições e conectivos.
--
-- MICROAUDITORIA EXTERNA (executada antes deste apply): encontrou Q246
-- (assunto 31, Diagramas lógicos, futura ordem 93) e confirmou por
-- contraste direto que sua tarefa é estruturalmente diagrama-primeiro
-- (parte de um diagrama já dado, pede tradução textual da relação
-- gráfica), genuinamente distinta da tarefa premissas-primeiro deste
-- corpus. Q246 permanece corretamente em Diagramas lógicos; nenhuma
-- ação foi tomada sobre ela.
--
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_argumentacao_logica.sql ja
-- foi aplicado (reaproveita unidade placeholder ja existente
-- 5e2d5159-41da-4af7-b75d-4dc21239177d — nao cria unidade nova).
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (90, '5e2d5159-41da-4af7-b75d-4dc21239177d'::uuid, 1, 'alta'),
  (285, '5e2d5159-41da-4af7-b75d-4dc21239177d'::uuid, 1, 'alta'),
  (286, '5e2d5159-41da-4af7-b75d-4dc21239177d'::uuid, 1, 'alta');

-- Lock deterministico das linhas envolvidas antes de revalidar.
select id from public.unidades_pedagogicas
where curso_conteudo_id = 10
order by id
for update;
...
declare
  ...
  v_assunto_246 bigint;
  v_vinculos_246_aqui int;
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 10 nao existe';
  end if;
  if v_materia_id is distinct from 18 or v_assunto_id is distinct from 40 then
    raise exception 'Precondicao falhou: conteudo 10 materia_id=% assunto_id=% (esperado 18/40)', v_materia_id, v_assunto_id;
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '5e2d5159-41da-4af7-b75d-4dc21239177d' and curso_conteudo_id = 10 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_argumentacao_logica.sql precisa ter sido aplicado antes';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 10) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 10 — decisao aprovada foi manter unidade unica';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 10
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
  ...
  -- Q246 (candidata externa examinada pela microauditoria, pertencente
  -- a Diagramas logicos) nao deve ter sido tocada: continua sem
  -- vinculo neste conteudo e mantem seu assunto_id original (31).
  select assunto_id into v_assunto_246 from public.questoes where id = 246;
  if v_assunto_246 is distinct from 31 then
    raise exception 'Precondicao falhou: assunto_id da questao 246 diverge do esperado (31) — atual: %', v_assunto_246;
  end if;

  select count(*) into v_vinculos_246_aqui
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 10 and qup.questao_id = 246;
  if v_vinculos_246_aqui <> 0 then
    raise exception 'Precondicao falhou: questao 246 ja possui vinculo neste conteudo, nao deveria';
  end if;
end $$;
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 18 and q.assunto_id = 40);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=18/assunto_id=40', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 10);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 10', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  -- Pos-condicao extra: Q246 continua fora deste conteudo, com
  -- assunto_id=31 intacto — este arquivo nao a toca.
  select assunto_id into v_assunto_246 from public.questoes where id = 246;
  if v_assunto_246 is distinct from 31 then
    raise exception 'Pos-condicao falhou: assunto_id da questao 246 foi alterado indevidamente (%), esperado 31', v_assunto_246;
  end if;

  select count(*) into v_vinculos_246
  from public.questao_unidades_pedagogicas qup
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 10 and qup.questao_id = 246;
  if v_vinculos_246 <> 0 then
    raise exception 'Pos-condicao falhou: questao 246 ganhou vinculo neste conteudo, nao deveria';
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 10) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 10 != 1';
  end if;
  ...
  where id = '5e2d5159-41da-4af7-b75d-4dc21239177d';
  ...
  raise notice 'Pos-condicoes OK: 3 questoes classificadas (Q90/Q285/Q286) / 3 vinculos / 0 multiunidade / artigos_esperados NULL / questao 246 intacta e sem vinculo neste conteudo.';
end $$;
