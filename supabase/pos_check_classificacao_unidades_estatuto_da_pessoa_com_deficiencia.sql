...
-- Pessoa com Deficiencia (curso_conteudos.id = 75) — gerado seguindo o
...
-- somente SELECT) porque este ambiente local não tinha SUPABASE_URL/
...
--
-- NOTA: Lei no 13.146/2015, dispositivos substantivos proprios (arts.
-- 3o, XIII; 6o, caput e I; 84, SS3o; 85, caput e SS1o). NAO confundir
-- com curso_conteudo_id 71 (ja concluido), que cita a Lei 13.146/2015
-- apenas como lei alteradora da Lei 7.853/1989 — consulta 8 confirma
-- que o conteudo 71 permanece intocado.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 75, com
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 75
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 75 aponta para questao
...
--    128d9183-6188-4ca1-abdb-fc5c57e78d28): curso_conteudos=93,
...
-- 8) Conteudo 71 (Pessoa com deficiencia, ja concluido) permanece
--    intocado por esta operacao. Esperado (baseline ao vivo em
--    2026-08-22, antes desta operacao): unidades_conteudo_71=1,
--    vinculos_conteudo_71=8.
select
...
-- 9) Confirma exatamente os 6 artigos_esperados aprovados.
--    Esperado: total_artigos_esperados=6, todos os campos true.
select
  (artigos_esperados @> array['art. 3º, XIII']::text[]) as contem_art3_XIII,
  (artigos_esperados @> array['art. 6º, caput']::text[]) as contem_art6_caput,
  (artigos_esperados @> array['art. 6º, I']::text[]) as contem_art6_I,
  (artigos_esperados @> array['art. 84, §3º']::text[]) as contem_art84_p3,
  (artigos_esperados @> array['art. 85, caput']::text[]) as contem_art85_caput,
  (artigos_esperados @> array['art. 85, §1º']::text[]) as contem_art85_p1,
