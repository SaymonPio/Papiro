...
-- supabase/mapa_classificacao_acentuacao_grafica.sql,
...
--
-- Terceiro conteudo de Lingua Portuguesa da fila. artigos_esperados
-- desta unidade e NULL (metodologia nao juridica consolidada no
-- piloto de Ortografia, conteudo 20). 0 exclusoes intencionais.
--
-- ATENCAO: sobreposicao tematica (nao duplicata) com o conteudo ja
-- concluido 20 (Ortografia) — Q745 (item III, 'difícil') e Q747 (item
-- I, 'escalável') testam a mesma regra de paroxítona terminada em L
-- ja usada la para 'lavável' (Q786). Os conteudos 20 e 22 (Classes de
-- palavras, tambem ja concluido) nao sao tocados por este arquivo.
--
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_classes_de_palavras.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_acentuacao_grafica.sql
-- precisa já ter sido aplicado (reaproveita unidade placeholder já
-- existente d100427d-d567-43e8-8ec8-81d44e5e3afe — não cria unidade
-- nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (216, 'd100427d-d567-43e8-8ec8-81d44e5e3afe'::uuid, 1, 'alta'),
  (217, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (218, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (679, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (680, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (681, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (744, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (745, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (746, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (747, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (748, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (872, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (873, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (874, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta'),
  (875, 'd100427d-d567-43e8-8ec8-81d44e5e3afe', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 26 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 44 then
    raise exception 'Precondicao falhou: conteudo 26 materia_id=% assunto_id=% (esperado 6/44)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 26) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 26 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'd100427d-d567-43e8-8ec8-81d44e5e3afe' and curso_conteudo_id = 26 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_acentuacao_grafica.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 26
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 6 and q.assunto_id = 44);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=44', v_invalidas;
  end if;

  select count(*) into v_distintas from (select distinct questao_id from _mapa) x;
  if v_distintas <> 15 then
    raise exception 'Mapa invalido: cobre % questoes distintas (esperado 15)', v_distintas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 26);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 26', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 26;
  if v_total_vinculos <> 15 then
    raise exception 'Pos-condicao falhou: total_vinculos=% (esperado 15)', v_total_vinculos;
  end if;
  ...
  join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
  where u.curso_conteudo_id = 26;
  if v_questoes_classificadas <> 15 then
    raise exception 'Pos-condicao falhou: questoes_classificadas=% (esperado 15)', v_questoes_classificadas;
  end if;
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 26) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 26 != 1';
  end if;
  ...
  if (select count(*) from public.questao_unidades_pedagogicas) <> (select total_vinculos from _snapshot_antes) + 15 then
    raise exception 'Pos-condicao falhou: total de vinculos nao cresceu exatamente 15';
  end if;

  -- Pos-condicao extra: artigos_esperados deve ser NULL.
  select artigos_esperados into v_artigos_esperados
  from public.unidades_pedagogicas
  where id = 'd100427d-d567-43e8-8ec8-81d44e5e3afe';
  ...
  -- Pos-condicao extra: conteudos 20 e 22 (sobreposicao tematica) permanecem intocados.
  ...
  raise notice 'Pos-condicoes OK: 15 questoes classificadas / 15 vinculos / 0 multiunidade / artigos_esperados NULL / conteudos 20 e 22 intocados.';
end $$;
