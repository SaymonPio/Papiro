...
-- supabase/mapa_classificacao_organizacao_dos_estados_americanos.sql,
...
-- conteudo ja concluido 85 (Sistema Interamericano de Direitos
-- Humanos), e com o futuro conteudo 87 (Comissao Interamericana de
-- Direitos Humanos, ainda pendente, ordem 45 — NAO antecipado nem
...
-- mesmo padrão de
-- supabase/classificar_questoes_unidades_sistema_interamericano_de_direitos_humanos.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_organizacao_dos_estados_americanos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (171, '7a9e1731-626a-44cc-90f0-004faed11e0f'::uuid, 1, 'alta'),
  (172, '7a9e1731-626a-44cc-90f0-004faed11e0f', 1, 'alta'),
  (173, '7a9e1731-626a-44cc-90f0-004faed11e0f', 1, 'alta');
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 86 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 87 then
    raise exception 'Precondicao falhou: conteudo 86 materia_id=% assunto_id=% (esperado 11/87)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 86) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 86 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '7a9e1731-626a-44cc-90f0-004faed11e0f' and curso_conteudo_id = 86 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_organizacao_dos_estados_americanos.sql precisa ter sido aplicado antes';
  end if;

  select count(*) into v_total_candidatas
  from public.questoes q
  join public.curso_conteudos cc on cc.id = 86
  join public.curso_materias cm on cm.id = cc.curso_materia_id
  where q.ativa = true
...
begin
  ...
  where not (q.ativa = true and q.materia_id = 11 and q.assunto_id = 87);

  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=87', v_invalidas;
  end if;
  ...
  from _mapa m
  where m.unidade_pedagogica_id not in (select id from public.unidades_pedagogicas where curso_conteudo_id = 86);
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 86', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 86) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 86 != 1';
  end if;
  ...
  -- Pos-condicao extra: conteudos sobrepostos (85 e 87) permanecem intocados.
  ...
  raise notice 'Pos-condicoes OK: 3 questoes / 3 vinculos / 0 multiunidade / conteudos 85 e 87 intocados.';
end $$;
