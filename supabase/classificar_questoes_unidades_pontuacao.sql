...
--
-- Décimo quinto conteúdo de Língua Portuguesa da fila. artigos_esperados
-- desta unidade é NULL (metodologia não jurídica). 0 exclusões
-- intencionais — todas as 3 questões ativas foram classificadas. CASO
-- ATÍPICO: as 3 questões são integralmente AUTORAL_PAPIRO (0 questões
-- REAIS neste corpus) — GAP_DE_BANCO_PONTUACAO quantitativo real
-- extremo, confirmado por microauditoria global prévia (nenhuma
-- candidata REAL encontrada em outro assunto, nenhum problema de
-- taxonomia/duplicação/fidelidade explicando a ausência).
--
...
-- mesmo padrão de supabase/classificar_questoes_unidades_interpretacao_de_textos.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_pontuacao.sql precisa já
-- ter sido aplicado (reaproveita unidade placeholder já existente
-- dca4fe2e-50e9-41db-abeb-0ef6b388c5af — não cria unidade nova).
begin;
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (237, 'dca4fe2e-50e9-41db-abeb-0ef6b388c5af'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 16 nao existe';
  end if;
  if v_materia_id is distinct from 6 or v_assunto_id is distinct from 48 then
    raise exception 'Precondicao falhou: conteudo 16 materia_id=% assunto_id=% (esperado 6/48)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 16) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 16 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = 'dca4fe2e-50e9-41db-abeb-0ef6b388c5af' and curso_conteudo_id = 16 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_pontuacao.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=6/assunto_id=48', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 16', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 16) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 16 != 1';
  end if;
  ...
  where id = 'dca4fe2e-50e9-41db-abeb-0ef6b388c5af';
