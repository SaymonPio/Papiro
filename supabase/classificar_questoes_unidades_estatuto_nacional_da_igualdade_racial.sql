...
-- supabase/mapa_classificacao_estatuto_nacional_da_igualdade_racial.sql,
...
-- mesmo padrão de supabase/classificar_questoes_unidades_estatuto_dos_militares_estaduais.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_estatuto_nacional_da_igualdade_racial.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (54, '5bf890e1-8e09-4f7a-9410-7f5e5168d9c4'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 67 nao existe';
  end if;
  if v_materia_id is distinct from 10 or v_assunto_id is distinct from 75 then
    raise exception 'Precondicao falhou: conteudo 67 materia_id=% assunto_id=% (esperado 10/75)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 67) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 67 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '5bf890e1-8e09-4f7a-9410-7f5e5168d9c4' and curso_conteudo_id = 67 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_estatuto_nacional_da_igualdade_racial.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=10/assunto_id=75', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 67', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 67) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 67 != 1';
  end if;
