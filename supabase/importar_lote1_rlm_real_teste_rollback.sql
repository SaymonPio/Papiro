-- Harness de teste (SEMPRE termina em ROLLBACK) da importacao do Lote 1 REAL
-- de Raciocinio Logico — 8 questoes novas + 40 alternativas + 8 vinculos.
--
...
-- nao aparece em nenhuma linha abaixo.
--
...
-- copiados do relatorio do Gemini) durante a auditoria desta sessao; a
-- questao de Viamao (Tautologia) corrige explicitamente um erro do Gemini
-- sobre a alternativa E (contingencia, nao contradicao).
--
-- Usa a RPC administrativa oficial classificar_questao_unidade_admin para
-- os vinculos (nenhum INSERT direto em questao_unidades_pedagogicas), no
-- mesmo padrao ja usado em supabase/importar_lote1_lei_maria_penha.sql.
--
...
-- ----------------------------------------------------------------------------
-- Staging das 8 questoes (chave local = ordem, nunca usada como id real).
-- ----------------------------------------------------------------------------
...
begin
  ...
  insert into _relatorio values ('staging_8_questoes', v_cnt = 8, format('staging tem %s questoes (esperado 8)', v_cnt));

  select count(*) into v_dup
  from _lote_questoes lq
  where exists (
    select 1 from public.questoes q
    where q.enunciado = lq.enunciado
  );
...
declare
  ...
  v_nao_ativas int;
  v_nao_real int;
  v_antigas_alteradas int;
begin
  select count(*) into v_novas_questoes from public.questoes where id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('8_questoes_novas_inseridas', v_novas_questoes = 8, format('questoes novas=%s (esperado 8)', v_novas_questoes));

  select count(*) into v_novas_alternativas from public.alternativas where questao_id in (select questao_id from _mapa_ids);
  insert into _relatorio values ('40_alternativas_inseridas', v_novas_alternativas = 40, format('alternativas novas=%s (esperado 40)', v_novas_alternativas));
  ...
  insert into _relatorio values ('exatamente_1_correta_por_questao', v_corretas_por_questao = 0, format('%s questao(oes) sem exatamente 1 alternativa correta', v_corretas_por_questao));
  ...
  insert into _relatorio values ('8_vinculos_criados', v_novos_vinculos = 8, format('vinculos novos=%s (esperado 8)', v_novos_vinculos));
  ...
  insert into _relatorio values ('todas_ativas', v_nao_ativas = 0, format('%s questao(oes) nao ativas', v_nao_ativas));
  ...
  insert into _relatorio values ('origem_real_banca_fundatec', v_nao_real = 0, format('%s questao(oes) com banca papiro (esperado 0, todas Fundatec)', v_nao_real));

  select count(*) into v_antigas_alteradas
  from public.questoes q
  where q.materia_id = 18 and q.id not in (select questao_id from _mapa_ids)
    and q.atualizado_em > (select min(criado_em) from public.questoes where id in (select questao_id from _mapa_ids));
  insert into _relatorio values ('34_questoes_antigas_nao_tocadas', v_antigas_alteradas = 0, format('%s questao(oes) antiga(s) de RLM parecem ter sido tocadas', v_antigas_alteradas));

  insert into _relatorio values ('total_questoes_cresceu_8',
    (select count(*) from public.questoes) = (select total_questoes from _snapshot_antes) + 8, 'contagem total de questoes deve crescer exatamente 8');
  insert into _relatorio values ('total_alternativas_cresceu_40',
    (select count(*) from public.alternativas) = (select total_alternativas from _snapshot_antes) + 40, 'contagem total de alternativas deve crescer exatamente 40');
  insert into _relatorio values ('total_vinculos_cresceu_8',
    (select count(*) from public.questao_unidades_pedagogicas) = (select total_vinculos from _snapshot_antes) + 8, 'contagem total de vinculos deve crescer exatamente 8');
  ...
  insert into _relatorio values ('curso_questoes_inalterado',
    (select count(*) from public.curso_questoes) = (select total_curso_questoes from _snapshot_antes), 'curso_questoes nao deve sofrer alteracao (fora do escopo deste lote)');
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (importar_lote1_rlm_real) ===';
