...
-- Humanos no Mercosul (curso_conteudos.id = 91) — gerado seguindo o
...
-- somente SELECT) porque este ambiente local não tinha
...
-- classificar_questoes_unidades_direitos_humanos_no_mercosul.sql (a
...
-- (Q162, Q163). Q164 sem artigo (fundamento institucional/doutrinario —
-- principio da complementaridade). Art. 2 do Protocolo de Assuncao,
-- Protocolo de Ushuaia, Decisao CMC 40/04 (RAADH) e Decisao CMC 14/09
-- (IPPDH) documentados apenas em escopo/mapa/_nota como contexto, sem
-- entrar em artigos_esperados.

-- 1) A(s) 1 unidade(s) pedagogica(s) do conteudo 91, com
...
where q.ativa = true
  ...
  and not exists (
    ...
    from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where qup.questao_id = q.id and u.curso_conteudo_id = 91
  );

-- 6) Confirma que nenhuma classificacao vazou de/para outro conteudo —
--    todo vinculo de uma unidade do conteudo 91 aponta para questao
...
--    fec87f6c-c735-46f1-8bd1-7bbaf6f56e93): curso_conteudos=93,
...
-- 8) Confirma exatamente o artigo_esperado aprovado (art. 1) e que
