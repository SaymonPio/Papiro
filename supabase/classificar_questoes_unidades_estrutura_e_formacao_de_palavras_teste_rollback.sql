...
-- questoes de Estrutura e formação de palavras (curso_conteudos.id = 33)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/estrutura_e_formacao_de_palavras.unidades.json + config/estrutura_e_formacao_de_palavras.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_estrutura_e_formacao_de_palavras.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_estrutura_e_formacao_de_palavras.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (231, '1a6bb75c-23d7-4110-be96-8803cbd85331', 1, 'alta'),
...
begin
  ...
  insert into _relatorio values (
    'conteudo_33_materia_assunto',
    v_materia_id is not distinct from 6 and v_assunto_id is not distinct from 45,
    format('materia_id=%s assunto_id=%s (esperado 6/45)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '1a6bb75c-23d7-4110-be96-8803cbd85331' and curso_conteudo_id = 33 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 33 — depende de curadoria_unidades_estrutura_e_formacao_de_palavras.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 33) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 33 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 33)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=6/assunto_id=45', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 33', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '1a6bb75c-23d7-4110-be96-8803cbd85331';
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_estrutura_e_formacao_de_palavras) ===';
