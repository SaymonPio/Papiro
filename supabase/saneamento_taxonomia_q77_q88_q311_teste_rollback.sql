...
-- Operacao atomica: as 3 questoes migram juntas, ou nenhuma migra.
--
...
-- inalterados. A decisao estrutural sobre o futuro dessa unidade
-- (SEM_CORPUS_PROPRIO_POR_SOBREPOSICAO_CURRICULAR) e separada e
-- posterior a este saneamento. NAO fecha a ordem 86.
--
...
-- alvo): primeiro muda o assunto_id de cada questao, so entao cria os
-- vinculos.
--
-- Harness de teste (SEMPRE termina em ROLLBACK) — nada aqui persiste no
-- banco. Ver saneamento_taxonomia_q77_q88_q311.sql para a aplicacao
-- real (termina em COMMIT).
...
begin
  ...
  raise notice 'tudo_ok = true — Q77/Q88/Q311 reclassificadas para assunto_id=35 com exatamente 1 vinculo cada na unidade Negacao de proposicoes (total 7: 77,81,86,88,311,312,337), assunto 34 com 0 candidatas ativas, unidade De Morgan intocada (ativa, 0 vinculos).';
end $$;
