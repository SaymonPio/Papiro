...
-- permanente; ver atualizar_artigos_esperados_dgf_u1_final.sql para o
-- apply real) — unidade "Direitos Individuais e Coletivos Fundamentais"
-- (BM RS), incluindo os incisos XV e XIX do art. 5º.
--
...
from _snapshot_old_unidade_u1_final t
where up.id = t.id;
...
begin
  ...
  join _snapshot_old_unidade_u1_final t on t.id = up.id
  ...
  raise notice 'OLD_FINAL OK: escopo e artigos_esperados identicos ao estado OLD original (14 itens, sem XV/XIX) — reversao comprovada';
end $$;
