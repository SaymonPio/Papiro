-- Manutencao pos-apply DH-AUT-06: correcao do campo explicacao das
-- questoes LIVE pre-existentes Q180/Q181/Q182 (cc92 - Protocolo de
-- Assuncao sobre Direitos Humanos no Mercosul) e Q189/Q190/Q191 (cc94 -
-- Tratado de Marraqueche), cujas explicacoes estavam corrompidas com
-- conteudo de outras questoes/unidades (Corte IDH/CADH em Q180-Q182;
-- DUDH arts.11/12/13 em Q189-Q191).
--
-- Escopo estritamente estreito: altera SOMENTE public.questoes.explicacao
-- das 6 linhas identificadas por id + guarda dupla (hash md5 do texto
-- atual + fingerprint imutavel dos campos que nao podem mudar). Nenhum
-- outro campo (enunciado, alternativas, ordem, gabarito, dificuldade,
-- banca, concurso, ano, materia_id, assunto_id, gerada_por_ia, ativa) e
-- vinculo em questao_unidades_pedagogicas e tocado.
--
-- Textos novos representados em base64 (decode + convert_from) para
-- eliminar qualquer risco de alteracao acidental de bytes/acentuacao
-- durante a transcricao para este arquivo. O md5 de cada texto
-- decodificado e conferido em precondicao e em poscondicao contra os
-- hashes congelados na auditoria de manutencao.
--
...
-- (old_explicacao_md5) E o fingerprint imutavel (id+enunciado+
-- dificuldade+ativa+alternativas) bater com o congelado no freeze.
-- Apos a primeira aplicacao bem-sucedida, o old_explicacao_md5 deixa de
-- existir no banco, de modo que uma segunda execucao deste arquivo
-- abortara na precondicao (0 linhas encontradas com os hashes antigos).
