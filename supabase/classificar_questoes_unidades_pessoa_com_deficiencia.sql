...
-- supabase/mapa_classificacao_pessoa_com_deficiencia.sql,
...
-- questoes/alternativas/unidades_pedagogicas/aulas/histórico — Q141 e
-- Q827, quase-duplicatas, permanecem ambas intactas e classificadas).
-- Segue o mesmo padrão de
-- supabase/classificar_questoes_unidades_estatuto_da_igualdade_racial.sql.
--
-- PRÉ-REQUISITO: supabase/curadoria_unidades_pessoa_com_deficiencia.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (30, '435543fe-bdc2-452a-be2d-ffa414c5e27d'::uuid, 1, 'alta'),
...
begin
  ...
  if v_materia_id is null then
    raise exception 'Precondicao falhou: curso_conteudos 71 nao existe';
  end if;
  if v_materia_id is distinct from 11 or v_assunto_id is distinct from 27 then
    raise exception 'Precondicao falhou: conteudo 71 materia_id=% assunto_id=% (esperado 11/27)', v_materia_id, v_assunto_id;
  end if;

  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 71) <> 1 then
    raise exception 'Precondicao falhou: nao ha exatamente 1 unidade pedagogica para o conteudo 71 — decisao aprovada foi manter unidade unica';
  end if;

  if not exists (select 1 from public.unidades_pedagogicas where id = '435543fe-bdc2-452a-be2d-ffa414c5e27d' and curso_conteudo_id = 71 and ordem = 1 and ativa)
  then
    raise exception 'Precondicao falhou: a unidade oficial nao confere (id/ordem/conteudo/ativa) — curadoria_unidades_pessoa_com_deficiencia.sql precisa ter sido aplicado antes';
  end if;
...
begin
  ...
  if v_invalidas <> 0 then
    raise exception 'Mapa invalido: % linha(s) apontam para questao que nao esta ativa=true/materia_id=11/assunto_id=27', v_invalidas;
  end if;
  ...
  if v_unidade_fora <> 0 then
    raise exception 'Mapa invalido: % linha(s) referenciam unidade pedagogica fora do conteudo 71', v_unidade_fora;
  end if;
end $$;
...
begin
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 71) <> 1 then
    raise exception 'Pos-condicao falhou: quantidade de unidades do conteudo 71 != 1';
  end if;
