...
-- questoes de Editor de textos (curso_conteudos.id = 36)
-- nas 1 unidade(s) pedagogica(s) aprovada(s). Gerado pelo
-- pipeline automatico (scripts/curadoria-pedagogica/gerar-rollback.mjs) a
-- partir de config/editor_de_textos.unidades.json + config/editor_de_textos.mapa.json. Aplica o
...
--
-- PRE-REQUISITO: supabase/curadoria_unidades_editor_de_textos.sql precisa ja ter sido
...
--
-- Diferenca deste arquivo para classificar_questoes_unidades_editor_de_textos.sql
...
insert into _mapa (questao_id, unidade_pedagogica_id, ordem_unidade, confianca) values
  (61, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (91, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (92, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (96, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (104, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (110, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (341, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (627, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (628, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (629, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (701, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (769, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta'),
  (831, '71df17f6-18a9-49c8-a8da-025329e43bc7', 1, 'alta');
...
begin
  ...
  insert into _relatorio values (
    'conteudo_36_materia_assunto',
    v_materia_id is not distinct from 9 and v_assunto_id is not distinct from 61,
    format('materia_id=%s assunto_id=%s (esperado 9/61)', v_materia_id, v_assunto_id)
  );

  select bool_and(x.ok) into v_unidades_ok from (
    select exists (select 1 from public.unidades_pedagogicas where id = '71df17f6-18a9-49c8-a8da-025329e43bc7' and curso_conteudo_id = 36 and ordem = 1 and ativa) as ok
  ) x;

  insert into _relatorio values (
    'unidades_oficiais_existem',
    coalesce(v_unidades_ok, false),
    'todas as 1 unidade(s) oficial(is) precisam existir, ativas, no conteudo 36 — depende de curadoria_unidades_editor_de_textos.sql ja aplicado'
  );
  ...
  if (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 36) <> 1 then
    insert into _relatorio values ('exatamente_1_unidade_s', false,
      format('unidades do conteudo 36 = %s (esperado 1)', (select count(*) from public.unidades_pedagogicas where curso_conteudo_id = 36)));
  else
...
begin
  ...
  insert into _relatorio values (
    ...
    v_invalidas = 0,
    format('%s linha(s) apontam para questao fora de ativa=true/materia_id=9/assunto_id=61', v_invalidas)
  );
  ...
  insert into _relatorio values (
    ...
    'as questoes excluidas intencionalmente (791, 832) nao devem constar no mapa'
  );
  ...
  insert into _relatorio values (
    ...
    v_unidade_fora = 0,
    format('%s linha(s) referenciam unidade pedagogica fora do conteudo 36', v_unidade_fora)
  );
end $$;
...
begin
  ...
  from public.questao_unidades_pedagogicas qup
  where qup.unidade_pedagogica_id = '71df17f6-18a9-49c8-a8da-025329e43bc7';
  ...
  select exists (
    ...
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 36 and qup.questao_id = 791
  ) into v_excluida_tmp;
  insert into _relatorio values ('791_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 791 (PROBLEMA_DE_FIDELIDADE_IMAGEM_Q791 — CONFIRMADO POR FONTE PRIMÁRIA. Localizada e consultada a prova original (Fundatec, Guarda Municipal de Imbé/RS, concurso 01/2023, Questão 16 do caderno de Informática, PDF oficial via qconcursos), que mostra o enunciado "Assinale a alternativa que apresenta INCORRETAMENTE a funcionalidade do botão apresentado no LibreOffice Writer" com cada alternativa (A a E) composta por um ÍCONE GRÁFICO REAL seguido do nome da função (a imagem do ícone está efetivamente embutida no PDF). A versão viva no Papiro substituiu cada ícone por descrição textual (ex.: "Ícone do Pincel de Pintura – Clonar Formatação"), entregando ao aluno a informação que ele deveria reconhecer visualmente e alterando a habilidade nuclear de RECONHECER VISUALMENTE O ÍCONE para associar uma função a um ícone já nomeado por escrito. Q791 permanece ATIVA, INTACTA e SEM vínculo — não reconstruída, sem invenção de mídia, sem alteração de enunciado. Continua sendo evidência de INCIDÊNCIA REAL histórica (LibreOffice Writer e reconhecimento de ícones/funções foram efetivamente cobrados pela Fundatec), mas não pode ser prática específica vinculada enquanto a fidelidade visual estiver comprometida.) nao deve ter sido classificada por este arquivo');

  select exists (
    select 1 from public.questao_unidades_pedagogicas qup
    join public.unidades_pedagogicas u on u.id = qup.unidade_pedagogica_id
    where u.curso_conteudo_id = 36 and qup.questao_id = 832
  ) into v_excluida_tmp;
  insert into _relatorio values ('832_permanece_nao_classificada', not coalesce(v_excluida_tmp, false), 'a questao 832 (DUPLICATA_Q832 — mesmo enunciado, mesmas 5 alternativas (Tab, Ctrl+C, Ctrl+X, Shift+End, Shift+Enter) e mesmo gabarito (C = Ctrl+X) de Q91, mesmo evento SUSEPE/Polícia Penal RS 01/2022 (cargos diferentes, mesma prova de origem). Q832 possui glitch textual "área detransferência" (sem espaço), enquanto Q91 é a versão mais limpa e foi escolhida como questão canônica vinculada. Q832 permanece ATIVA, INTACTA e SEM vínculo neste conteúdo — não corrigida, não desativada, não contabilizada como segunda incidência independente do fenômeno de atalhos de recortar.) nao deve ter sido classificada por este arquivo');
...
begin
  ...
  raise notice '=== RELATORIO DO TESTE (classificar_questoes_unidades_editor_de_textos) ===';
